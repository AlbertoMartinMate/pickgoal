(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const B="/api";function S(){return localStorage.getItem("token")}async function l(e,a={}){const t={"Content-Type":"application/json",...a.headers},s=S();s&&(t.Authorization=`Bearer ${s}`);const n=await fetch(`${B}${e}`,{...a,headers:t}),i=await n.json().catch(()=>({}));if(!n.ok)throw{status:n.status,message:i.error||"Error desconocido"};return i}const m={get:e=>l(e),post:(e,a)=>l(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>l(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>l(e,{method:"DELETE"}),auth:{register:e=>l("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>l("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>l("/auth/me"),forgotPassword:e=>l("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>l("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:()=>l("/auth/ranking"),users:()=>l("/auth/users"),toggleAdmin:e=>l(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>l("/matches/grouped"),list:(e="")=>l(`/matches/${e}`),get:e=>l(`/matches/${e}`),sync:()=>l("/matches/sync",{method:"POST"})},predictions:{mine:()=>l("/predictions/"),forMatch:e=>l(`/predictions/match/${e}`),save:e=>l("/predictions/",{method:"POST",body:JSON.stringify(e)}),getChampion:()=>l("/predictions/champion"),saveChampion:e=>l("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e})}),awardChampion:e=>l("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{public:()=>l("/leagues/public"),my:()=>l("/leagues/my"),create:e=>l("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>l("/leagues/join",{method:"POST",body:JSON.stringify(e)}),get:e=>l(`/leagues/${e}`),leave:e=>l(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>l(`/leagues/${e}/predictions/${a}`)},board:{messages:(e=1)=>l(`/board/?page=${e}`),post:e=>l("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>l(`/board/${e}`,{method:"DELETE"})}};let h=null;const p={async init(){if(localStorage.getItem("token"))try{const{user:a}=await m.auth.me();h=a}catch{localStorage.removeItem("token")}},setUser(e,a){h=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){h=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return h},isLoggedIn(){return!!h},isAdmin(){return(h==null?void 0:h.is_admin)===!0}};function M(e){const a=p.getUser();e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),s=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",s.classList.add("hidden");const n={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await m.auth.register(n);p.setUser(o,i),u("¡Cuenta creada! Bienvenido a PickGoal"),g.navigate("/campeon")}catch(i){s.textContent=i.message||"Error al registrarse",s.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}async function k(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[{groups:a},t]=await Promise.all([m.matches.grouped(),p.isLoggedIn()?m.predictions.mine():Promise.resolve({predictions:[]})]),s={};for(const o of t.predictions)s[o.match_id]=o;e.innerHTML=`
    ...
    <div class="match-card ${t?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!t&&p.isLoggedIn()?q(e,a):t&&a?`<div class="prediction-result">
               Tu predicción: <strong>${a.predicted_home}-${a.predicted_away}</strong>
               (${a.predicted_result}) · ${a.total_points} pts
             </div>`:""}
    </div>
  `}function q(e,a){const t=(a==null?void 0:a.predicted_home)??"",s=(a==null?void 0:a.predicted_away)??"";return`
  ...
  `}function A(e,a){e.addEventListener("submit",async t=>{t.preventDefault();const s=parseInt(e.dataset.matchId),n=parseInt(e.querySelector("[name=predicted_home]").value),i=parseInt(e.querySelector("[name=predicted_away]").value);if(isNaN(n)||isNaN(i))return;let o="1";n===i?o="X":i>n&&(o="2");const r=e.querySelector("button");r.disabled=!0,r.textContent="…";try{const{prediction:c}=await m.predictions.save({match_id:s,predicted_result:o,predicted_home:n,predicted_away:i});a[s]=c,u("Predicción guardada"),r.textContent="✓ Guardado",setTimeout(()=>{r.disabled=!1,r.textContent="Guardar"},2e3)}catch(c){u(c.message||"Error al guardar","error"),r.disabled=!1,r.textContent="Guardar"}})}async function O(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{ranking:a}=await m.auth.ranking(),t=p.getUser();e.innerHTML=`
      <div class="container">
    ...
    `}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}async function U(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let a=1,t=1;async function s(){const{messages:r,pages:c}=await m.board.messages(a);return t=c,r}try{const r=await s();n(r)}catch(r){e.innerHTML=`<div class="container"><p class="form__error">Error: ${r.message}</p></div>`}function n(r){var d,_,y;const c=p.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(d=document.getElementById("boardForm"))==null||d.addEventListener("submit",async v=>{v.preventDefault();const E=document.getElementById("boardMsg"),$=E.value.trim();if($)try{await m.board.post($),E.value="";const b=await s();document.getElementById("boardMessages").innerHTML=i(b,c),o(c),u("Mensaje publicado")}catch(b){u(b.message,"error")}}),(_=document.getElementById("prevPage"))==null||_.addEventListener("click",async()=>{a--;const v=await s();document.getElementById("boardMessages").innerHTML=i(v,c),o(c)}),(y=document.getElementById("nextPage"))==null||y.addEventListener("click",async()=>{a++;const v=await s();document.getElementById("boardMessages").innerHTML=i(v,c),o(c)}),o(c)}function i(r,c){return r.length?r.map(d=>`
      <div class="board-message ${d.is_deleted?"board-message--deleted":""}" data-id="${d.id}">
        ...
        <p class="board-message__text">${j(d.message)}</p>
      </div>
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function o(r){e.querySelectorAll(".delete-msg").forEach(c=>{c.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await m.board.delete(c.dataset.id);const d=await s();document.getElementById("boardMessages").innerHTML=i(d,r),o(r),u("Mensaje eliminado")}catch(d){u(d.message,"error")}})})}}function j(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function R(e){var a,t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[s,n]=await Promise.all([m.leagues.public(),p.isLoggedIn()?m.leagues.my():Promise.resolve({leagues:[]})]),i=p.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,e.querySelectorAll(".league-card").forEach(o=>{o.addEventListener("click",()=>{g.navigate(`/ligas/${o.dataset.id}`)})}),(a=document.getElementById("joinCodeForm"))==null||a.addEventListener("submit",async o=>{o.preventDefault();const r=document.getElementById("inviteCode").value.trim().toUpperCase();if(r)try{const{league:c}=await m.leagues.join({invite_code:r});u(`Te has unido a "${c.name}"`),g.navigate(`/ligas/${c.id}`)}catch(c){u(c.message,"error")}}),(t=document.getElementById("createLeagueForm"))==null||t.addEventListener("submit",async o=>{o.preventDefault();const r=document.getElementById("leagueName").value.trim(),c=document.getElementById("isPublic").checked;try{const{league:d}=await m.leagues.create({name:r,is_public:c});u(`Liga "${d.name}" creada`),g.navigate(`/ligas/${d.id}`)}catch(d){u(d.message,"error")}})}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error: ${s.message}</p></div>`}}function L(e){return`
  ...
  `}async function F(e,{params:a}){var s,n;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:i,ranking:o,is_member:r}=await m.leagues.get(t),c=p.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(s=document.getElementById("btnLeave"))==null||s.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await m.leagues.leave(t),u("Has abandonado la liga"),g.navigate("/ligas")}catch(d){u(d.message,"error")}}),(n=document.getElementById("btnJoin"))==null||n.addEventListener("click",async()=>{try{await m.leagues.join({league_id:t}),u("¡Te has unido a la liga!"),g.navigate(`/ligas/${t}`)}catch(d){u(d.message,"error")}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p><a href="#/ligas">Volver</a></div>`}}async function D(e){var t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=p.getUser();try{const[s,n,i]=await Promise.all([m.predictions.mine(),m.predictions.getChampion(),m.leagues.my()]),o=s.predictions.reduce((r,c)=>r+c.total_points,0)+(((t=n.champion_prediction)==null?void 0:t.points_earned)||0);e.innerHTML=`
      <div class="container">
  ...
  `}const w=new Date("2026-06-11T00:00:00Z"),G=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];async function P(e){var a;if(!p.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{champion_prediction:t}=await m.predictions.getChampion(),s=new Date>=w;e.innerHTML=`
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const t=document.getElementById("syncResult");t.textContent="Sincronizando…";try{await m.matches.sync(),t.textContent="✓ Sincronización completada",u("Sincronización completada")}catch(s){t.textContent=`Error: ${s.message}`,u(s.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async t=>{t.preventDefault();const s=document.getElementById("winnerTeam").value.trim();if(s)try{const{message:n}=await m.predictions.awardChampion(s);u(n)}catch(n){u(n.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async t=>{const s=t.target.closest(".toggle-admin");if(!s)return;const n=parseInt(s.dataset.id);try{const{user:i}=await m.auth.toggleAdmin(n);s.closest("tr").querySelector(".admin-badge").textContent=i.is_admin?"Sí":"No",u(`${i.username} ${i.is_admin?"ahora es admin":"ya no es admin"}`)}catch(i){u(i.message,"error")}})}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}function V(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
