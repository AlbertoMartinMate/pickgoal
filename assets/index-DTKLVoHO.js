(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function a(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=a(n);fetch(n.href,i)}})();const O="https://pickgoal-backend.onrender.com/api";function U(){return localStorage.getItem("token")}async function u(e,t={}){const a={"Content-Type":"application/json",...t.headers},s=U();s&&(a.Authorization=`Bearer ${s}`);const n=await fetch(`${O}${e}`,{...t,headers:a}),i=await n.json().catch(()=>({}));if(!n.ok)throw{status:n.status,message:i.error||"Error desconocido"};return i}const m={get:e=>u(e),post:(e,t)=>u(e,{method:"POST",body:JSON.stringify(t)}),patch:(e,t)=>u(e,{method:"PATCH",body:JSON.stringify(t)}),delete:e=>u(e,{method:"DELETE"}),auth:{register:e=>u("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>u("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>u("/auth/me"),forgotPassword:e=>u("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,t)=>u("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:t})}),ranking:()=>u("/auth/ranking"),users:()=>u("/auth/users"),toggleAdmin:e=>u(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>u("/matches/grouped"),list:(e="")=>u(`/matches/${e}`),get:e=>u(`/matches/${e}`),sync:()=>u("/matches/sync",{method:"POST"})},predictions:{mine:()=>u("/predictions/"),forMatch:e=>u(`/predictions/match/${e}`),save:e=>u("/predictions/",{method:"POST",body:JSON.stringify(e)}),getChampion:()=>u("/predictions/champion"),saveChampion:e=>u("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e})}),awardChampion:e=>u("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>u("/leagues/all"),public:()=>u("/leagues/public"),my:()=>u("/leagues/my"),create:e=>u("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>u("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>u(`/leagues/join/${encodeURIComponent(e)}`),get:e=>u(`/leagues/${e}`),leave:e=>u(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,t)=>u(`/leagues/${e}/predictions/${t}`)},board:{messages:(e=1)=>u(`/board/?page=${e}`),post:e=>u("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>u(`/board/${e}`,{method:"DELETE"})}};let y=null;const h={async init(){if(localStorage.getItem("token"))try{const{user:t}=await m.auth.me();y=t}catch{localStorage.removeItem("token")}},setUser(e,t){y=e,localStorage.setItem("token",t),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){y=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return y},isLoggedIn(){return!!y},isAdmin(){return(y==null?void 0:y.is_admin)===!0}};function R(e){const t=h.getUser();e.innerHTML=`
  ...
  `}let I=null;function p(e,t="success"){let a=document.getElementById("toast");a||(a=document.createElement("div"),a.id="toast",document.body.appendChild(a)),a.textContent=e,a.className=`toast toast--${t} toast--visible`,I&&clearTimeout(I),I=setTimeout(()=>{a.classList.remove("toast--visible")},3e3)}function j(){return`
    <div class="container">
  ...
  `}function S(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}function G(e){e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("registerBtn"),s=document.getElementById("registerError");a.disabled=!0,a.textContent="Creando cuenta…",s.classList.add("hidden");const n={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:r}=await m.auth.register(n);h.setUser(r,i),p("¡Cuenta creada! Bienvenido a PickGoal");const o=sessionStorage.getItem("pendingInviteCode");if(o){sessionStorage.removeItem("pendingInviteCode");try{const{league:c}=await m.leagues.joinByCode(o);p(`¡Te has unido a "${c.name}"!`),_.navigate(`/ligas/${c.id}`)}catch{_.navigate("/ligas")}}else _.navigate("/campeon")}catch(i){s.textContent=i.message||"Error al registrarse",s.classList.remove("hidden")}finally{a.disabled=!1,a.textContent="Crear cuenta"}})}async function z(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:d}=await m.leagues.my();if(d.length===0){e.innerHTML=j();return}}const[{groups:t},a]=await Promise.all([m.matches.grouped(),h.isLoggedIn()?m.predictions.mine():Promise.resolve({predictions:[]})]),s={};for(const d of a.predictions)s[d.match_id]=d;const n=t.flatMap(d=>d.matches),i=new Map;for(const d of n){const l=P(d.match_datetime);i.has(l)||i.set(l,[]),i.get(l).push(d)}const r=[...i.keys()].sort(),o=P(new Date().toISOString()),c=r.find(d=>d>=o)??r[0];e.innerHTML=`
      <div class="container">
    ...
    `,J(r,c,i,s),h.isLoggedIn()&&X(n,s)}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${t.message}</p></div>`}}function J(e,t,a,s){var i;const n=document.getElementById("dateNav");n&&(n.innerHTML=e.map(r=>`
    <button class="date-nav__btn ${r===t?"date-nav__btn--active":""}" data-day="${r}">
      ${V(r)}
    </button>
  `).join(""),(i=n.querySelector(".date-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),n.querySelectorAll(".date-nav__btn").forEach(r=>{r.addEventListener("click",()=>{n.querySelectorAll(".date-nav__btn").forEach(o=>o.classList.remove("date-nav__btn--active")),r.classList.add("date-nav__btn--active"),C(a.get(r.dataset.day)??[],s)})}),C(a.get(t)??[],s))}function C(e,t){const a=document.getElementById("matchesContent");if(a){if(e.length===0){a.innerHTML='<p class="empty">Sin partidos este día.</p>';return}a.innerHTML=`<div class="matches-grid">${e.map(s=>Y(s,t[s.id])).join("")}</div>`,h.isLoggedIn()&&a.querySelectorAll(".prediction-form").forEach(s=>{K(s,t)})}}function P(e){const t=new Date(e);return`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}-${String(t.getDate()).padStart(2,"0")}`}function V(e){const[t,a,s]=e.split("-").map(Number);return new Date(t,a-1,s).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function Y(e,t){const a=e.is_locked,s=t?`<span class="pts-badge">${t.total_points} pts</span>`:"",n={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
    <div class="match-card ${a?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!a&&h.isLoggedIn()?Q(e,t):a&&t?`<div class="prediction-result">
               Tu predicción: <strong>${t.predicted_home}-${t.predicted_away}</strong>
               (${t.predicted_result}) · ${t.total_points} pts
             </div>`:""}
    </div>
  `}function Q(e,t){const a=(t==null?void 0:t.predicted_home)??0,s=(t==null?void 0:t.predicted_away)??0,n=(t==null?void 0:t.predicted_result)??"X";return`
  ...
  `}async function X(e,t){const a=e.filter(s=>!s.is_locked&&!t[s.id]);for(const s of a)try{const{prediction:n}=await m.predictions.save({match_id:s.id,predicted_result:"X",predicted_home:0,predicted_away:0});t[s.id]=n}catch{}}function K(e,t){e.addEventListener("submit",async a=>{var c;a.preventDefault();const s=parseInt(e.dataset.matchId),n=parseInt(e.querySelector("[name=predicted_home]").value),i=parseInt(e.querySelector("[name=predicted_away]").value),r=(c=e.querySelector("[name=predicted_result]:checked"))==null?void 0:c.value;if(isNaN(n)||isNaN(i)||!r)return;const o=e.querySelector("button");o.disabled=!0,o.textContent="…";try{const{prediction:d}=await m.predictions.save({match_id:s,predicted_result:r,predicted_home:n,predicted_away:i});t[s]=d,p("Predicción guardada"),o.textContent="✓ Guardado",setTimeout(()=>{o.disabled=!1,o.textContent="Guardar"},2e3)}catch(d){p(d.message||"Error al guardar","error"),o.disabled=!1,o.textContent="Guardar"}})}async function W(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:s}=await m.leagues.my();if(s.length===0){e.innerHTML=j();return}}const{ranking:t}=await m.auth.ranking(),a=h.getUser();e.innerHTML=`
      <div class="container">
    ...
    `}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}async function Z(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let t=1,a=1;async function s(){const{messages:o,pages:c}=await m.board.messages(t);return a=c,o}try{const o=await s();n(o)}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p></div>`}function n(o){var d,l,v;const c=h.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(d=document.getElementById("boardForm"))==null||d.addEventListener("submit",async g=>{g.preventDefault();const b=document.getElementById("boardMsg"),L=b.value.trim();if(L)try{await m.board.post(L),b.value="";const $=await s();document.getElementById("boardMessages").innerHTML=i($,c),r(c),p("Mensaje publicado")}catch($){p($.message,"error")}}),(l=document.getElementById("prevPage"))==null||l.addEventListener("click",async()=>{t--;const g=await s();document.getElementById("boardMessages").innerHTML=i(g,c),r(c)}),(v=document.getElementById("nextPage"))==null||v.addEventListener("click",async()=>{t++;const g=await s();document.getElementById("boardMessages").innerHTML=i(g,c),r(c)}),r(c)}function i(o,c){return o.length?o.map(d=>`
      <div class="board-message ${d.is_deleted?"board-message--deleted":""}" data-id="${d.id}">
        ...
        <p class="board-message__text">${ee(d.message)}</p>
      </div>
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function r(o){e.querySelectorAll(".delete-msg").forEach(c=>{c.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await m.board.delete(c.dataset.id);const d=await s();document.getElementById("boardMessages").innerHTML=i(d,o),r(o),p("Mensaje eliminado")}catch(d){p(d.message,"error")}})})}}function ee(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function te(e){var t,a,s,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[i,r]=await Promise.all([m.leagues.all(),h.isLoggedIn()?m.leagues.my():Promise.resolve({leagues:[]})]),o=h.getUser(),c=new Set(r.leagues.map(l=>l.id)),d=i.leagues.filter(l=>!c.has(l.id));e.innerHTML=`
      <div class="container">
  ...
  `}function ae(e){var s,n;const t=e.invite_link||"",a=document.getElementById("createLeaguePanel");a&&(a.innerHTML=`
  ...
  `,(s=document.getElementById("btnCopyLink"))==null||s.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(t),p("Enlace copiado")}catch{p("No se pudo copiar","error")}}),(n=document.getElementById("btnShare"))==null||n.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:t})}catch{}}))}async function se(e,{params:t}){var s,n,i,r;const a=parseInt(t.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:o,ranking:c,is_member:d}=await m.leagues.get(a),l=h.getUser(),v=o.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
    ...
    `,(s=document.getElementById("btnCopyInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(o.invite_link),p("Enlace copiado")}catch{p("No se pudo copiar","error")}}),(n=document.getElementById("btnShareInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${o.name} en PickGoal`,url:o.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await m.leagues.leave(a),p("Has abandonado la liga"),_.navigate("/ligas")}catch(g){p(g.message,"error")}}),(r=document.getElementById("btnJoin"))==null||r.addEventListener("click",async()=>{try{await m.leagues.join({league_id:a}),p("¡Te has unido a la liga!"),_.navigate(`/ligas/${a}`)}catch(g){p(g.message,"error")}})}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p><a href="#/ligas">Volver</a></div>`}}async function ne(e){var a,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const t=h.getUser();try{const[n,i,r]=await Promise.all([m.predictions.mine(),m.predictions.getChampion(),m.leagues.my()]),o=n.predictions.reduce((c,d)=>c+d.total_points,0)+(((a=i.champion_prediction)==null?void 0:a.points_earned)||0);e.innerHTML=`
      <div class="container">
        ...
        <section class="section">
          ...
          ${n.predictions.length?`<div class="predictions-list">${n.predictions.map(ie).join("")}</div>`:'<p class="empty">Sin predicciones aún. <a href="#/quiniela">Ir a la quiniela</a></p>'}
        </section>
    ...
    `,(s=e.querySelector("#btnLogoutPerfil"))==null||s.addEventListener("click",()=>{h.logout(),window.location.hash="/"})}catch(n){e.innerHTML=`<div class="container"><p class="form__error">Error: ${n.message}</p></div>`}}function ie(e){return`
  ...
  `}const k=new Date("2026-06-11T00:00:00Z"),re=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];async function q(e){var t;if(!h.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{champion_prediction:a}=await m.predictions.getChampion(),s=new Date>=k;e.innerHTML=`
      <div class="container">
        ...
        <p class="champion-desc">
          ...
          (${k.toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric"})}).
        </p>
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const a=document.getElementById("syncResult");a.textContent="Sincronizando…";try{await m.matches.sync(),a.textContent="✓ Sincronización completada",p("Sincronización completada")}catch(s){a.textContent=`Error: ${s.message}`,p(s.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async a=>{a.preventDefault();const s=document.getElementById("winnerTeam").value.trim();if(s)try{const{message:n}=await m.predictions.awardChampion(s);p(n)}catch(n){p(n.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async a=>{const s=a.target.closest(".toggle-admin");if(!s)return;const n=parseInt(s.dataset.id);try{const{user:i}=await m.auth.toggleAdmin(n);s.closest("tr").querySelector(".admin-badge").textContent=i.is_admin?"Sí":"No",p(`${i.username} ${i.is_admin?"ahora es admin":"ya no es admin"}`)}catch(i){p(i.message,"error")}})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}function ce(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
  ...
  `,document.getElementById("resetForm").addEventListener("submit",async s=>{s.preventDefault();const n=document.getElementById("resetBtn"),i=document.getElementById("resetError"),r=document.getElementById("password").value;n.disabled=!0,n.textContent="Guardando…",i.classList.add("hidden");try{await m.auth.resetPassword(a,r),p("Contraseña actualizada. Ya puedes iniciar sesión."),_.navigate("/login")}catch(o){i.textContent=o.message||"Error al restablecer la contraseña",i.classList.remove("hidden")}finally{n.disabled=!1,n.textContent="Guardar contraseña"}})}const me={r32:"Dieciseisavos",r16:"Octavos",quarters:"Cuartos",semis:"Semis",third:"3º y 4º",final:"Final"};async function ue(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{groups:t}=await m.matches.grouped();e.innerHTML=`
      <div class="container">
        <h1 class="page-title">Resultados — Mundial 2026</h1>
        <nav class="phase-nav" id="phaseNav"></nav>
        <div id="phaseContent"></div>
      </div>
    `,pe(t)}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${t.message}</p></div>`}}function pe(e){var i;const t=document.getElementById("phaseNav");if(!t)return;const a=e.filter(r=>r.phase==="group"),s=e.filter(r=>r.phase!=="group"),n=[...a.map(r=>({key:`group_${r.group_name}`,label:`Grupo ${r.group_name}`,data:r,isGroup:!0})),...s.map(r=>({key:r.phase,label:me[r.phase]||r.label,data:r,isGroup:!1}))];n.length!==0&&(t.innerHTML=n.map((r,o)=>`
    <button class="phase-nav__btn ${o===0?"phase-nav__btn--active":""}" data-key="${r.key}">
      ${r.label}
    </button>
  `).join(""),(i=t.querySelector(".phase-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),t.querySelectorAll(".phase-nav__btn").forEach((r,o)=>{r.addEventListener("click",()=>{t.querySelectorAll(".phase-nav__btn").forEach(d=>d.classList.remove("phase-nav__btn--active")),r.classList.add("phase-nav__btn--active");const c=n.find(d=>d.key===r.dataset.key);c&&M(c.data,c.isGroup)})}),M(n[0].data,n[0].isGroup))}function M(e,t){const a=document.getElementById("phaseContent");if(!a)return;const s=ge(e.matches);if(t){const n=he(e.matches);a.innerHTML=`
    ...
    `}).join("")}function he(e){const t={};for(const a of e)if(t[a.home_team]||(t[a.home_team]={name:a.home_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),t[a.away_team]||(t[a.away_team]={name:a.away_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),a.status==="finished"&&a.home_score_90!==null&&a.away_score_90!==null){const s=t[a.home_team],n=t[a.away_team];s.pj++,n.pj++,s.gf+=a.home_score_90,s.gc+=a.away_score_90,n.gf+=a.away_score_90,n.gc+=a.home_score_90,a.home_score_90>a.away_score_90?(s.g++,s.pts+=3,n.p++):a.home_score_90<a.away_score_90?(n.g++,n.pts+=3,s.p++):(s.e++,s.pts++,n.e++,n.pts++)}return Object.values(t).sort((a,s)=>{if(s.pts!==a.pts)return s.pts-a.pts;const n=s.gf-s.gc,i=a.gf-a.gc;return n!==i?n-i:s.gf-a.gf})}function ve(e){return e.length===0?'<p class="empty">Sin datos de clasificación.</p>':`
  ...
  `}async function _e(e,{query:t}){const a=(t.codigo||"").trim().toUpperCase();if(!a){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!h.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",a),_.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:s}=await m.leagues.joinByCode(a);p(`¡Te has unido a "${s.name}"!`),_.navigate(`/ligas/${s.id}`)}catch(s){if(s.status===409){p("Ya eres miembro de esta liga");try{const{leagues:n}=await m.leagues.my(),i=n.find(r=>r.invite_code===a);if(i){_.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
  ...
  `}function A(){const e=window.location.hash.slice(1).split("?")[0]||"/";document.querySelectorAll(".bottom-nav__item").forEach(t=>{const a=t.dataset.route,s=a==="/"?e==="/":e===a||e.startsWith(a+"/");t.classList.toggle("bottom-nav__item--active",s)})}ye();
