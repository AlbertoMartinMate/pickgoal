(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const ce="https://pickgoal-backend.onrender.com/api";function pe(){return localStorage.getItem("token")}async function g(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=pe();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${ce}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const c={get:e=>g(e),post:(e,a)=>g(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>g(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>g(e,{method:"DELETE"}),auth:{register:e=>g("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>g("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>g("/auth/me"),forgotPassword:e=>g("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>g("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>g(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>g("/auth/users"),toggleAdmin:e=>g(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>g("/matches/grouped"),list:(e="")=>g(`/matches/${e}`),get:e=>g(`/matches/${e}`),sync:()=>g("/matches/sync",{method:"POST"}),recalculate:()=>g("/matches/recalculate",{method:"POST"})},predictions:{mine:e=>g(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>g(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>g("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>g(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>g(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>g("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>g("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>g("/leagues/all"),public:()=>g("/leagues/public"),my:()=>g("/leagues/my"),create:e=>g("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>g("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>g(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>g("/leagues/admin"),get:e=>g(`/leagues/${e}`),update:(e,a)=>g(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(a)}),leave:e=>g(`/leagues/${e}/leave`,{method:"DELETE"}),members:e=>g(`/leagues/${e}/members`),matchPredictions:(e,a)=>g(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>g("/home/summary")},board:{messages:(e=1,a=null)=>g(`/board/?page=${e}${a?`&league_id=${a}`:""}`),unread:(e,a)=>g(`/board/unread?league_id=${e}&since=${encodeURIComponent(a)}`),post:(e,a=null)=>g("/board/",{method:"POST",body:JSON.stringify({message:e,league_id:a})}),pin:e=>g(`/board/${e}/pin`,{method:"POST"}),reply:(e,a)=>g(`/board/${e}/reply`,{method:"POST",body:JSON.stringify({message:a})}),delete:e=>g(`/board/${e}`,{method:"DELETE"})},notifications:{vapidPublicKey:()=>g("/notifications/vapid-public-key"),subscribe:e=>g("/notifications/subscribe",{method:"POST",body:JSON.stringify(e)}),send:e=>g("/notifications/send",{method:"POST",body:JSON.stringify(e)})}};let T=null;const y={async init(){if(localStorage.getItem("token"))try{const{user:a}=await c.auth.me();T=a}catch{localStorage.removeItem("token")}},setUser(e,a){T=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){T=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return T},isLoggedIn(){return!!T},isAdmin(){return(T==null?void 0:T.is_admin)===!0}};let R=null;function _(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,R&&clearTimeout(R),R=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function te(){return`
  ...
  `}function M(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}async function ue(e){if(!y.getUser()){me(e);return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{leagues_summary:t,upcoming_matches:n}=await c.home.summary();if(t.length===0){ge(e);return}const s=(()=>{const o=localStorage.getItem("activeLeagueId");return o?parseInt(o):null})(),i=[...t].sort((o,l)=>o.league_id===s?-1:l.league_id===s?1:0);e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await c.auth.register(s);y.setUser(o,i),_("¡Cuenta creada! Bienvenido a PickGoal");const l=sessionStorage.getItem("pendingInviteCode");if(l){sessionStorage.removeItem("pendingInviteCode");try{const{league:p}=await c.leagues.joinByCode(l);_(`¡Te has unido a "${p.name}"!`),L.navigate(`/ligas/${p.id}`)}catch{L.navigate("/ligas")}}else L.navigate("/campeon"),se("/ligas")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function V(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Ee(e){var a,t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{let n=null;if(y.isLoggedIn()){const{leagues:$}=await c.leagues.my();if($.length===0){e.innerHTML=te();return}const w=V(),k=$.find(r=>r.id===w);n=k?k.name:((a=$[0])==null?void 0:a.name)??null}const s=V(),[{groups:i},o,l]=await Promise.all([c.matches.grouped(),y.isLoggedIn()?c.predictions.mine(s):Promise.resolve({predictions:[]}),y.isLoggedIn()?c.predictions.getChampion(s):Promise.resolve({champion_prediction:null})]),p={};for(const $ of o.predictions)p[$.match_id]=$;const m=i.flatMap($=>$.matches),v=new Map;for(const $ of m){const w=W($.match_datetime);v.has(w)||v.set(w,[]),v.get(w).push($)}const u=[...v.keys()].sort(),b=W(new Date().toISOString()),h=u.find($=>$>=b)??u[0],f=((t=l.champion_prediction)==null?void 0:t.team_name)??null,S=y.isLoggedIn()?f?`<p class="champion-banner champion-banner--set">🏆 Tu campeón: <a href="#/campeon" style="color:inherit;font-weight:bold;">${f}</a></p>`:'<p class="champion-banner champion-banner--missing">⚠️ <a href="#/campeon">¡Elige tu campeón antes del inicio del torneo!</a></p>':"";e.innerHTML=`
    ...
    `,Le(u,h,v,p,s)}catch(n){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${n.message}</p></div>`}}function Le(e,a,t,n,s){var o;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(l=>`
  ...
  `}function Se(e,a){const t=!!a,n=(a==null?void 0:a.predicted_home)??0,s=(a==null?void 0:a.predicted_away)??0,i=(a==null?void 0:a.predicted_result)??"X",o=t?"prediction-form--saved":"prediction-form--unsaved",l=t?'<span class="pred-status pred-status--saved">✓ Guardado</span>':'<span class="pred-status pred-status--unsaved">Sin predicción</span>',p=t?"btn btn--saved btn--sm pred-save-btn":"btn btn--ghost btn--sm pred-save-btn",m=t?"✓ Guardado":"Guardar";return`
    <form class="prediction-form ${o}" data-match-id="${e.id}" data-saved="${t}">
      ${l}
      <div class="result-selector">
        ${["1","X","2"].map(v=>`
          <label class="result-selector__option">
            <input type="radio" name="predicted_result" value="${v}" ${i===v?"checked":""} required />
            ${v}
          </label>
        `).join("")}
      </div>
  ...
  `}function xe(e,a,t){const n=parseInt(e.dataset.matchId),s=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function l(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function p(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado",s.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(m=>{m.addEventListener("change",l),m.addEventListener("input",l)}),e.addEventListener("submit",async m=>{var h;m.preventDefault();const v=parseInt(e.querySelector("[name=predicted_home]").value),u=parseInt(e.querySelector("[name=predicted_away]").value),b=(h=e.querySelector("[name=predicted_result]:checked"))==null?void 0:h.value;if(!(isNaN(v)||isNaN(u)||!b)){s.disabled=!0,s.textContent="…";try{const{prediction:f}=await c.predictions.save({match_id:n,predicted_result:b,predicted_home:v,predicted_away:u,league_id:t??null});a[n]=f,_("Predicción guardada"),p()}catch(f){_(f.message||"Error al guardar","error"),s.disabled=!1,o?(s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado"):(s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar")}}})}function Ce(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ke(e){var a;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(y.isLoggedIn()){const{leagues:v}=await c.leagues.my();if(v.length===0){e.innerHTML=te();return}}const t=Ce(),[{ranking:n},s]=await Promise.all([c.auth.ranking(t),y.isLoggedIn()?c.leagues.my():Promise.resolve({leagues:[]})]),i=y.getUser(),o=s.leagues.find(v=>v.id===t),l=document.getElementById("tablonBadge"),p=l&&!l.classList.contains("hidden"),m=p?l.textContent:"";e.innerHTML=`
      ...
      <div class="container">
        <div class="ranking-header">
          ...
          ${t?`
            <button class="ranking-tablon-btn" data-league-id="${t}">
              💬 Tablón
              <span class="ranking-tablon-btn__badge${p?"":" hidden"}">${m}</span>
            </button>
          `:""}
        </div>
        <div class="ranking-table-wrapper">
          <table class="ranking-table">
            ...
            <tbody>
              ${n.map(v=>{var u,b;return`
                <tr class="${i&&v.id===i.id?"ranking-table__row--me":""}">
                  <td class="ranking-table__pos" data-pos="${v.position}">${v.position}</td>
                  <td>
                    <a class="ranking-table__link" href="#/jugador/${v.id}">
                      <span class="status-emoji" title="${((u=v.status)==null?void 0:u.name)||""}">${((b=v.status)==null?void 0:b.emoji)||""}</span>${v.username}
                    </a>
                  </td>
                  <td>${v.country||"—"}</td>
                  <td class="ranking-table__pts">${v.total_points}</td>
                  <td class="ranking-table__stat">${v.correct_results}</td>
                  <td class="ranking-table__stat">${v.exact_scores}</td>
                </tr>
              `}).join("")}
            </tbody>
          </table>
    ...
    `,f(),S(),$()}function v(r){return r.length?r.map(d=>`
    ...
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function h(r){const d=B(r);if(!i.length)return d;const E=i.map(I=>Pe(I.username)),x=new RegExp(`@(${E.join("|")})`,"gi");return d.replace(x,'<span class="mention">@$1</span>')}function f(){const r=document.getElementById("boardForm");if(!r)return;const d=document.getElementById("boardMsg"),E=document.getElementById("charCounter"),x=document.getElementById("mentionDropdown");d.addEventListener("input",()=>{E.textContent=`${d.value.length} / 500`,k(d,x)}),r.addEventListener("submit",async I=>{I.preventDefault();const j=d.value.trim();if(j)try{await c.board.post(j,n),d.value="",E.textContent="0 / 500",x.classList.add("hidden");const P=await p();w(P),_("Mensaje publicado")}catch(P){_(P.message,"error")}})}function S(){e.querySelectorAll(".reply-form").forEach(r=>{const d=parseInt(r.dataset.parent),E=r.querySelector(".reply-input"),x=`mentionDropdown-${d}`,I=document.getElementById(x);E==null||E.addEventListener("input",()=>{k(E,I)}),r.addEventListener("submit",async j=>{j.preventDefault();const P=E.value.trim();if(P)try{await c.board.reply(d,P),E.value="",I==null||I.classList.add("hidden");const N=await p();w(N),_("Respuesta enviada")}catch(N){_(N.message,"error")}})}),e.querySelectorAll(".pin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await c.board.pin(r.dataset.id);const d=await p();w(d),_("Mensaje fijado")}catch(d){_(d.message,"error")}})}),e.querySelectorAll(".unpin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await c.board.pin(r.dataset.id);const d=await p();w(d),_("Mensaje desfijado")}catch(d){_(d.message,"error")}})})}function $(){e.querySelectorAll(".delete-msg").forEach(r=>{r.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await c.board.delete(r.dataset.id);const d=await p();w(d),_("Mensaje eliminado")}catch(d){_(d.message,"error")}})})}function w(r){const{pinned:d=[],messages:E=[]}=r,x=document.getElementById("boardPinned");if(x)x.innerHTML=v(d);else if(d.length){m(r);return}const I=document.getElementById("boardMessages");I&&(I.innerHTML=b(E)),S(),$()}e.addEventListener("click",async r=>{if(r.target.id==="prevPage"&&o>1){o--;const d=await p();w(d)}else if(r.target.id==="nextPage"&&o<l){o++;const d=await p();w(d)}});function k(r,d){if(!d||!i.length)return;const E=r.value,x=r.selectionStart,I=E.slice(0,x),j=I.match(/@(\w*)$/);if(!j){d.classList.add("hidden");return}const P=j[1].toLowerCase(),N=i.filter(C=>C.username.toLowerCase().startsWith(P)&&C.id!==(t==null?void 0:t.id)),F=[...y.isAdmin()&&"todos".startsWith(P)?[{username:"todos",description:"Notificar a todos los miembros"}]:[],...N.slice(0,6)];if(!F.length){d.classList.add("hidden");return}d.innerHTML=F.map(C=>C.description?`<div class="mention-item mention-item--broadcast" data-username="${B(C.username)}">
             <span class="mention-item__name">@${B(C.username)}</span>
             <span class="mention-item__desc">${B(C.description)}</span>
           </div>`:`<div class="mention-item" data-username="${B(C.username)}">${B(C.username)}</div>`).join(""),d.classList.remove("hidden"),d.querySelectorAll(".mention-item").forEach(C=>{C.addEventListener("mousedown",le=>{le.preventDefault();const de=C.dataset.username,O=I.replace(/@(\w*)$/,`@${de} `);if(r.value=O+E.slice(x),r.setSelectionRange(O.length,O.length),d.classList.add("hidden"),r.tagName==="TEXTAREA"){const z=document.getElementById("charCounter");z&&(z.textContent=`${r.value.length} / 500`)}})})}document.addEventListener("click",r=>{!r.target.closest(".board-form__input-wrap")&&!r.target.closest(".reply-form__input-wrap")&&document.querySelectorAll(".mention-dropdown").forEach(d=>d.classList.add("hidden"))},{capture:!0})}function B(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Pe(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}async function Be(e){var a,t,n,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const i=y.getUser(),o=i==null?void 0:i.is_admin,[l,p]=await Promise.all([o?c.leagues.adminAll():c.leagues.all(),y.isLoggedIn()&&!o?c.leagues.my():Promise.resolve({leagues:[]})]),m=new Set(p.leagues.map(u=>u.id)),v=o?l.leagues:l.leagues.filter(u=>!m.has(u.id));e.innerHTML=`
      <div class="container">
        ...
        ${i&&!o&&p.leagues.length>0?`
          <section class="section">
            <h2>Mis ligas</h2>
            <div class="leagues-grid">${p.leagues.map(u=>K(u,!0)).join("")}</div>
          </section>
        `:""}
        ...
        <section class="section">
          ...
          ${v.length?`<div class="leagues-grid">${v.map(u=>K(u,!1,m,o)).join("")}</div>`:o?'<p class="empty">No hay ligas creadas aún.</p>':p.leagues.length>0?'<p class="empty">Ya participas en todas las ligas disponibles.</p>':'<p class="empty">No hay ligas aún. ¡Crea la primera!</p>'}
        </section>
      </div>
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(u=>{u.addEventListener("click",()=>L.navigate(`/ligas/${u.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(u=>{u.addEventListener("click",async b=>{b.stopPropagation();const h=parseInt(u.dataset.id);u.disabled=!0,u.textContent="…";try{const{league:f}=await c.leagues.join({league_id:h});_(`¡Te has unido a "${f.name}"!`),L.navigate(`/ligas/${f.id}`)}catch(f){_(f.message,"error"),u.disabled=!1,u.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(u=>{u.addEventListener("click",b=>{b.stopPropagation(),_("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(a=document.getElementById("btnShowCreate"))==null||a.addEventListener("click",()=>{var u,b;(u=document.getElementById("createLeaguePanel"))==null||u.classList.remove("hidden"),(b=document.getElementById("btnShowCreate"))==null||b.classList.add("hidden")}),(t=document.getElementById("btnCancelCreate"))==null||t.addEventListener("click",()=>{var u,b;(u=document.getElementById("createLeaguePanel"))==null||u.classList.add("hidden"),(b=document.getElementById("btnShowCreate"))==null||b.classList.remove("hidden")}),(n=document.getElementById("joinCodeForm"))==null||n.addEventListener("submit",async u=>{u.preventDefault();const b=document.getElementById("inviteCode").value.trim().toUpperCase();if(b)try{const{league:h}=await c.leagues.join({invite_code:b});_(`Te has unido a "${h.name}"`),L.navigate(`/ligas/${h.id}`)}catch(h){_(h.message,"error")}}),(s=document.getElementById("createLeagueForm"))==null||s.addEventListener("submit",async u=>{var k;u.preventDefault();const b=document.getElementById("createBtn");b.disabled=!0,b.textContent="Creando…";const h=document.getElementById("leagueName").value.trim(),f=document.getElementById("leagueDesc").value.trim(),S=document.getElementById("leaguePrize").value.trim(),$=document.getElementById("isPublic").checked,w=((k=document.getElementById("isOfficial"))==null?void 0:k.checked)??!1;try{const{league:r}=await c.leagues.create({name:h,description:f,prize:S,is_public:$,is_official:w});Te(r)}catch(r){_(r.message,"error"),b.disabled=!1,b.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function K(e,a=!1,t=new Set,n=!1){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=n?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:a?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),_("Enlace copiado")}catch{_("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function Me(e,{params:a}){var n,s,i,o,l;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const p=await c.leagues.get(t),{league:m,ranking:v,is_member:u,is_admin_view:b}=p,h=y.getUser(),f=m.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
    ...
    `,(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(m.invite_link),_("Enlace copiado")}catch{_("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${m.name} en PickGoal`,url:m.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await c.leagues.leave(t),_("Has abandonado la liga"),L.navigate("/ligas")}catch(r){_(r.message,"error")}}),(o=document.getElementById("btnJoin"))==null||o.addEventListener("click",async()=>{try{await c.leagues.join({league_id:t}),_("¡Te has unido a la liga!"),L.navigate(`/ligas/${t}`)}catch(r){_(r.message,"error")}}),(l=document.getElementById("btnEditLeague"))==null||l.addEventListener("click",()=>{je(m,t,h)});const S=document.getElementById("tabRanking"),$=document.getElementById("tabTablon"),w=document.getElementById("sectionRanking"),k=document.getElementById("sectionTablon");S&&$&&(S.addEventListener("click",()=>{S.classList.add("league-tab--active"),$.classList.remove("league-tab--active"),w.classList.remove("hidden"),k.classList.add("hidden")}),$.addEventListener("click",()=>{$.classList.add("league-tab--active"),S.classList.remove("league-tab--active"),w.classList.add("hidden"),k.classList.remove("hidden");const r=document.getElementById("tablonEmbed");r&&!r.dataset.loaded&&(r.dataset.loaded="1",ne(r,{query:{liga:String(t)}}))}))}catch(p){e.innerHTML=`<div class="container"><p class="form__error">Error: ${p.message}</p><a href="#/ligas">Volver</a></div>`}}function je(e,a,t){const n=document.getElementById("editLeagueModal");n&&n.remove();const s=document.createElement("div");s.id="editLeagueModal",s.className="edit-league-modal",s.innerHTML=`
  ...
  `}const Y=new Date("2026-06-11T21:00:00Z"),Ne=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];function De(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ie(e){var a;if(!y.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const t=De(),{champion_prediction:n}=await c.predictions.getChampion(t),s=new Date>=Y;let i;n&&s?i=`
    ...
    `,(a=document.getElementById("championForm"))==null||a.addEventListener("submit",async o=>{o.preventDefault();const l=document.getElementById("champBtn"),p=document.getElementById("champError"),m=document.getElementById("teamSearch").value.trim();if(m){l.disabled=!0,l.textContent="Guardando…",p.classList.add("hidden");try{await c.predictions.saveChampion(m,t),_(`¡${m} guardado como campeón!`),ie(e)}catch(v){p.textContent=v.message,p.classList.remove("hidden"),l.disabled=!1,l.textContent=l.dataset.label||"Confirmar predicción"}}})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}function U(e){const a=e?"Actualizar predicción":"Confirmar predicción";return`
  ...
  `}async function Oe(e){if(!y.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{users:a}=await c.auth.users();e.innerHTML=`
      <div class="container">
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const s=document.getElementById("syncResult");s.textContent="Sincronizando…";try{await c.matches.sync(),s.textContent="✓ Sincronización completada",_("Sincronización completada")}catch(i){s.textContent=`Error: ${i.message}`,_(i.message,"error")}}),document.getElementById("btnRecalculate").addEventListener("click",async()=>{const s=document.getElementById("syncResult");s.textContent="Recalculando puntos…";try{const{message:i}=await c.matches.recalculate();s.textContent=`✓ ${i}`,_(i)}catch(i){s.textContent=`Error: ${i.message}`,_(i.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async s=>{s.preventDefault();const i=document.getElementById("winnerTeam").value.trim();if(i)try{const{message:o}=await c.predictions.awardChampion(i);_(o)}catch(o){_(o.message,"error")}});const t=document.getElementById("pushTarget"),n=document.getElementById("pushTargetIdGroup");t.addEventListener("change",()=>{n.classList.toggle("hidden",t.value==="all")}),document.getElementById("pushForm").addEventListener("submit",async s=>{s.preventDefault();const i=document.getElementById("pushTitle").value.trim()||"Aviso",o=document.getElementById("pushBody").value.trim(),l=t.value,p=parseInt(document.getElementById("pushTargetId").value)||null,m=document.getElementById("pushResult"),v={title:`📣 PickGoal — ${i}`,body:o};l==="league"&&p&&(v.league_id=p),l==="user"&&p&&(v.user_id=p),m.textContent="Enviando…";try{const{sent:u}=await c.notifications.send(v);m.textContent=`✓ Enviada a ${u} suscripción(es)`,_(`Notificación enviada a ${u} suscripción(es)`)}catch(u){m.textContent=`Error: ${u.message}`,_(u.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async s=>{const i=s.target.closest(".toggle-admin");if(!i)return;const o=parseInt(i.dataset.id);try{const{user:l}=await c.auth.toggleAdmin(o);i.closest("tr").querySelector(".admin-badge").textContent=l.is_admin?"Sí":"No",_(`${l.username} ${l.is_admin?"ahora es admin":"ya no es admin"}`)}catch(l){_(l.message,"error")}})}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}function Re(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
  ...
  `,document.getElementById("forgotForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("forgotBtn"),n=document.getElementById("forgotMsg"),s=document.getElementById("email").value.trim();t.disabled=!0,t.textContent="Enviando…";try{await c.auth.forgotPassword(s),n.textContent="Si el email existe, recibirás un enlace en breve.",n.classList.remove("hidden","form__error"),n.classList.add("form__success")}catch{_("Error al enviar el email","error")}finally{t.disabled=!1,t.textContent="Enviar enlace"}})}function Ge(e,{query:a}){const t=a.token||"";if(!t){e.innerHTML='<div class="container"><p class="form__error">Token inválido o expirado.</p><a href="#/login">Volver</a></div>';return}e.innerHTML=`
    <div class="auth-container container">
      <div class="auth-card">
  ...
  `,document.getElementById("resetForm").addEventListener("submit",async n=>{n.preventDefault();const s=document.getElementById("resetBtn"),i=document.getElementById("resetError"),o=document.getElementById("password").value;s.disabled=!0,s.textContent="Guardando…",i.classList.add("hidden");try{await c.auth.resetPassword(t,o),_("Contraseña actualizada. Ya puedes iniciar sesión."),L.navigate("/login")}catch(l){i.textContent=l.message||"Error al restablecer la contraseña",i.classList.remove("hidden")}finally{s.disabled=!1,s.textContent="Guardar contraseña"}})}const Fe={r32:"Dieciseisavos",r16:"Octavos",quarters:"Cuartos",semis:"Semis",third:"3º y 4º",final:"Final"};async function ze(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{groups:a}=await c.matches.grouped();e.innerHTML=`
      <div class="container">
  ...
  `}async function Ke(e,{query:a}){const t=(a.codigo||"").trim().toUpperCase();if(!t){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!y.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",t),L.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:n}=await c.leagues.joinByCode(t);_(`¡Te has unido a "${n.name}"!`),L.navigate(`/ligas/${n.id}`)}catch(n){if(n.status===409){_("Ya eres miembro de esta liga");try{const{leagues:s}=await c.leagues.my(),i=s.find(o=>o.invite_code===t);if(i){L.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
    ...
    `}}function Ye(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Qe(e,{params:a}){const t=parseInt(a.id);if(!t){e.innerHTML='<div class="container"><p class="form__error">Usuario no válido.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const n=Ye(),{user:s,predictions:i}=await c.predictions.forUser(t,n);e.innerHTML=`
      <div class="container">
