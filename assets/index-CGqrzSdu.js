(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const J="https://pickgoal-backend.onrender.com/api";function V(){return localStorage.getItem("token")}async function u(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=V();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${J}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const p={get:e=>u(e),post:(e,a)=>u(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>u(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>u(e,{method:"DELETE"}),auth:{register:e=>u("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>u("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>u("/auth/me"),forgotPassword:e=>u("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>u("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>u(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>u("/auth/users"),toggleAdmin:e=>u(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>u("/matches/grouped"),list:(e="")=>u(`/matches/${e}`),get:e=>u(`/matches/${e}`),sync:()=>u("/matches/sync",{method:"POST"})},predictions:{mine:e=>u(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>u(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>u("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>u(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>u(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>u("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>u("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>u("/leagues/all"),public:()=>u("/leagues/public"),my:()=>u("/leagues/my"),create:e=>u("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>u("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>u(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>u("/leagues/admin"),get:e=>u(`/leagues/${e}`),update:(e,a)=>u(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(a)}),leave:e=>u(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>u(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>u("/home/summary")},board:{messages:(e=1)=>u(`/board/?page=${e}`),post:e=>u("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>u(`/board/${e}`,{method:"DELETE"})}};let E=null;const b={async init(){if(localStorage.getItem("token"))try{const{user:a}=await p.auth.me();E=a}catch{localStorage.removeItem("token")}},setUser(e,a){E=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){E=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return E},isLoggedIn(){return!!E},isAdmin(){return(E==null?void 0:E.is_admin)===!0}};let B=null;function g(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,B&&clearTimeout(B),B=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function A(){return`
  ...
  `}function I(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}async function X(e){if(!b.getUser()){Y(e);return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{leagues_summary:t,upcoming_matches:n}=await p.home.summary();if(t.length===0){K(e);return}const s=(()=>{const o=localStorage.getItem("activeLeagueId");return o?parseInt(o):null})(),i=[...t].sort((o,r)=>o.league_id===s?-1:r.league_id===s?1:0);e.innerHTML=`
  ...
  `}function Q(e){return`${e}º`}function W(e){const a=e.next_to_predict?`<div class="league-card__next">
         ...
         <span class="league-card__next-date">${I(e.next_to_predict.match_datetime)}</span>
        ...
        ${e.map(({match:a,has_prediction:t})=>`
          <div class="upcoming-match">
            ...
            <div class="upcoming-match__meta">
              <span class="upcoming-match__date">${I(a.match_datetime)}</span>
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await p.auth.register(s);b.setUser(o,i),g("¡Cuenta creada! Bienvenido a PickGoal");const r=sessionStorage.getItem("pendingInviteCode");if(r){sessionStorage.removeItem("pendingInviteCode");try{const{league:l}=await p.leagues.joinByCode(r);g(`¡Te has unido a "${l.name}"!`),y.navigate(`/ligas/${l.id}`)}catch{y.navigate("/ligas")}}else y.navigate("/campeon"),D("/ligas")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function ne(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ie(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(b.isLoggedIn()){const{leagues:_}=await p.leagues.my();if(_.length===0){e.innerHTML=A();return}}const a=ne(),[{groups:t},n]=await Promise.all([p.matches.grouped(),b.isLoggedIn()?p.predictions.mine(a):Promise.resolve({predictions:[]})]),s={};for(const _ of n.predictions)s[_.match_id]=_;const i=t.flatMap(_=>_.matches),o=new Map;for(const _ of i){const c=M(_.match_datetime);o.has(c)||o.set(c,[]),o.get(c).push(_)}const r=[...o.keys()].sort(),l=M(new Date().toISOString()),d=r.find(_=>_>=l)??r[0];e.innerHTML=`
      <div class="container">
  ...
  `}function ce(e,a,t){const n=parseInt(e.dataset.matchId),s=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function r(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function l(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado",s.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(d=>{d.addEventListener("change",r),d.addEventListener("input",r)}),e.addEventListener("submit",async d=>{var m;d.preventDefault();const _=parseInt(e.querySelector("[name=predicted_home]").value),c=parseInt(e.querySelector("[name=predicted_away]").value),v=(m=e.querySelector("[name=predicted_result]:checked"))==null?void 0:m.value;if(!(isNaN(_)||isNaN(c)||!v)){s.disabled=!0,s.textContent="…";try{const{prediction:h}=await p.predictions.save({match_id:n,predicted_result:v,predicted_home:_,predicted_away:c,league_id:t??null});a[n]=h,g("Predicción guardada"),l()}catch(h){g(h.message||"Error al guardar","error"),s.disabled=!1,o?(s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado"):(s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar")}}})}function pe(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function me(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(b.isLoggedIn()){const{leagues:r}=await p.leagues.my();if(r.length===0){e.innerHTML=A();return}}const a=pe(),[{ranking:t},n]=await Promise.all([p.auth.ranking(a),b.isLoggedIn()?p.leagues.my():Promise.resolve({leagues:[]})]),s=b.getUser(),i=n.leagues.find(r=>r.id===a),o=i?i.name:"Clasificación General";e.innerHTML=`
      <div class="container">
    ...
    `}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}async function ue(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let a=1,t=1;async function n(){const{messages:r,pages:l}=await p.board.messages(a);return t=l,r}try{const r=await n();s(r)}catch(r){e.innerHTML=`<div class="container"><p class="form__error">Error: ${r.message}</p></div>`}function s(r){var d,_,c;const l=b.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(d=document.getElementById("boardForm"))==null||d.addEventListener("submit",async v=>{v.preventDefault();const m=document.getElementById("boardMsg"),h=m.value.trim();if(h)try{await p.board.post(h),m.value="";const f=await n();document.getElementById("boardMessages").innerHTML=i(f,l),o(l),g("Mensaje publicado")}catch(f){g(f.message,"error")}}),(_=document.getElementById("prevPage"))==null||_.addEventListener("click",async()=>{a--;const v=await n();document.getElementById("boardMessages").innerHTML=i(v,l),o(l)}),(c=document.getElementById("nextPage"))==null||c.addEventListener("click",async()=>{a++;const v=await n();document.getElementById("boardMessages").innerHTML=i(v,l),o(l)}),o(l)}function i(r,l){return r.length?r.map(d=>`
      <div class="board-message ${d.is_deleted?"board-message--deleted":""}" data-id="${d.id}">
        <div class="board-message__header">
          <strong>${d.username}</strong>
          <span class="board-message__date">${I(d.created_at)}</span>
    ...
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function o(r){e.querySelectorAll(".delete-msg").forEach(l=>{l.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await p.board.delete(l.dataset.id);const d=await n();document.getElementById("boardMessages").innerHTML=i(d,r),o(r),g("Mensaje eliminado")}catch(d){g(d.message,"error")}})})}}function ge(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function _e(e){var a,t,n,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const i=b.getUser(),o=i==null?void 0:i.is_admin,[r,l]=await Promise.all([o?p.leagues.adminAll():p.leagues.all(),b.isLoggedIn()&&!o?p.leagues.my():Promise.resolve({leagues:[]})]),d=new Set(l.leagues.map(c=>c.id)),_=o?r.leagues:r.leagues.filter(c=>!d.has(c.id));e.innerHTML=`
      <div class="container">
        ...
        ${i&&!o&&l.leagues.length>0?`
          <section class="section">
            <h2>Mis ligas</h2>
            <div class="leagues-grid">${l.leagues.map(c=>T(c,!0)).join("")}</div>
          </section>
        `:""}
        ...
        <section class="section">
          ...
          ${_.length?`<div class="leagues-grid">${_.map(c=>T(c,!1,d,o)).join("")}</div>`:o?'<p class="empty">No hay ligas creadas aún.</p>':l.leagues.length>0?'<p class="empty">Ya participas en todas las ligas disponibles.</p>':'<p class="empty">No hay ligas aún. ¡Crea la primera!</p>'}
        </section>
      </div>
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(c=>{c.addEventListener("click",()=>y.navigate(`/ligas/${c.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(c=>{c.addEventListener("click",async v=>{v.stopPropagation();const m=parseInt(c.dataset.id);c.disabled=!0,c.textContent="…";try{const{league:h}=await p.leagues.join({league_id:m});g(`¡Te has unido a "${h.name}"!`),y.navigate(`/ligas/${h.id}`)}catch(h){g(h.message,"error"),c.disabled=!1,c.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(c=>{c.addEventListener("click",v=>{v.stopPropagation(),g("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(a=document.getElementById("btnShowCreate"))==null||a.addEventListener("click",()=>{var c,v;(c=document.getElementById("createLeaguePanel"))==null||c.classList.remove("hidden"),(v=document.getElementById("btnShowCreate"))==null||v.classList.add("hidden")}),(t=document.getElementById("btnCancelCreate"))==null||t.addEventListener("click",()=>{var c,v;(c=document.getElementById("createLeaguePanel"))==null||c.classList.add("hidden"),(v=document.getElementById("btnShowCreate"))==null||v.classList.remove("hidden")}),(n=document.getElementById("joinCodeForm"))==null||n.addEventListener("submit",async c=>{c.preventDefault();const v=document.getElementById("inviteCode").value.trim().toUpperCase();if(v)try{const{league:m}=await p.leagues.join({invite_code:v});g(`Te has unido a "${m.name}"`),y.navigate(`/ligas/${m.id}`)}catch(m){g(m.message,"error")}}),(s=document.getElementById("createLeagueForm"))==null||s.addEventListener("submit",async c=>{var x;c.preventDefault();const v=document.getElementById("createBtn");v.disabled=!0,v.textContent="Creando…";const m=document.getElementById("leagueName").value.trim(),h=document.getElementById("leagueDesc").value.trim(),f=document.getElementById("leaguePrize").value.trim(),F=document.getElementById("isPublic").checked,R=((x=document.getElementById("isOfficial"))==null?void 0:x.checked)??!1;try{const{league:S}=await p.leagues.create({name:m,description:h,prize:f,is_public:F,is_official:R});ve(S)}catch(S){g(S.message,"error"),v.disabled=!1,v.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function T(e,a=!1,t=new Set,n=!1){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=n?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:a?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),g("Enlace copiado")}catch{g("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function he(e,{params:a}){var n,s,i,o,r;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const l=await p.leagues.get(t);console.log("[liga-detalle] API response for league",t,":",JSON.stringify(l));const{league:d,ranking:_,is_member:c,is_admin_view:v}=l,m=b.getUser();console.log("[liga-detalle] user:",JSON.stringify(m));const h=d.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
        ...
        <div class="league-header">
          <h1 class="page-title">${d.name} ${h}</h1>
          ${d.description?`<p class="league-header__desc">${d.description}</p>`:""}
          <div class="league-header__meta">
            <span>${d.is_public?"🌍 Pública":"🔒 Privada"}</span>
            <span>${d.member_count} participantes</span>
            ${d.prize?`<span>🏆 ${d.prize}</span>`:""}
          </div>
        </div>

        ${(c||m!=null&&m.is_admin)&&d.invite_link?`
          <div class="invite-share-box">
            <span class="invite-share-box__label">Enlace de invitación:</span>
            <div class="invite-link-box">
              <span class="invite-link-box__url">${d.invite_link}</span>
        ...
        <div class="league-actions">
          ${c?'<button class="btn btn--danger btn--sm" id="btnLeave">Abandonar liga</button>':!(m!=null&&m.is_admin)&&m?'<button class="btn btn--primary" id="btnJoin">Unirse a esta liga</button>':""}
          ${(()=>{const f=(m==null?void 0:m.is_admin)||c&&m&&d.created_by===m.id;return console.log("[liga-detalle] canEdit:",f,"| user.is_admin:",m==null?void 0:m.is_admin,"| is_member:",c,"| created_by:",d.created_by,"| user.id:",m==null?void 0:m.id),f?'<button class="btn btn--outline btn--sm" id="btnEditLeague">Editar liga</button>':""})()}
        </div>
        ...
        <section class="section">
          ...
          <table class="ranking-table">
            ...
            <tbody>
              ${_.map(f=>`
                <tr class="${m&&f.id===m.id?"ranking-table__row--me":""}">
                  <td>${f.position}</td>
                  <td>${f.username}</td>
                  <td>${f.country||"—"}</td>
    ...
    `,console.log("[liga-detalle] post-render — btnEditLeague in DOM:",document.getElementById("btnEditLeague")),(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(d.invite_link),g("Enlace copiado")}catch{g("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${d.name} en PickGoal`,url:d.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await p.leagues.leave(t),g("Has abandonado la liga"),y.navigate("/ligas")}catch(f){g(f.message,"error")}}),(o=document.getElementById("btnJoin"))==null||o.addEventListener("click",async()=>{try{await p.leagues.join({league_id:t}),g("¡Te has unido a la liga!"),y.navigate(`/ligas/${t}`)}catch(f){g(f.message,"error")}}),(r=document.getElementById("btnEditLeague"))==null||r.addEventListener("click",()=>{be(d,t,m)})}catch(l){e.innerHTML=`<div class="container"><p class="form__error">Error: ${l.message}</p><a href="#/ligas">Volver</a></div>`}}function be(e,a,t){const n=document.getElementById("editLeagueModal");n&&n.remove();const s=document.createElement("div");s.id="editLeagueModal",s.className="edit-league-modal",s.innerHTML=`
  ...
  `,document.body.appendChild(s),requestAnimationFrame(()=>s.classList.add("edit-league-modal--open"));const i=()=>{s.classList.remove("edit-league-modal--open"),s.addEventListener("transitionend",()=>s.remove(),{once:!0})};s.querySelector(".edit-league-modal__overlay").addEventListener("click",i),document.getElementById("btnCancelEdit").addEventListener("click",i),document.getElementById("editLeagueForm").addEventListener("submit",async o=>{o.preventDefault();const r=document.getElementById("btnSaveEdit");r.disabled=!0,r.textContent="Guardando…";const l={name:document.getElementById("editName").value.trim(),description:document.getElementById("editDesc").value.trim(),prize:document.getElementById("editPrize").value.trim(),is_public:document.getElementById("editPublic").checked};t!=null&&t.is_admin&&(l.is_official=document.getElementById("editOfficial").checked);try{await p.leagues.update(a,l),g("Liga actualizada"),i(),y.navigate(`/ligas/${a}`)}catch(d){g(d.message,"error"),r.disabled=!1,r.textContent="Guardar cambios"}})}async function fe(e){var t,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=b.getUser();try{const s=(()=>{const h=localStorage.getItem("activeLeagueId");return h?parseInt(h):null})(),[i,o,r,l,d]=await Promise.all([p.predictions.mine(s),p.predictions.getChampion(s),p.leagues.my(),p.auth.me(),a!=null&&a.is_admin?p.leagues.adminAll():Promise.resolve({leagues:[]})]),_=i.predictions.reduce((h,f)=>h+f.total_points,0)+(((t=o.champion_prediction)==null?void 0:t.points_earned)||0),c=l.user,v=c.status,m=c.total_points_all_time;e.innerHTML=`
      <div class="container">
        ...
        ${a!=null&&a.is_admin&&d.leagues.length?`
          <section class="section">
  ...
  `}const j=new Date("2026-06-11T21:00:00Z"),Ee=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];function Le(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function O(e){var a;if(!b.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const t=Le(),{champion_prediction:n}=await p.predictions.getChampion(t),s=new Date>=j;let i;n&&s?i=`
  ...
  `}async function we(e){if(!b.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{users:a}=await p.auth.users();e.innerHTML=`
      <div class="container">
  ...
  `,document.getElementById("resetForm").addEventListener("submit",async n=>{n.preventDefault();const s=document.getElementById("resetBtn"),i=document.getElementById("resetError"),o=document.getElementById("password").value;s.disabled=!0,s.textContent="Guardando…",i.classList.add("hidden");try{await p.auth.resetPassword(t,o),g("Contraseña actualizada. Ya puedes iniciar sesión."),y.navigate("/login")}catch(r){i.textContent=r.message||"Error al restablecer la contraseña",i.classList.remove("hidden")}finally{s.disabled=!1,s.textContent="Guardar contraseña"}})}const Ce={r32:"Dieciseisavos",r16:"Octavos",quarters:"Cuartos",semis:"Semis",third:"3º y 4º",final:"Final"};async function xe(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{groups:a}=await p.matches.grouped();e.innerHTML=`
      <div class="container">
  ...
  `}async function je(e,{query:a}){const t=(a.codigo||"").trim().toUpperCase();if(!t){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!b.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",t),y.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:n}=await p.leagues.joinByCode(t);g(`¡Te has unido a "${n.name}"!`),y.navigate(`/ligas/${n.id}`)}catch(n){if(n.status===409){g("Ya eres miembro de esta liga");try{const{leagues:s}=await p.leagues.my(),i=s.find(o=>o.invite_code===t);if(i){y.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
    ...
    `}}function He(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function qe(e,{params:a}){const t=parseInt(a.id);if(!t){e.innerHTML='<div class="container"><p class="form__error">Usuario no válido.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const n=He(),{user:s,predictions:i}=await p.predictions.forUser(t,n);e.innerHTML=`
      <div class="container">
  ...
  `,document.body.appendChild(e),document.getElementById("installBtn").addEventListener("click",async()=>{w&&(w.prompt(),await w.userChoice,w=null,e.remove())}),document.getElementById("installDismissBtn").addEventListener("click",()=>{sessionStorage.setItem("installBannerDismissed","1"),e.remove()})}function $(){var e,a,t,n;(e=document.getElementById("leagueDropdown"))==null||e.classList.add("hidden"),(a=document.getElementById("leagueBtn"))==null||a.classList.remove("navbar__dropdown-btn--open"),(t=document.getElementById("userDropdown"))==null||t.classList.add("hidden"),(n=document.getElementById("userBtn"))==null||n.classList.remove("navbar__dropdown-btn--open")}function ze(){var e,a,t,n,s;document.addEventListener("auth:change",N),window.addEventListener("hashchange",()=>{$(),z()}),document.addEventListener("click",$),(e=document.getElementById("leagueBtn"))==null||e.addEventListener("click",i=>{var l;i.stopPropagation();const o=document.getElementById("leagueDropdown"),r=o==null?void 0:o.classList.contains("hidden");$(),r&&(o==null||o.classList.remove("hidden"),(l=document.getElementById("leagueBtn"))==null||l.classList.add("navbar__dropdown-btn--open"))}),(a=document.getElementById("leagueDropdown"))==null||a.addEventListener("click",i=>{var r;i.stopPropagation();const o=i.target.closest("[data-league-id]");if(o){const l=String(o.dataset.leagueId);localStorage.setItem("activeLeagueId",l),$(),U(L),(r=b.getUser())!=null&&r.is_admin?y.navigate(`/ligas/${l}`):y.resolve();return}i.target.closest("a")&&$()}),(t=document.getElementById("userBtn"))==null||t.addEventListener("click",i=>{var l;i.stopPropagation();const o=document.getElementById("userDropdown"),r=o==null?void 0:o.classList.contains("hidden");$(),r&&(o==null||o.classList.remove("hidden"),(l=document.getElementById("userBtn"))==null||l.classList.add("navbar__dropdown-btn--open"))}),(n=document.getElementById("userDropdown"))==null||n.addEventListener("click",i=>{i.stopPropagation(),i.target.closest("#navProfileLink")&&$()}),(s=document.getElementById("navLogoutBtn"))==null||s.addEventListener("click",()=>{L=[],localStorage.removeItem("activeLeagueId"),$(),b.logout(),y.navigate("/")}),N()}async function N(){const e=document.getElementById("navAuthLinks"),a=document.getElementById("userBtn"),t=document.getElementById("navUsername"),n=document.getElementById("navLeague"),s=document.getElementById("bottomNav"),i=b.getUser();if($(),i){e==null||e.classList.add("hidden"),t&&(t.textContent=i.username),n.style.visibility="visible",a.style.visibility="visible",s==null||s.classList.remove("hidden"),document.body.classList.add("has-bottom-nav");try{const{leagues:o}=i.is_admin?await p.leagues.adminAll():await p.leagues.my();L=o}catch{L=[]}U(L)}else e==null||e.classList.remove("hidden"),n.style.visibility="hidden",a.style.visibility="hidden",s==null||s.classList.add("hidden"),document.body.classList.remove("has-bottom-nav"),L=[],localStorage.removeItem("activeLeagueId");z()}function U(e){const a=document.getElementById("leagueDropdown"),t=document.getElementById("navLeagueName");if(!a||!t)return;let n=localStorage.getItem("activeLeagueId"),s=e.find(o=>String(o.id)===String(n));!s&&e.length>0&&(s=e[0],localStorage.setItem("activeLeagueId",String(s.id))),s||localStorage.removeItem("activeLeagueId"),t.textContent=s?s.name:"Inicia Liga";const i=e.map(o=>`
