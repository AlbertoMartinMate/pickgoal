(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const R="https://pickgoal-backend.onrender.com/api";function V(){return localStorage.getItem("token")}async function u(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=V();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${R}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const p={get:e=>u(e),post:(e,a)=>u(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>u(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>u(e,{method:"DELETE"}),auth:{register:e=>u("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>u("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>u("/auth/me"),forgotPassword:e=>u("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>u("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>u(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>u("/auth/users"),toggleAdmin:e=>u(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>u("/matches/grouped"),list:(e="")=>u(`/matches/${e}`),get:e=>u(`/matches/${e}`),sync:()=>u("/matches/sync",{method:"POST"})},predictions:{mine:e=>u(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>u(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>u("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>u(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>u(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>u("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>u("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>u("/leagues/all"),public:()=>u("/leagues/public"),my:()=>u("/leagues/my"),create:e=>u("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>u("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>u(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>u("/leagues/admin"),get:e=>u(`/leagues/${e}`),update:(e,a)=>u(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(a)}),leave:e=>u(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>u(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>u("/home/summary")},board:{messages:(e=1)=>u(`/board/?page=${e}`),post:e=>u("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>u(`/board/${e}`,{method:"DELETE"})}};let $=null;const b={async init(){if(localStorage.getItem("token"))try{const{user:a}=await p.auth.me();$=a}catch{localStorage.removeItem("token")}},setUser(e,a){$=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){$=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return $},isLoggedIn(){return!!$},isAdmin(){return($==null?void 0:$.is_admin)===!0}};let B=null;function g(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,B&&clearTimeout(B),B=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function A(){return`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await p.auth.register(s);b.setUser(o,i),g("¡Cuenta creada! Bienvenido a PickGoal");const r=sessionStorage.getItem("pendingInviteCode");if(r){sessionStorage.removeItem("pendingInviteCode");try{const{league:l}=await p.leagues.joinByCode(r);g(`¡Te has unido a "${l.name}"!`),f.navigate(`/ligas/${l.id}`)}catch{f.navigate("/ligas")}}else f.navigate("/campeon"),D("/ligas")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function se(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ne(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(b.isLoggedIn()){const{leagues:_}=await p.leagues.my();if(_.length===0){e.innerHTML=A();return}}const a=se(),[{groups:t},n]=await Promise.all([p.matches.grouped(),b.isLoggedIn()?p.predictions.mine(a):Promise.resolve({predictions:[]})]),s={};for(const _ of n.predictions)s[_.match_id]=_;const i=t.flatMap(_=>_.matches),o=new Map;for(const _ of i){const d=M(_.match_datetime);o.has(d)||o.set(d,[]),o.get(d).push(_)}const r=[...o.keys()].sort(),l=M(new Date().toISOString()),c=r.find(_=>_>=l)??r[0];e.innerHTML=`
    ...
    `,ie(r,c,o,s,a)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${a.message}</p></div>`}}function ie(e,a,t,n,s){var o;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(r=>`
  ...
  `}function de(e,a,t){const n=parseInt(e.dataset.matchId),s=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function r(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function l(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado",s.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(c=>{c.addEventListener("change",r),c.addEventListener("input",r)}),e.addEventListener("submit",async c=>{var h;c.preventDefault();const _=parseInt(e.querySelector("[name=predicted_home]").value),d=parseInt(e.querySelector("[name=predicted_away]").value),m=(h=e.querySelector("[name=predicted_result]:checked"))==null?void 0:h.value;if(!(isNaN(_)||isNaN(d)||!m)){s.disabled=!0,s.textContent="…";try{const{prediction:v}=await p.predictions.save({match_id:n,predicted_result:m,predicted_home:_,predicted_away:d,league_id:t??null});a[n]=v,g("Predicción guardada"),l()}catch(v){g(v.message||"Error al guardar","error"),s.disabled=!1,o?(s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado"):(s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar")}}})}function ce(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function pe(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(b.isLoggedIn()){const{leagues:r}=await p.leagues.my();if(r.length===0){e.innerHTML=A();return}}const a=ce(),[{ranking:t},n]=await Promise.all([p.auth.ranking(a),b.isLoggedIn()?p.leagues.my():Promise.resolve({leagues:[]})]),s=b.getUser(),i=n.leagues.find(r=>r.id===a),o=i?i.name:"Clasificación General";e.innerHTML=`
    ...
    `,(c=document.getElementById("boardForm"))==null||c.addEventListener("submit",async m=>{m.preventDefault();const h=document.getElementById("boardMsg"),v=h.value.trim();if(v)try{await p.board.post(v),h.value="";const I=await n();document.getElementById("boardMessages").innerHTML=i(I,l),o(l),g("Mensaje publicado")}catch(I){g(I.message,"error")}}),(_=document.getElementById("prevPage"))==null||_.addEventListener("click",async()=>{a--;const m=await n();document.getElementById("boardMessages").innerHTML=i(m,l),o(l)}),(d=document.getElementById("nextPage"))==null||d.addEventListener("click",async()=>{a++;const m=await n();document.getElementById("boardMessages").innerHTML=i(m,l),o(l)}),o(l)}function i(r,l){return r.length?r.map(c=>`
    ...
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(d=>{d.addEventListener("click",()=>f.navigate(`/ligas/${d.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(d=>{d.addEventListener("click",async m=>{m.stopPropagation();const h=parseInt(d.dataset.id);d.disabled=!0,d.textContent="…";try{const{league:v}=await p.leagues.join({league_id:h});g(`¡Te has unido a "${v.name}"!`),f.navigate(`/ligas/${v.id}`)}catch(v){g(v.message,"error"),d.disabled=!1,d.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(d=>{d.addEventListener("click",m=>{m.stopPropagation(),g("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(a=document.getElementById("btnShowCreate"))==null||a.addEventListener("click",()=>{var d,m;(d=document.getElementById("createLeaguePanel"))==null||d.classList.remove("hidden"),(m=document.getElementById("btnShowCreate"))==null||m.classList.add("hidden")}),(t=document.getElementById("btnCancelCreate"))==null||t.addEventListener("click",()=>{var d,m;(d=document.getElementById("createLeaguePanel"))==null||d.classList.add("hidden"),(m=document.getElementById("btnShowCreate"))==null||m.classList.remove("hidden")}),(n=document.getElementById("joinCodeForm"))==null||n.addEventListener("submit",async d=>{d.preventDefault();const m=document.getElementById("inviteCode").value.trim().toUpperCase();if(m)try{const{league:h}=await p.leagues.join({invite_code:m});g(`Te has unido a "${h.name}"`),f.navigate(`/ligas/${h.id}`)}catch(h){g(h.message,"error")}}),(s=document.getElementById("createLeagueForm"))==null||s.addEventListener("submit",async d=>{var x;d.preventDefault();const m=document.getElementById("createBtn");m.disabled=!0,m.textContent="Creando…";const h=document.getElementById("leagueName").value.trim(),v=document.getElementById("leagueDesc").value.trim(),I=document.getElementById("leaguePrize").value.trim(),z=document.getElementById("isPublic").checked,F=((x=document.getElementById("isOfficial"))==null?void 0:x.checked)??!1;try{const{league:S}=await p.leagues.create({name:h,description:v,prize:I,is_public:z,is_official:F});_e(S)}catch(S){g(S.message,"error"),m.disabled=!1,m.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function T(e,a=!1,t=new Set,n=!1){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=n?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:a?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),g("Enlace copiado")}catch{g("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function ve(e,{params:a}){var n,s,i,o,r;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:l,ranking:c,is_member:_,is_admin_view:d}=await p.leagues.get(t),m=b.getUser(),h=l.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
        ...
        <div class="league-actions">
          ${_?'<button class="btn btn--danger btn--sm" id="btnLeave">Abandonar liga</button>':!d&&m?'<button class="btn btn--primary" id="btnJoin">Unirse a esta liga</button>':""}
          ${m!=null&&m.is_admin||_&&m&&l.created_by===m.id?'<button class="btn btn--outline btn--sm" id="btnEditLeague">Editar liga</button>':""}
        </div>
        ...
        <section class="section">
          ...
          <table class="ranking-table">
            ...
            <tbody>
              ${c.map(v=>`
                <tr class="${m&&v.id===m.id?"ranking-table__row--me":""}">
                  <td>${v.position}</td>
                  <td>${v.username}</td>
                  <td>${v.country||"—"}</td>
    ...
    `,(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(l.invite_link),g("Enlace copiado")}catch{g("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${l.name} en PickGoal`,url:l.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await p.leagues.leave(t),g("Has abandonado la liga"),f.navigate("/ligas")}catch(v){g(v.message,"error")}}),(o=document.getElementById("btnJoin"))==null||o.addEventListener("click",async()=>{try{await p.leagues.join({league_id:t}),g("¡Te has unido a la liga!"),f.navigate(`/ligas/${t}`)}catch(v){g(v.message,"error")}}),(r=document.getElementById("btnEditLeague"))==null||r.addEventListener("click",()=>{he(l,t,m)})}catch(l){e.innerHTML=`<div class="container"><p class="form__error">Error: ${l.message}</p><a href="#/ligas">Volver</a></div>`}}function he(e,a,t){const n=document.getElementById("editLeagueModal");n&&n.remove();const s=document.createElement("div");s.id="editLeagueModal",s.className="edit-league-modal",s.innerHTML=`
