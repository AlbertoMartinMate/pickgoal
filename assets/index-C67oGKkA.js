(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function a(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=a(s);fetch(s.href,i)}})();const de="https://pickgoal-backend.onrender.com/api";function le(){return localStorage.getItem("token")}async function g(e,t={}){const a={"Content-Type":"application/json",...t.headers},n=le();n&&(a.Authorization=`Bearer ${n}`);const s=await fetch(`${de}${e}`,{...t,headers:a}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const u={get:e=>g(e),post:(e,t)=>g(e,{method:"POST",body:JSON.stringify(t)}),patch:(e,t)=>g(e,{method:"PATCH",body:JSON.stringify(t)}),delete:e=>g(e,{method:"DELETE"}),auth:{register:e=>g("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>g("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>g("/auth/me"),forgotPassword:e=>g("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,t)=>g("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:t})}),ranking:e=>g(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>g("/auth/users"),toggleAdmin:e=>g(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>g("/matches/grouped"),list:(e="")=>g(`/matches/${e}`),get:e=>g(`/matches/${e}`),sync:()=>g("/matches/sync",{method:"POST"})},predictions:{mine:e=>g(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,t)=>g(`/predictions/match/${e}${t?`?league_id=${t}`:""}`),save:e=>g("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,t)=>g(`/predictions/user/${e}${t?`?league_id=${t}`:""}`),getChampion:e=>g(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,t)=>g("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:t??null})}),awardChampion:e=>g("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>g("/leagues/all"),public:()=>g("/leagues/public"),my:()=>g("/leagues/my"),create:e=>g("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>g("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>g(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>g("/leagues/admin"),get:e=>g(`/leagues/${e}`),update:(e,t)=>g(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(t)}),leave:e=>g(`/leagues/${e}/leave`,{method:"DELETE"}),members:e=>g(`/leagues/${e}/members`),matchPredictions:(e,t)=>g(`/leagues/${e}/predictions/${t}`)},home:{summary:()=>g("/home/summary")},board:{messages:(e=1,t=null)=>g(`/board/?page=${e}${t?`&league_id=${t}`:""}`),unread:(e,t)=>g(`/board/unread?league_id=${e}&since=${encodeURIComponent(t)}`),post:(e,t=null)=>g("/board/",{method:"POST",body:JSON.stringify({message:e,league_id:t})}),pin:e=>g(`/board/${e}/pin`,{method:"POST"}),reply:(e,t)=>g(`/board/${e}/reply`,{method:"POST",body:JSON.stringify({message:t})}),delete:e=>g(`/board/${e}`,{method:"DELETE"})}};let T=null;const y={async init(){if(localStorage.getItem("token"))try{const{user:t}=await u.auth.me();T=t}catch{localStorage.removeItem("token")}},setUser(e,t){T=e,localStorage.setItem("token",t),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){T=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return T},isLoggedIn(){return!!T},isAdmin(){return(T==null?void 0:T.is_admin)===!0}};let U=null;function _(e,t="success"){let a=document.getElementById("toast");a||(a=document.createElement("div"),a.id="toast",document.body.appendChild(a)),a.textContent=e,a.className=`toast toast--${t} toast--visible`,U&&clearTimeout(U),U=setTimeout(()=>{a.classList.remove("toast--visible")},3e3)}function Z(){return`
    <div class="container">
  ...
  `}function Se(e,t,a){const n=parseInt(e.dataset.matchId),s=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function d(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function p(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado",s.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(m=>{m.addEventListener("change",d),m.addEventListener("input",d)}),e.addEventListener("submit",async m=>{var v;m.preventDefault();const h=parseInt(e.querySelector("[name=predicted_home]").value),c=parseInt(e.querySelector("[name=predicted_away]").value),b=(v=e.querySelector("[name=predicted_result]:checked"))==null?void 0:v.value;if(!(isNaN(h)||isNaN(c)||!b)){s.disabled=!0,s.textContent="…";try{const{prediction:f}=await u.predictions.save({match_id:n,predicted_result:b,predicted_home:h,predicted_away:c,league_id:a??null});t[n]=f,_("Predicción guardada"),p()}catch(f){_(f.message||"Error al guardar","error"),s.disabled=!1,o?(s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado"):(s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar")}}})}function xe(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Be(e){var t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(y.isLoggedIn()){const{leagues:c}=await u.leagues.my();if(c.length===0){e.innerHTML=Z();return}}const a=xe(),[{ranking:n},s]=await Promise.all([u.auth.ranking(a),y.isLoggedIn()?u.leagues.my():Promise.resolve({leagues:[]})]),i=y.getUser(),o=s.leagues.find(c=>c.id===a),d=o?o.name:"Clasificación General",p=document.getElementById("tablonBadge"),m=p&&!p.classList.contains("hidden"),h=m?p.textContent:"";e.innerHTML=`
                ...
                <tr class="${i&&c.id===i.id?"ranking-table__row--me":""}">
                  ...
                  <td>
                    <a class="ranking-table__link" href="#/jugador/${c.id}">
                      <span class="status-emoji" title="${((b=c.status)==null?void 0:b.name)||""}">${((v=c.status)==null?void 0:v.emoji)||""}</span>${c.username}
                    </a>
                  </td>
                  <td>${c.country||"—"}</td>
    ...
    `,f(),S(),x()}function h(r){return r.length?r.map(l=>`
      <div class="board-message board-message--pinned" data-id="${l.id}">
        <div class="board-message__header">
          ...
          <strong class="board-message__author">${A(l.username)}</strong>
    ...
    `).join(""):""}function c(r){return`
      <div class="board-reply ${r.is_deleted?"board-reply--deleted":""}" data-id="${r.id}">
        <div class="board-reply__header">
          <strong class="board-reply__author">${A(r.username)}</strong>
    ...
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function v(r){const l=A(r);if(!i.length)return l;const $=i.map(L=>Ce(L.username)),I=new RegExp(`@(${$.join("|")})`,"gi");return l.replace(I,'<span class="mention">@$1</span>')}function f(){const r=document.getElementById("boardForm");if(!r)return;const l=document.getElementById("boardMsg"),$=document.getElementById("charCounter"),I=document.getElementById("mentionDropdown");l.addEventListener("input",()=>{$.textContent=`${l.value.length} / 500`,C(l,I)}),r.addEventListener("submit",async L=>{L.preventDefault();const j=l.value.trim();if(j)try{await u.board.post(j,n),l.value="",$.textContent="0 / 500",I.classList.add("hidden");const k=await p();w(k),_("Mensaje publicado")}catch(k){_(k.message,"error")}})}function S(){e.querySelectorAll(".reply-form").forEach(r=>{const l=parseInt(r.dataset.parent),$=r.querySelector(".reply-input"),I=`mentionDropdown-${l}`,L=document.getElementById(I);$==null||$.addEventListener("input",()=>{C($,L)}),r.addEventListener("submit",async j=>{j.preventDefault();const k=$.value.trim();if(k)try{await u.board.reply(l,k),$.value="",L==null||L.classList.add("hidden");const q=await p();w(q),_("Respuesta enviada")}catch(q){_(q.message,"error")}})}),e.querySelectorAll(".pin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await u.board.pin(r.dataset.id);const l=await p();w(l),_("Mensaje fijado")}catch(l){_(l.message,"error")}})}),e.querySelectorAll(".unpin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await u.board.pin(r.dataset.id);const l=await p();w(l),_("Mensaje desfijado")}catch(l){_(l.message,"error")}})})}function x(){e.querySelectorAll(".delete-msg").forEach(r=>{r.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await u.board.delete(r.dataset.id);const l=await p();w(l),_("Mensaje eliminado")}catch(l){_(l.message,"error")}})})}function w(r){const{pinned:l=[],messages:$=[]}=r,I=document.getElementById("boardPinned");if(I)I.innerHTML=h(l);else if(l.length){m(r);return}const L=document.getElementById("boardMessages");L&&(L.innerHTML=b($)),S(),x()}e.addEventListener("click",async r=>{if(r.target.id==="prevPage"&&o>1){o--;const l=await p();w(l)}else if(r.target.id==="nextPage"&&o<d){o++;const l=await p();w(l)}});function C(r,l){if(!l||!i.length)return;const $=r.value,I=r.selectionStart,L=$.slice(0,I),j=L.match(/@(\w*)$/);if(!j){l.classList.add("hidden");return}const k=j[1].toLowerCase(),q=i.filter(P=>P.username.toLowerCase().startsWith(k)&&P.id!==(a==null?void 0:a.id));if(!q.length){l.classList.add("hidden");return}l.innerHTML=q.slice(0,6).map(P=>`<div class="mention-item" data-username="${A(P.username)}">${A(P.username)}</div>`).join(""),l.classList.remove("hidden"),l.querySelectorAll(".mention-item").forEach(P=>{P.addEventListener("mousedown",oe=>{oe.preventDefault();const re=P.dataset.username,R=L.replace(/@(\w*)$/,`@${re} `);if(r.value=R+$.slice(I),r.setSelectionRange(R.length,R.length),l.classList.add("hidden"),r.tagName==="TEXTAREA"){const F=document.getElementById("charCounter");F&&(F.textContent=`${r.value.length} / 500`)}})})}document.addEventListener("click",r=>{!r.target.closest(".board-form__input-wrap")&&!r.target.closest(".reply-form__input-wrap")&&document.querySelectorAll(".mention-dropdown").forEach(l=>l.classList.add("hidden"))},{capture:!0})}function A(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Ce(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}async function ke(e){var t,a,n,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const i=y.getUser(),o=i==null?void 0:i.is_admin,[d,p]=await Promise.all([o?u.leagues.adminAll():u.leagues.all(),y.isLoggedIn()&&!o?u.leagues.my():Promise.resolve({leagues:[]})]),m=new Set(p.leagues.map(c=>c.id)),h=o?d.leagues:d.leagues.filter(c=>!m.has(c.id));e.innerHTML=`
      <div class="container">
    ...
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(c=>{c.addEventListener("click",()=>E.navigate(`/ligas/${c.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(c=>{c.addEventListener("click",async b=>{b.stopPropagation();const v=parseInt(c.dataset.id);c.disabled=!0,c.textContent="…";try{const{league:f}=await u.leagues.join({league_id:v});_(`¡Te has unido a "${f.name}"!`),E.navigate(`/ligas/${f.id}`)}catch(f){_(f.message,"error"),c.disabled=!1,c.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(c=>{c.addEventListener("click",b=>{b.stopPropagation(),_("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(t=document.getElementById("btnShowCreate"))==null||t.addEventListener("click",()=>{var c,b;(c=document.getElementById("createLeaguePanel"))==null||c.classList.remove("hidden"),(b=document.getElementById("btnShowCreate"))==null||b.classList.add("hidden")}),(a=document.getElementById("btnCancelCreate"))==null||a.addEventListener("click",()=>{var c,b;(c=document.getElementById("createLeaguePanel"))==null||c.classList.add("hidden"),(b=document.getElementById("btnShowCreate"))==null||b.classList.remove("hidden")}),(n=document.getElementById("joinCodeForm"))==null||n.addEventListener("submit",async c=>{c.preventDefault();const b=document.getElementById("inviteCode").value.trim().toUpperCase();if(b)try{const{league:v}=await u.leagues.join({invite_code:b});_(`Te has unido a "${v.name}"`),E.navigate(`/ligas/${v.id}`)}catch(v){_(v.message,"error")}}),(s=document.getElementById("createLeagueForm"))==null||s.addEventListener("submit",async c=>{var C;c.preventDefault();const b=document.getElementById("createBtn");b.disabled=!0,b.textContent="Creando…";const v=document.getElementById("leagueName").value.trim(),f=document.getElementById("leagueDesc").value.trim(),S=document.getElementById("leaguePrize").value.trim(),x=document.getElementById("isPublic").checked,w=((C=document.getElementById("isOfficial"))==null?void 0:C.checked)??!1;try{const{league:r}=await u.leagues.create({name:v,description:f,prize:S,is_public:x,is_official:w});Pe(r)}catch(r){_(r.message,"error"),b.disabled=!1,b.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function X(e,t=!1,a=new Set,n=!1){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=n?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:t?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(t),_("Enlace copiado")}catch{_("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:t})}catch{}}))}async function Te(e,{params:t}){var n,s,i,o,d;const a=parseInt(t.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const p=await u.leagues.get(a),{league:m,ranking:h,is_member:c,is_admin_view:b}=p,v=y.getUser(),f=m.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
        ...
        ${(c||v!=null&&v.is_admin)&&m.invite_link?`
        ...
        <div class="league-actions">
          ${c?'<button class="btn btn--danger btn--sm" id="btnLeave">Abandonar liga</button>':!(v!=null&&v.is_admin)&&v?'<button class="btn btn--primary" id="btnJoin">Unirse a esta liga</button>':""}
          ${v!=null&&v.is_admin||c&&v&&m.created_by===v.id?'<button class="btn btn--outline btn--sm" id="btnEditLeague">Editar liga</button>':""}
        </div>
  ...
  `}async function De(e){if(!y.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{users:t}=await u.auth.users();e.innerHTML=`
      <div class="container">
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const a=document.getElementById("syncResult");a.textContent="Sincronizando…";try{await u.matches.sync(),a.textContent="✓ Sincronización completada",_("Sincronización completada")}catch(n){a.textContent=`Error: ${n.message}`,_(n.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async a=>{a.preventDefault();const n=document.getElementById("winnerTeam").value.trim();if(n)try{const{message:s}=await u.predictions.awardChampion(n);_(s)}catch(s){_(s.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async a=>{const n=a.target.closest(".toggle-admin");if(!n)return;const s=parseInt(n.dataset.id);try{const{user:i}=await u.auth.toggleAdmin(s);n.closest("tr").querySelector(".admin-badge").textContent=i.is_admin?"Sí":"No",_(`${i.username} ${i.is_admin?"ahora es admin":"ya no es admin"}`)}catch(i){_(i.message,"error")}})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}function Oe(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
  ...
  `}async function Ye(e,{query:t}){const a=(t.codigo||"").trim().toUpperCase();if(!a){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!y.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",a),E.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:n}=await u.leagues.joinByCode(a);_(`¡Te has unido a "${n.name}"!`),E.navigate(`/ligas/${n.id}`)}catch(n){if(n.status===409){_("Ya eres miembro de esta liga");try{const{leagues:s}=await u.leagues.my(),i=s.find(o=>o.invite_code===a);if(i){E.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
  ...
  `}const Ze={"/":ce,"/login":be,"/register":fe,"/quiniela":$e,"/resultados":Fe,"/ranking":Be,"/tablon":ae,"/ligas":ke,"/ligas/:id":Te,"/perfil":je,"/campeon":te,"/admin":De,"/forgot-password":Re,"/reset-password":Ue,"/unirse":Ye,"/jugador/:id":Ke};function ea(e){for(const[t,a]of Object.entries(Ze)){const n=[],s=new RegExp("^"+t.replace(/:([^/]+)/g,(o,d)=>(n.push(d),"([^/]+)"))+"$"),i=e.match(s);if(i){const o={};return n.forEach((d,p)=>{o[d]=i[p+1]}),{handler:a,params:o}}}return null}const K=()=>document.getElementById("mainContent"),E={init(){window.addEventListener("hashchange",()=>this.resolve()),this.resolve()},navigate(e){window.location.hash=e},resolve(){const e=window.location.hash.slice(1)||"/",[t,a]=e.split("?"),n=Object.fromEntries(new URLSearchParams(a||"")),s=ea(t);if(!s){K().innerHTML='<div class="error-page"><h2>Página no encontrada</h2><a href="#/">Volver al inicio</a></div>';return}const{handler:i,params:o}=s;if(["/perfil","/campeon","/admin"].includes(t)&&!y.isLoggedIn()){this.navigate("/login");return}if(t==="/admin"&&!y.isAdmin()){this.navigate("/");return}const p=K();p.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>',i(p,{params:o,query:n})}};let H=[],D=null,N=null;async function aa(){await y.init(),E.init(),na(),ta()}function se(){return localStorage.getItem("pwa_installed")==="true"||window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===!0}function ta(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),!se()&&(D=e,sa())}),window.addEventListener("appinstalled",()=>{var e;localStorage.setItem("pwa_installed","true"),D=null,(e=document.getElementById("installBanner"))==null||e.remove()})}function sa(){if(se()||sessionStorage.getItem("installBannerDismissed"))return;const e=document.createElement("div");e.id="installBanner",e.className="install-banner",e.innerHTML=`
