(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function a(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=a(n);fetch(n.href,i)}})();const U="https://pickgoal-backend.onrender.com/api";function R(){return localStorage.getItem("token")}async function u(e,t={}){const a={"Content-Type":"application/json",...t.headers},s=R();s&&(a.Authorization=`Bearer ${s}`);const n=await fetch(`${U}${e}`,{...t,headers:a}),i=await n.json().catch(()=>({}));if(!n.ok)throw{status:n.status,message:i.error||"Error desconocido"};return i}const l={get:e=>u(e),post:(e,t)=>u(e,{method:"POST",body:JSON.stringify(t)}),patch:(e,t)=>u(e,{method:"PATCH",body:JSON.stringify(t)}),delete:e=>u(e,{method:"DELETE"}),auth:{register:e=>u("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>u("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>u("/auth/me"),forgotPassword:e=>u("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,t)=>u("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:t})}),ranking:()=>u("/auth/ranking"),users:()=>u("/auth/users"),toggleAdmin:e=>u(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>u("/matches/grouped"),list:(e="")=>u(`/matches/${e}`),get:e=>u(`/matches/${e}`),sync:()=>u("/matches/sync",{method:"POST"})},predictions:{mine:e=>u(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,t)=>u(`/predictions/match/${e}${t?`?league_id=${t}`:""}`),save:e=>u("/predictions/",{method:"POST",body:JSON.stringify(e)}),getChampion:e=>u(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,t)=>u("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:t??null})}),awardChampion:e=>u("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>u("/leagues/all"),public:()=>u("/leagues/public"),my:()=>u("/leagues/my"),create:e=>u("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>u("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>u(`/leagues/join/${encodeURIComponent(e)}`),get:e=>u(`/leagues/${e}`),leave:e=>u(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,t)=>u(`/leagues/${e}/predictions/${t}`)},board:{messages:(e=1)=>u(`/board/?page=${e}`),post:e=>u("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>u(`/board/${e}`,{method:"DELETE"})}};let y=null;const h={async init(){if(localStorage.getItem("token"))try{const{user:t}=await l.auth.me();y=t}catch{localStorage.removeItem("token")}},setUser(e,t){y=e,localStorage.setItem("token",t),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){y=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return y},isLoggedIn(){return!!y},isAdmin(){return(y==null?void 0:y.is_admin)===!0}};function G(e){const t=h.getUser();e.innerHTML=`
  ...
  `}let B=null;function m(e,t="success"){let a=document.getElementById("toast");a||(a=document.createElement("div"),a.id="toast",document.body.appendChild(a)),a.textContent=e,a.className=`toast toast--${t} toast--visible`,B&&clearTimeout(B),B=setTimeout(()=>{a.classList.remove("toast--visible")},3e3)}function j(){return`
    <div class="container">
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("registerBtn"),s=document.getElementById("registerError");a.disabled=!0,a.textContent="Creando cuenta…",s.classList.add("hidden");const n={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:r}=await l.auth.register(n);h.setUser(r,i),m("¡Cuenta creada! Bienvenido a PickGoal");const o=sessionStorage.getItem("pendingInviteCode");if(o){sessionStorage.removeItem("pendingInviteCode");try{const{league:c}=await l.leagues.joinByCode(o);m(`¡Te has unido a "${c.name}"!`),_.navigate(`/ligas/${c.id}`)}catch{_.navigate("/ligas")}}else _.navigate("/campeon")}catch(i){s.textContent=i.message||"Error al registrarse",s.classList.remove("hidden")}finally{a.disabled=!1,a.textContent="Crear cuenta"}})}function J(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function V(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:d}=await l.leagues.my();if(d.length===0){e.innerHTML=j();return}}const t=J(),[{groups:a},s]=await Promise.all([l.matches.grouped(),h.isLoggedIn()?l.predictions.mine(t):Promise.resolve({predictions:[]})]),n={};for(const d of s.predictions)n[d.match_id]=d;const i=a.flatMap(d=>d.matches),r=new Map;for(const d of i){const g=k(d.match_datetime);r.has(g)||r.set(g,[]),r.get(g).push(d)}const o=[...r.keys()].sort(),c=k(new Date().toISOString()),p=o.find(d=>d>=c)??o[0];e.innerHTML=`
      <div class="container">
    ...
    `,Y(o,p,r,n,t),h.isLoggedIn()&&W(i,n,t)}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${t.message}</p></div>`}}function Y(e,t,a,s,n){var r;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(o=>`
    <button class="date-nav__btn ${o===t?"date-nav__btn--active":""}" data-day="${o}">
      ${Q(o)}
    </button>
  `).join(""),(r=i.querySelector(".date-nav__btn--active"))==null||r.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),i.querySelectorAll(".date-nav__btn").forEach(o=>{o.addEventListener("click",()=>{i.querySelectorAll(".date-nav__btn").forEach(c=>c.classList.remove("date-nav__btn--active")),o.classList.add("date-nav__btn--active"),P(a.get(o.dataset.day)??[],s,n)})}),P(a.get(t)??[],s,n))}function P(e,t,a){const s=document.getElementById("matchesContent");if(s){if(e.length===0){s.innerHTML='<p class="empty">Sin partidos este día.</p>';return}s.innerHTML=`<div class="matches-grid">${e.map(n=>X(n,t[n.id])).join("")}</div>`,h.isLoggedIn()&&s.querySelectorAll(".prediction-form").forEach(n=>{Z(n,t,a)})}}function k(e){const t=new Date(e);return`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}-${String(t.getDate()).padStart(2,"0")}`}function Q(e){const[t,a,s]=e.split("-").map(Number);return new Date(t,a-1,s).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function X(e,t){const a=e.is_locked,s=t?`<span class="pts-badge">${t.total_points} pts</span>`:"",n={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
    <div class="match-card ${a?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!a&&h.isLoggedIn()?K(e,t):a&&t?`<div class="prediction-result">
               Tu predicción: <strong>${t.predicted_home}-${t.predicted_away}</strong>
               (${t.predicted_result}) · ${t.total_points} pts
             </div>`:""}
    </div>
  `}function K(e,t){const a=(t==null?void 0:t.predicted_home)??0,s=(t==null?void 0:t.predicted_away)??0,n=(t==null?void 0:t.predicted_result)??"X";return`
  ...
  `}async function W(e,t,a){const s=e.filter(n=>!n.is_locked&&!t[n.id]);for(const n of s)try{const{prediction:i}=await l.predictions.save({match_id:n.id,predicted_result:"X",predicted_home:0,predicted_away:0,league_id:a??null});t[n.id]=i}catch{}}function Z(e,t,a){e.addEventListener("submit",async s=>{var p;s.preventDefault();const n=parseInt(e.dataset.matchId),i=parseInt(e.querySelector("[name=predicted_home]").value),r=parseInt(e.querySelector("[name=predicted_away]").value),o=(p=e.querySelector("[name=predicted_result]:checked"))==null?void 0:p.value;if(isNaN(i)||isNaN(r)||!o)return;const c=e.querySelector("button");c.disabled=!0,c.textContent="…";try{const{prediction:d}=await l.predictions.save({match_id:n,predicted_result:o,predicted_home:i,predicted_away:r,league_id:a??null});t[n]=d,m("Predicción guardada"),c.textContent="✓ Guardado",setTimeout(()=>{c.disabled=!1,c.textContent="Guardar"},2e3)}catch(d){m(d.message||"Error al guardar","error"),c.disabled=!1,c.textContent="Guardar"}})}async function ee(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:s}=await l.leagues.my();if(s.length===0){e.innerHTML=j();return}}const{ranking:t}=await l.auth.ranking(),a=h.getUser();e.innerHTML=`
      <div class="container">
    ...
    `}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}async function te(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let t=1,a=1;async function s(){const{messages:o,pages:c}=await l.board.messages(t);return a=c,o}try{const o=await s();n(o)}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p></div>`}function n(o){var p,d,g;const c=h.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(p=document.getElementById("boardForm"))==null||p.addEventListener("submit",async v=>{v.preventDefault();const b=document.getElementById("boardMsg"),w=b.value.trim();if(w)try{await l.board.post(w),b.value="";const $=await s();document.getElementById("boardMessages").innerHTML=i($,c),r(c),m("Mensaje publicado")}catch($){m($.message,"error")}}),(d=document.getElementById("prevPage"))==null||d.addEventListener("click",async()=>{t--;const v=await s();document.getElementById("boardMessages").innerHTML=i(v,c),r(c)}),(g=document.getElementById("nextPage"))==null||g.addEventListener("click",async()=>{t++;const v=await s();document.getElementById("boardMessages").innerHTML=i(v,c),r(c)}),r(c)}function i(o,c){return o.length?o.map(p=>`
      <div class="board-message ${p.is_deleted?"board-message--deleted":""}" data-id="${p.id}">
        <div class="board-message__header">
          <strong>${p.username}</strong>
          <span class="board-message__date">${S(p.created_at)}</span>
          ${!p.is_deleted&&c&&(c.id===p.user_id||c.is_admin)?`<button class="btn btn--danger btn--xs delete-msg" data-id="${p.id}">✕</button>`:""}
        </div>
        <p class="board-message__text">${ae(p.message)}</p>
      </div>
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function r(o){e.querySelectorAll(".delete-msg").forEach(c=>{c.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await l.board.delete(c.dataset.id);const p=await s();document.getElementById("boardMessages").innerHTML=i(p,o),r(o),m("Mensaje eliminado")}catch(p){m(p.message,"error")}})})}}function ae(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function ne(e){var t,a,s,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[i,r]=await Promise.all([l.leagues.all(),h.isLoggedIn()?l.leagues.my():Promise.resolve({leagues:[]})]),o=h.getUser(),c=new Set(r.leagues.map(d=>d.id)),p=i.leagues.filter(d=>!c.has(d.id));e.innerHTML=`
      <div class="container">
    ...
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(d=>{d.addEventListener("click",()=>_.navigate(`/ligas/${d.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(d=>{d.addEventListener("click",async g=>{g.stopPropagation();const v=parseInt(d.dataset.id);d.disabled=!0,d.textContent="…";try{const{league:b}=await l.leagues.join({league_id:v});m(`¡Te has unido a "${b.name}"!`),_.navigate(`/ligas/${b.id}`)}catch(b){m(b.message,"error"),d.disabled=!1,d.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(d=>{d.addEventListener("click",g=>{g.stopPropagation(),m("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(t=document.getElementById("btnShowCreate"))==null||t.addEventListener("click",()=>{var d,g;(d=document.getElementById("createLeaguePanel"))==null||d.classList.remove("hidden"),(g=document.getElementById("btnShowCreate"))==null||g.classList.add("hidden")}),(a=document.getElementById("btnCancelCreate"))==null||a.addEventListener("click",()=>{var d,g;(d=document.getElementById("createLeaguePanel"))==null||d.classList.add("hidden"),(g=document.getElementById("btnShowCreate"))==null||g.classList.remove("hidden")}),(s=document.getElementById("joinCodeForm"))==null||s.addEventListener("submit",async d=>{d.preventDefault();const g=document.getElementById("inviteCode").value.trim().toUpperCase();if(g)try{const{league:v}=await l.leagues.join({invite_code:g});m(`Te has unido a "${v.name}"`),_.navigate(`/ligas/${v.id}`)}catch(v){m(v.message,"error")}}),(n=document.getElementById("createLeagueForm"))==null||n.addEventListener("submit",async d=>{var C;d.preventDefault();const g=document.getElementById("createBtn");g.disabled=!0,g.textContent="Creando…";const v=document.getElementById("leagueName").value.trim(),b=document.getElementById("leagueDesc").value.trim(),w=document.getElementById("leaguePrize").value.trim(),$=document.getElementById("isPublic").checked,O=((C=document.getElementById("isOfficial"))==null?void 0:C.checked)??!1;try{const{league:I}=await l.leagues.create({name:v,description:b,prize:w,is_public:$,is_official:O});se(I)}catch(I){m(I.message,"error"),g.disabled=!1,g.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function T(e,t=!1,a=new Set){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",n=e.is_public?"🌍":"🔒",i=t?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
    <div class="league-card ${t?"league-card--mine":""}" data-id="${e.id}" data-navigate="${t}">
      ...
      <div class="league-card__meta">
        <span>${n} ${e.is_public?"Pública":"Privada"}</span>
  ...
  `}function se(e){var s,n;const t=e.invite_link||"",a=document.getElementById("createLeaguePanel");a&&(a.innerHTML=`
  ...
  `,(s=document.getElementById("btnCopyLink"))==null||s.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(t),m("Enlace copiado")}catch{m("No se pudo copiar","error")}}),(n=document.getElementById("btnShare"))==null||n.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:t})}catch{}}))}async function ie(e,{params:t}){var s,n,i,r;const a=parseInt(t.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:o,ranking:c,is_member:p}=await l.leagues.get(a),d=h.getUser(),g=o.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
    ...
    `,(s=document.getElementById("btnCopyInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(o.invite_link),m("Enlace copiado")}catch{m("No se pudo copiar","error")}}),(n=document.getElementById("btnShareInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${o.name} en PickGoal`,url:o.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await l.leagues.leave(a),m("Has abandonado la liga"),_.navigate("/ligas")}catch(v){m(v.message,"error")}}),(r=document.getElementById("btnJoin"))==null||r.addEventListener("click",async()=>{try{await l.leagues.join({league_id:a}),m("¡Te has unido a la liga!"),_.navigate(`/ligas/${a}`)}catch(v){m(v.message,"error")}})}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p><a href="#/ligas">Volver</a></div>`}}async function re(e){var a,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const t=h.getUser();try{const[n,i,r]=await Promise.all([l.predictions.mine(),l.predictions.getChampion(),l.leagues.my()]),o=n.predictions.reduce((c,p)=>c+p.total_points,0)+(((a=i.champion_prediction)==null?void 0:a.points_earned)||0);e.innerHTML=`
      <div class="container">
        ...
        <section class="section">
          ...
          ${n.predictions.length?`<div class="predictions-list">${n.predictions.map(oe).join("")}</div>`:'<p class="empty">Sin predicciones aún. <a href="#/quiniela">Ir a la quiniela</a></p>'}
        </section>
    ...
    `,(s=e.querySelector("#btnLogoutPerfil"))==null||s.addEventListener("click",()=>{h.logout(),window.location.hash="/"})}catch(n){e.innerHTML=`<div class="container"><p class="form__error">Error: ${n.message}</p></div>`}}function oe(e){return`
  ...
  `}const M=new Date("2026-06-11T00:00:00Z"),ce=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];async function q(e){var t;if(!h.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{champion_prediction:a}=await l.predictions.getChampion(),s=new Date>=M;e.innerHTML=`
      <div class="container">
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const a=document.getElementById("syncResult");a.textContent="Sincronizando…";try{await l.matches.sync(),a.textContent="✓ Sincronización completada",m("Sincronización completada")}catch(s){a.textContent=`Error: ${s.message}`,m(s.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async a=>{a.preventDefault();const s=document.getElementById("winnerTeam").value.trim();if(s)try{const{message:n}=await l.predictions.awardChampion(s);m(n)}catch(n){m(n.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async a=>{const s=a.target.closest(".toggle-admin");if(!s)return;const n=parseInt(s.dataset.id);try{const{user:i}=await l.auth.toggleAdmin(n);s.closest("tr").querySelector(".admin-badge").textContent=i.is_admin?"Sí":"No",m(`${i.username} ${i.is_admin?"ahora es admin":"ya no es admin"}`)}catch(i){m(i.message,"error")}})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}function le(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
  ...
  `,document.getElementById("resetForm").addEventListener("submit",async s=>{s.preventDefault();const n=document.getElementById("resetBtn"),i=document.getElementById("resetError"),r=document.getElementById("password").value;n.disabled=!0,n.textContent="Guardando…",i.classList.add("hidden");try{await l.auth.resetPassword(a,r),m("Contraseña actualizada. Ya puedes iniciar sesión."),_.navigate("/login")}catch(o){i.textContent=o.message||"Error al restablecer la contraseña",i.classList.remove("hidden")}finally{n.disabled=!1,n.textContent="Guardar contraseña"}})}const pe={r32:"Dieciseisavos",r16:"Octavos",quarters:"Cuartos",semis:"Semis",third:"3º y 4º",final:"Final"};async function ge(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{groups:t}=await l.matches.grouped();e.innerHTML=`
      <div class="container">
        <h1 class="page-title">Resultados — Mundial 2026</h1>
        <nav class="phase-nav" id="phaseNav"></nav>
        <div id="phaseContent"></div>
      </div>
    `,ve(t)}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${t.message}</p></div>`}}function ve(e){var i;const t=document.getElementById("phaseNav");if(!t)return;const a=e.filter(r=>r.phase==="group"),s=e.filter(r=>r.phase!=="group"),n=[...a.map(r=>({key:`group_${r.group_name}`,label:`Grupo ${r.group_name}`,data:r,isGroup:!0})),...s.map(r=>({key:r.phase,label:pe[r.phase]||r.label,data:r,isGroup:!1}))];n.length!==0&&(t.innerHTML=n.map((r,o)=>`
    <button class="phase-nav__btn ${o===0?"phase-nav__btn--active":""}" data-key="${r.key}">
      ${r.label}
    </button>
  `).join(""),(i=t.querySelector(".phase-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),t.querySelectorAll(".phase-nav__btn").forEach((r,o)=>{r.addEventListener("click",()=>{t.querySelectorAll(".phase-nav__btn").forEach(p=>p.classList.remove("phase-nav__btn--active")),r.classList.add("phase-nav__btn--active");const c=n.find(p=>p.key===r.dataset.key);c&&x(c.data,c.isGroup)})}),x(n[0].data,n[0].isGroup))}function x(e,t){const a=document.getElementById("phaseContent");if(!a)return;const s=he(e.matches);if(t){const n=_e(e.matches);a.innerHTML=`
    ...
    `}).join("")}function _e(e){const t={};for(const a of e)if(t[a.home_team]||(t[a.home_team]={name:a.home_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),t[a.away_team]||(t[a.away_team]={name:a.away_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),a.status==="finished"&&a.home_score_90!==null&&a.away_score_90!==null){const s=t[a.home_team],n=t[a.away_team];s.pj++,n.pj++,s.gf+=a.home_score_90,s.gc+=a.away_score_90,n.gf+=a.away_score_90,n.gc+=a.home_score_90,a.home_score_90>a.away_score_90?(s.g++,s.pts+=3,n.p++):a.home_score_90<a.away_score_90?(n.g++,n.pts+=3,s.p++):(s.e++,s.pts++,n.e++,n.pts++)}return Object.values(t).sort((a,s)=>{if(s.pts!==a.pts)return s.pts-a.pts;const n=s.gf-s.gc,i=a.gf-a.gc;return n!==i?n-i:s.gf-a.gf})}function be(e){return e.length===0?'<p class="empty">Sin datos de clasificación.</p>':`
  ...
  `}async function fe(e,{query:t}){const a=(t.codigo||"").trim().toUpperCase();if(!a){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!h.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",a),_.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:s}=await l.leagues.joinByCode(a);m(`¡Te has unido a "${s.name}"!`),_.navigate(`/ligas/${s.id}`)}catch(s){if(s.status===409){m("Ya eres miembro de esta liga");try{const{leagues:n}=await l.leagues.my(),i=n.find(r=>r.invite_code===a);if(i){_.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
    ...
    `}}const ye={"/":G,"/login":F,"/register":z,"/quiniela":V,"/resultados":ge,"/ranking":ee,"/tablon":te,"/ligas":ne,"/ligas/:id":ie,"/perfil":re,"/campeon":q,"/admin":de,"/forgot-password":ue,"/reset-password":me,"/unirse":fe};function Ee(e){for(const[t,a]of Object.entries(ye)){const s=[],n=new RegExp("^"+t.replace(/:([^/]+)/g,(r,o)=>(s.push(o),"([^/]+)"))+"$"),i=e.match(n);if(i){const r={};return s.forEach((o,c)=>{r[o]=i[c+1]}),{handler:a,params:r}}}return null}const H=()=>document.getElementById("mainContent"),_={init(){window.addEventListener("hashchange",()=>this.resolve()),this.resolve()},navigate(e){window.location.hash=e},resolve(){const e=window.location.hash.slice(1)||"/",[t,a]=e.split("?"),s=Object.fromEntries(new URLSearchParams(a||"")),n=Ee(t);if(!n){H().innerHTML='<div class="error-page"><h2>Página no encontrada</h2><a href="#/">Volver al inicio</a></div>';return}const{handler:i,params:r}=n;if(["/perfil","/campeon","/admin"].includes(t)&&!h.isLoggedIn()){this.navigate("/login");return}if(t==="/admin"&&!h.isAdmin()){this.navigate("/");return}const c=H();c.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>',i(c,{params:r,query:s})}};let E=[],L=null;async function $e(){await h.init(),_.init(),Ie(),Le()}function Le(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),L=e,we()})}function we(){if(sessionStorage.getItem("installBannerDismissed"))return;const e=document.createElement("div");e.id="installBanner",e.className="install-banner",e.innerHTML=`
  ...
  `}function A(){const e=window.location.hash.slice(1).split("?")[0]||"/";document.querySelectorAll(".bottom-nav__item").forEach(t=>{const a=t.dataset.route,s=a==="/"?e==="/":e===a||e.startsWith(a+"/");t.classList.toggle("bottom-nav__item--active",s)})}$e();
