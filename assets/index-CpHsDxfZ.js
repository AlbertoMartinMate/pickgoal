(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const J="https://pickgoal-backend.onrender.com/api";function V(){return localStorage.getItem("token")}async function u(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=V();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${J}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const m={get:e=>u(e),post:(e,a)=>u(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>u(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>u(e,{method:"DELETE"}),auth:{register:e=>u("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>u("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>u("/auth/me"),forgotPassword:e=>u("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>u("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>u(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>u("/auth/users"),toggleAdmin:e=>u(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>u("/matches/grouped"),list:(e="")=>u(`/matches/${e}`),get:e=>u(`/matches/${e}`),sync:()=>u("/matches/sync",{method:"POST"})},predictions:{mine:e=>u(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>u(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>u("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>u(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>u(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>u("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>u("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>u("/leagues/all"),public:()=>u("/leagues/public"),my:()=>u("/leagues/my"),create:e=>u("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>u("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>u(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>u("/leagues/admin"),get:e=>u(`/leagues/${e}`),update:(e,a)=>u(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(a)}),leave:e=>u(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>u(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>u("/home/summary")},board:{messages:(e=1)=>u(`/board/?page=${e}`),post:e=>u("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>u(`/board/${e}`,{method:"DELETE"})}};let E=null;const h={async init(){if(localStorage.getItem("token"))try{const{user:a}=await m.auth.me();E=a}catch{localStorage.removeItem("token")}},setUser(e,a){E=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){E=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return E},isLoggedIn(){return!!E},isAdmin(){return(E==null?void 0:E.is_admin)===!0}};let B=null;function g(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,B&&clearTimeout(B),B=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function A(){return`
  ...
  `}function I(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}async function X(e){if(!h.getUser()){Y(e);return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{leagues_summary:t,upcoming_matches:n}=await m.home.summary();if(t.length===0){K(e);return}const s=(()=>{const o=localStorage.getItem("activeLeagueId");return o?parseInt(o):null})(),i=[...t].sort((o,r)=>o.league_id===s?-1:r.league_id===s?1:0);e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await m.auth.register(s);h.setUser(o,i),g("¡Cuenta creada! Bienvenido a PickGoal");const r=sessionStorage.getItem("pendingInviteCode");if(r){sessionStorage.removeItem("pendingInviteCode");try{const{league:l}=await m.leagues.joinByCode(r);g(`¡Te has unido a "${l.name}"!`),y.navigate(`/ligas/${l.id}`)}catch{y.navigate("/ligas")}}else y.navigate("/campeon"),D("/ligas")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function ne(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ie(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:_}=await m.leagues.my();if(_.length===0){e.innerHTML=A();return}}const a=ne(),[{groups:t},n]=await Promise.all([m.matches.grouped(),h.isLoggedIn()?m.predictions.mine(a):Promise.resolve({predictions:[]})]),s={};for(const _ of n.predictions)s[_.match_id]=_;const i=t.flatMap(_=>_.matches),o=new Map;for(const _ of i){const c=M(_.match_datetime);o.has(c)||o.set(c,[]),o.get(c).push(_)}const r=[...o.keys()].sort(),l=M(new Date().toISOString()),d=r.find(_=>_>=l)??r[0];e.innerHTML=`
      <div class="container">
  ...
  `).join(""),(o=i.querySelector(".date-nav__btn--active"))==null||o.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),i.querySelectorAll(".date-nav__btn").forEach(r=>{r.addEventListener("click",()=>{i.querySelectorAll(".date-nav__btn").forEach(l=>l.classList.remove("date-nav__btn--active")),r.classList.add("date-nav__btn--active"),k(t.get(r.dataset.day)??[],n,s)})}),k(t.get(a)??[],n,s))}function k(e,a,t){const n=document.getElementById("matchesContent");if(n){if(e.length===0){n.innerHTML='<p class="empty">Sin partidos este día.</p>';return}n.innerHTML=`<div class="matches-grid">${e.map(s=>le(s,a[s.id])).join("")}</div>`,h.isLoggedIn()&&n.querySelectorAll(".prediction-form").forEach(s=>{ce(s,a,t)})}}function M(e){const a=new Date(e);return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`}function re(e){const[a,t,n]=e.split("-").map(Number);return new Date(a,t-1,n).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function le(e,a){const t=e.is_locked,n=a?`<span class="pts-badge">${a.total_points} pts</span>`:"",s={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
    <div class="match-card ${t?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!t&&h.isLoggedIn()?de(e,a):t&&a?`<div class="prediction-result">
  ...
  `}function ce(e,a,t){const n=parseInt(e.dataset.matchId),s=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function r(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function l(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado",s.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(d=>{d.addEventListener("change",r),d.addEventListener("input",r)}),e.addEventListener("submit",async d=>{var p;d.preventDefault();const _=parseInt(e.querySelector("[name=predicted_home]").value),c=parseInt(e.querySelector("[name=predicted_away]").value),v=(p=e.querySelector("[name=predicted_result]:checked"))==null?void 0:p.value;if(!(isNaN(_)||isNaN(c)||!v)){s.disabled=!0,s.textContent="…";try{const{prediction:f}=await m.predictions.save({match_id:n,predicted_result:v,predicted_home:_,predicted_away:c,league_id:t??null});a[n]=f,g("Predicción guardada"),l()}catch(f){g(f.message||"Error al guardar","error"),s.disabled=!1,o?(s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado"):(s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar")}}})}function pe(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function me(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:r}=await m.leagues.my();if(r.length===0){e.innerHTML=A();return}}const a=pe(),[{ranking:t},n]=await Promise.all([m.auth.ranking(a),h.isLoggedIn()?m.leagues.my():Promise.resolve({leagues:[]})]),s=h.getUser(),i=n.leagues.find(r=>r.id===a),o=i?i.name:"Clasificación General";e.innerHTML=`
      <div class="container">
    ...
    `}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}async function ue(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let a=1,t=1;async function n(){const{messages:r,pages:l}=await m.board.messages(a);return t=l,r}try{const r=await n();s(r)}catch(r){e.innerHTML=`<div class="container"><p class="form__error">Error: ${r.message}</p></div>`}function s(r){var d,_,c;const l=h.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(c=>{c.addEventListener("click",()=>y.navigate(`/ligas/${c.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(c=>{c.addEventListener("click",async v=>{v.stopPropagation();const p=parseInt(c.dataset.id);c.disabled=!0,c.textContent="…";try{const{league:f}=await m.leagues.join({league_id:p});g(`¡Te has unido a "${f.name}"!`),y.navigate(`/ligas/${f.id}`)}catch(f){g(f.message,"error"),c.disabled=!1,c.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(c=>{c.addEventListener("click",v=>{v.stopPropagation(),g("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(a=document.getElementById("btnShowCreate"))==null||a.addEventListener("click",()=>{var c,v;(c=document.getElementById("createLeaguePanel"))==null||c.classList.remove("hidden"),(v=document.getElementById("btnShowCreate"))==null||v.classList.add("hidden")}),(t=document.getElementById("btnCancelCreate"))==null||t.addEventListener("click",()=>{var c,v;(c=document.getElementById("createLeaguePanel"))==null||c.classList.add("hidden"),(v=document.getElementById("btnShowCreate"))==null||v.classList.remove("hidden")}),(n=document.getElementById("joinCodeForm"))==null||n.addEventListener("submit",async c=>{c.preventDefault();const v=document.getElementById("inviteCode").value.trim().toUpperCase();if(v)try{const{league:p}=await m.leagues.join({invite_code:v});g(`Te has unido a "${p.name}"`),y.navigate(`/ligas/${p.id}`)}catch(p){g(p.message,"error")}}),(s=document.getElementById("createLeagueForm"))==null||s.addEventListener("submit",async c=>{var x;c.preventDefault();const v=document.getElementById("createBtn");v.disabled=!0,v.textContent="Creando…";const p=document.getElementById("leagueName").value.trim(),f=document.getElementById("leagueDesc").value.trim(),b=document.getElementById("leaguePrize").value.trim(),F=document.getElementById("isPublic").checked,R=((x=document.getElementById("isOfficial"))==null?void 0:x.checked)??!1;try{const{league:S}=await m.leagues.create({name:p,description:f,prize:b,is_public:F,is_official:R});ve(S)}catch(S){g(S.message,"error"),v.disabled=!1,v.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function T(e,a=!1,t=new Set,n=!1){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=n?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:a?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),g("Enlace copiado")}catch{g("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function he(e,{params:a}){var n,s,i,o,r;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const l=await m.leagues.get(t);console.log("[liga-detalle] API response for league",t,":",JSON.stringify(l));const{league:d,ranking:_,is_member:c,is_admin_view:v}=l,p=h.getUser();console.log("[liga-detalle] user:",JSON.stringify(p));const f=d.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
        ...
        <div class="league-header">
          <h1 class="page-title">${d.name} ${f}</h1>
        ...
        ${(c||p!=null&&p.is_admin)&&d.invite_link?`
        ...
        <div class="league-actions">
          ${c?'<button class="btn btn--danger btn--sm" id="btnLeave">Abandonar liga</button>':!(p!=null&&p.is_admin)&&p?'<button class="btn btn--primary" id="btnJoin">Unirse a esta liga</button>':""}
          ${(()=>{const b=(p==null?void 0:p.is_admin)||c&&p&&d.created_by===p.id;return console.log("[liga-detalle] canEdit:",b,"| user.is_admin:",p==null?void 0:p.is_admin,"| is_member:",c,"| created_by:",d.created_by,"| user.id:",p==null?void 0:p.id),b?'<button class="btn btn--outline btn--sm" id="btnEditLeague">Editar liga</button>':""})()}
        </div>
        ...
        <section class="section">
          ...
          <table class="ranking-table">
            ...
            <tbody>
              ${_.map(b=>`
                <tr class="${p&&b.id===p.id?"ranking-table__row--me":""}">
                  <td>${b.position}</td>
                  <td>${b.username}</td>
                  <td>${b.country||"—"}</td>
    ...
    `,console.log("[liga-detalle] post-render — btnEditLeague in DOM:",document.getElementById("btnEditLeague")),(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(d.invite_link),g("Enlace copiado")}catch{g("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${d.name} en PickGoal`,url:d.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await m.leagues.leave(t),g("Has abandonado la liga"),y.navigate("/ligas")}catch(b){g(b.message,"error")}}),(o=document.getElementById("btnJoin"))==null||o.addEventListener("click",async()=>{try{await m.leagues.join({league_id:t}),g("¡Te has unido a la liga!"),y.navigate(`/ligas/${t}`)}catch(b){g(b.message,"error")}}),(r=document.getElementById("btnEditLeague"))==null||r.addEventListener("click",()=>{be(d,t,p)})}catch(l){e.innerHTML=`<div class="container"><p class="form__error">Error: ${l.message}</p><a href="#/ligas">Volver</a></div>`}}function be(e,a,t){const n=document.getElementById("editLeagueModal");n&&n.remove();const s=document.createElement("div");s.id="editLeagueModal",s.className="edit-league-modal",s.innerHTML=`
  ...
  `,document.body.appendChild(s),requestAnimationFrame(()=>s.classList.add("edit-league-modal--open"));const i=()=>{s.classList.remove("edit-league-modal--open"),s.addEventListener("transitionend",()=>s.remove(),{once:!0})};s.querySelector(".edit-league-modal__overlay").addEventListener("click",i),document.getElementById("btnCancelEdit").addEventListener("click",i),document.getElementById("editLeagueForm").addEventListener("submit",async o=>{o.preventDefault();const r=document.getElementById("btnSaveEdit");r.disabled=!0,r.textContent="Guardando…";const l={name:document.getElementById("editName").value.trim(),description:document.getElementById("editDesc").value.trim(),prize:document.getElementById("editPrize").value.trim(),is_public:document.getElementById("editPublic").checked};t!=null&&t.is_admin&&(l.is_official=document.getElementById("editOfficial").checked);try{await m.leagues.update(a,l),g("Liga actualizada"),i(),y.navigate(`/ligas/${a}`)}catch(d){g(d.message,"error"),r.disabled=!1,r.textContent="Guardar cambios"}})}async function fe(e){var t,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=h.getUser();try{const s=(()=>{const p=localStorage.getItem("activeLeagueId");return p?parseInt(p):null})(),[i,o,r,l]=await Promise.all([m.predictions.mine(s),m.predictions.getChampion(s),m.leagues.my(),m.auth.me()]),d=i.predictions.reduce((p,f)=>p+f.total_points,0)+(((t=o.champion_prediction)==null?void 0:t.points_earned)||0),_=l.user,c=_.status,v=_.total_points_all_time;e.innerHTML=`
      <div class="container">
        ...
        <section class="section">
          <h2>Mis ligas</h2>
          ${r.leagues.length?`<ul class="leagues-list">${r.leagues.map(p=>`<li><a href="#/ligas/${p.id}">${p.name}</a> <span class="tag">${p.is_public?"Pública":"Privada"}</span></li>`).join("")}</ul>`:'<p class="empty">No perteneces a ninguna liga. <a href="#/ligas">Ver ligas</a></p>'}
        </section>
      </div>
    `,(n=e.querySelector("#btnLogoutPerfil"))==null||n.addEventListener("click",()=>{h.logout(),window.location.hash="/"})}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error: ${s.message}</p></div>`}}function ye(e,a){if(e.next_threshold===null)return`
  ...
  `}async function Le(e){if(!h.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{users:a}=await m.auth.users();e.innerHTML=`
      <div class="container">
  ...
  `}async function je(e,{query:a}){const t=(a.codigo||"").trim().toUpperCase();if(!t){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!h.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",t),y.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:n}=await m.leagues.joinByCode(t);g(`¡Te has unido a "${n.name}"!`),y.navigate(`/ligas/${n.id}`)}catch(n){if(n.status===409){g("Ya eres miembro de esta liga");try{const{leagues:s}=await m.leagues.my(),i=s.find(o=>o.invite_code===t);if(i){y.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
