(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const k="https://pickgoal-backend.onrender.com/api";function x(){return localStorage.getItem("token")}async function l(e,a={}){const t={"Content-Type":"application/json",...a.headers},s=x();s&&(t.Authorization=`Bearer ${s}`);const n=await fetch(`${k}${e}`,{...a,headers:t}),i=await n.json().catch(()=>({}));if(!n.ok)throw{status:n.status,message:i.error||"Error desconocido"};return i}const m={get:e=>l(e),post:(e,a)=>l(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>l(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>l(e,{method:"DELETE"}),auth:{register:e=>l("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>l("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>l("/auth/me"),forgotPassword:e=>l("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>l("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:()=>l("/auth/ranking"),users:()=>l("/auth/users"),toggleAdmin:e=>l(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>l("/matches/grouped"),list:(e="")=>l(`/matches/${e}`),get:e=>l(`/matches/${e}`),sync:()=>l("/matches/sync",{method:"POST"})},predictions:{mine:()=>l("/predictions/"),forMatch:e=>l(`/predictions/match/${e}`),save:e=>l("/predictions/",{method:"POST",body:JSON.stringify(e)}),getChampion:()=>l("/predictions/champion"),saveChampion:e=>l("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e})}),awardChampion:e=>l("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{public:()=>l("/leagues/public"),my:()=>l("/leagues/my"),create:e=>l("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>l("/leagues/join",{method:"POST",body:JSON.stringify(e)}),get:e=>l(`/leagues/${e}`),leave:e=>l(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>l(`/leagues/${e}/predictions/${a}`)},board:{messages:(e=1)=>l(`/board/?page=${e}`),post:e=>l("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>l(`/board/${e}`,{method:"DELETE"})}};let h=null;const p={async init(){if(localStorage.getItem("token"))try{const{user:a}=await m.auth.me();h=a}catch{localStorage.removeItem("token")}},setUser(e,a){h=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){h=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return h},isLoggedIn(){return!!h},isAdmin(){return(h==null?void 0:h.is_admin)===!0}};function H(e){const a=p.getUser();e.innerHTML=`
  ...
  `}let f=null;function u(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,f&&clearTimeout(f),f=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function P(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}function N(e){e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),s=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",s.classList.add("hidden");const n={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:r}=await m.auth.register(n);p.setUser(r,i),u("¡Cuenta creada! Bienvenido a PickGoal"),g.navigate("/campeon")}catch(i){s.textContent=i.message||"Error al registrarse",s.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}async function A(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[{groups:a},t]=await Promise.all([m.matches.grouped(),p.isLoggedIn()?m.predictions.mine():Promise.resolve({predictions:[]})]),s={};for(const d of t.predictions)s[d.match_id]=d;const n=a.flatMap(d=>d.matches),i=new Map;for(const d of n){const v=w(d.match_datetime);i.has(v)||i.set(v,[]),i.get(v).push(d)}const r=[...i.keys()].sort(),o=w(new Date().toISOString()),c=r.find(d=>d>=o)??r[0];e.innerHTML=`
    ...
    `,O(r,c,i,s),p.isLoggedIn()&&F(n,s)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${a.message}</p></div>`}}function O(e,a,t,s){var i;const n=document.getElementById("dateNav");n&&(n.innerHTML=e.map(r=>`
    <button class="date-nav__btn ${r===a?"date-nav__btn--active":""}" data-day="${r}">
      ${j(r)}
    </button>
  `).join(""),(i=n.querySelector(".date-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),n.querySelectorAll(".date-nav__btn").forEach(r=>{r.addEventListener("click",()=>{n.querySelectorAll(".date-nav__btn").forEach(o=>o.classList.remove("date-nav__btn--active")),r.classList.add("date-nav__btn--active"),L(t.get(r.dataset.day)??[],s)})}),L(t.get(a)??[],s))}function L(e,a){const t=document.getElementById("matchesContent");if(t){if(e.length===0){t.innerHTML='<p class="empty">Sin partidos este día.</p>';return}t.innerHTML=`<div class="matches-grid">${e.map(s=>D(s,a[s.id])).join("")}</div>`,p.isLoggedIn()&&t.querySelectorAll(".prediction-form").forEach(s=>{U(s,a)})}}function w(e){const a=new Date(e);return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`}function j(e){const[a,t,s]=e.split("-").map(Number);return new Date(a,t-1,s).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function D(e,a){const t=e.is_locked,s=a?`<span class="pts-badge">${a.total_points} pts</span>`:"",n={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
    <div class="match-card ${t?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!t&&p.isLoggedIn()?R(e,a):t&&a?`<div class="prediction-result">
               Tu predicción: <strong>${a.predicted_home}-${a.predicted_away}</strong>
               (${a.predicted_result}) · ${a.total_points} pts
             </div>`:""}
    </div>
  `}function R(e,a){const t=(a==null?void 0:a.predicted_home)??0,s=(a==null?void 0:a.predicted_away)??0,n=(a==null?void 0:a.predicted_result)??"X";return`
  ...
  `}async function F(e,a){const t=e.filter(s=>!s.is_locked&&!a[s.id]);for(const s of t)try{const{prediction:n}=await m.predictions.save({match_id:s.id,predicted_result:"X",predicted_home:0,predicted_away:0});a[s.id]=n}catch{}}function U(e,a){e.addEventListener("submit",async t=>{var c;t.preventDefault();const s=parseInt(e.dataset.matchId),n=parseInt(e.querySelector("[name=predicted_home]").value),i=parseInt(e.querySelector("[name=predicted_away]").value),r=(c=e.querySelector("[name=predicted_result]:checked"))==null?void 0:c.value;if(isNaN(n)||isNaN(i)||!r)return;const o=e.querySelector("button");o.disabled=!0,o.textContent="…";try{const{prediction:d}=await m.predictions.save({match_id:s,predicted_result:r,predicted_home:n,predicted_away:i});a[s]=d,u("Predicción guardada"),o.textContent="✓ Guardado",setTimeout(()=>{o.disabled=!1,o.textContent="Guardar"},2e3)}catch(d){u(d.message||"Error al guardar","error"),o.disabled=!1,o.textContent="Guardar"}})}async function z(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{ranking:a}=await m.auth.ranking(),t=p.getUser();e.innerHTML=`
    ...
    `}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}async function J(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let a=1,t=1;async function s(){const{messages:o,pages:c}=await m.board.messages(a);return t=c,o}try{const o=await s();n(o)}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p></div>`}function n(o){var d,v,y;const c=p.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(d=document.getElementById("boardForm"))==null||d.addEventListener("submit",async b=>{b.preventDefault();const E=document.getElementById("boardMsg"),$=E.value.trim();if($)try{await m.board.post($),E.value="";const _=await s();document.getElementById("boardMessages").innerHTML=i(_,c),r(c),u("Mensaje publicado")}catch(_){u(_.message,"error")}}),(v=document.getElementById("prevPage"))==null||v.addEventListener("click",async()=>{a--;const b=await s();document.getElementById("boardMessages").innerHTML=i(b,c),r(c)}),(y=document.getElementById("nextPage"))==null||y.addEventListener("click",async()=>{a++;const b=await s();document.getElementById("boardMessages").innerHTML=i(b,c),r(c)}),r(c)}function i(o,c){return o.length?o.map(d=>`
      <div class="board-message ${d.is_deleted?"board-message--deleted":""}" data-id="${d.id}">
        ...
        <p class="board-message__text">${G(d.message)}</p>
      </div>
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function r(o){e.querySelectorAll(".delete-msg").forEach(c=>{c.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await m.board.delete(c.dataset.id);const d=await s();document.getElementById("boardMessages").innerHTML=i(d,o),r(o),u("Mensaje eliminado")}catch(d){u(d.message,"error")}})})}}function G(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function V(e){var a,t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[s,n]=await Promise.all([m.leagues.public(),p.isLoggedIn()?m.leagues.my():Promise.resolve({leagues:[]})]),i=p.getUser();e.innerHTML=`
      <div class="container">
        ...
        ${i?`
          <section class="section">
            <h2>Mis ligas</h2>
            ${n.leagues.length?`<div class="leagues-grid">${n.leagues.map(I).join("")}</div>`:'<p class="empty">No perteneces a ninguna liga aún.</p>'}
          </section>
  ...
  `}async function Q(e,{params:a}){var s,n;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:i,ranking:r,is_member:o}=await m.leagues.get(t),c=p.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(s=document.getElementById("btnLeave"))==null||s.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await m.leagues.leave(t),u("Has abandonado la liga"),g.navigate("/ligas")}catch(d){u(d.message,"error")}}),(n=document.getElementById("btnJoin"))==null||n.addEventListener("click",async()=>{try{await m.leagues.join({league_id:t}),u("¡Te has unido a la liga!"),g.navigate(`/ligas/${t}`)}catch(d){u(d.message,"error")}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p><a href="#/ligas">Volver</a></div>`}}async function X(e){var t,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=p.getUser();try{const[n,i,r]=await Promise.all([m.predictions.mine(),m.predictions.getChampion(),m.leagues.my()]),o=n.predictions.reduce((c,d)=>c+d.total_points,0)+(((t=i.champion_prediction)==null?void 0:t.points_earned)||0);e.innerHTML=`
      <div class="container">
        ...
        <section class="section">
          ...
          ${n.predictions.length?`<div class="predictions-list">${n.predictions.map(Y).join("")}</div>`:'<p class="empty">Sin predicciones aún. <a href="#/quiniela">Ir a la quiniela</a></p>'}
        </section>
    ...
    `,(s=e.querySelector("#btnLogoutPerfil"))==null||s.addEventListener("click",()=>{p.logout(),window.location.hash="/"})}catch(n){e.innerHTML=`<div class="container"><p class="form__error">Error: ${n.message}</p></div>`}}function Y(e){return`
  ...
  `}const S=new Date("2026-06-11T00:00:00Z"),K=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];async function M(e){var a;if(!p.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{champion_prediction:t}=await m.predictions.getChampion(),s=new Date>=S;e.innerHTML=`
      <div class="container">
        ...
        <p class="champion-desc">
          ...
          (${S.toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric"})}).
        </p>
    ...
    `,(a=document.getElementById("championForm"))==null||a.addEventListener("submit",async n=>{n.preventDefault();const i=document.getElementById("champBtn"),r=document.getElementById("champError"),o=document.getElementById("teamSearch").value.trim();if(o){i.disabled=!0,i.textContent="Guardando…",r.classList.add("hidden");try{await m.predictions.saveChampion(o),u(`¡${o} guardado como campeón!`),M(e)}catch(c){r.textContent=c.message,r.classList.remove("hidden"),i.disabled=!1,i.textContent="Confirmar predicción"}}})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}async function W(e){if(!p.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{users:a}=await m.auth.users();e.innerHTML=`
      <div class="container">
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const t=document.getElementById("syncResult");t.textContent="Sincronizando…";try{await m.matches.sync(),t.textContent="✓ Sincronización completada",u("Sincronización completada")}catch(s){t.textContent=`Error: ${s.message}`,u(s.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async t=>{t.preventDefault();const s=document.getElementById("winnerTeam").value.trim();if(s)try{const{message:n}=await m.predictions.awardChampion(s);u(n)}catch(n){u(n.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async t=>{const s=t.target.closest(".toggle-admin");if(!s)return;const n=parseInt(s.dataset.id);try{const{user:i}=await m.auth.toggleAdmin(n);s.closest("tr").querySelector(".admin-badge").textContent=i.is_admin?"Sí":"No",u(`${i.username} ${i.is_admin?"ahora es admin":"ya no es admin"}`)}catch(i){u(i.message,"error")}})}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}function Z(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
