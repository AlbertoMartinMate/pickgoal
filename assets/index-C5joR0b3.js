(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const R="https://pickgoal-backend.onrender.com/api";function G(){return localStorage.getItem("token")}async function m(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=G();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${R}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const u={get:e=>m(e),post:(e,a)=>m(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>m(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>m(e,{method:"DELETE"}),auth:{register:e=>m("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>m("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>m("/auth/me"),forgotPassword:e=>m("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>m("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>m(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>m("/auth/users"),toggleAdmin:e=>m(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>m("/matches/grouped"),list:(e="")=>m(`/matches/${e}`),get:e=>m(`/matches/${e}`),sync:()=>m("/matches/sync",{method:"POST"})},predictions:{mine:e=>m(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>m(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>m("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>m(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>m(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>m("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>m("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>m("/leagues/all"),public:()=>m("/leagues/public"),my:()=>m("/leagues/my"),create:e=>m("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>m("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>m(`/leagues/join/${encodeURIComponent(e)}`),get:e=>m(`/leagues/${e}`),leave:e=>m(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>m(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>m("/home/summary")},board:{messages:(e=1)=>m(`/board/?page=${e}`),post:e=>m("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>m(`/board/${e}`,{method:"DELETE"})}};let y=null;const _={async init(){if(localStorage.getItem("token"))try{const{user:a}=await u.auth.me();y=a}catch{localStorage.removeItem("token")}},setUser(e,a){y=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){y=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return y},isLoggedIn(){return!!y},isAdmin(){return(y==null?void 0:y.is_admin)===!0}};let B=null;function p(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,B&&clearTimeout(B),B=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function q(){return`
  ...
  `}function E(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}async function F(e){if(!_.getUser()){z(e);return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{leagues_summary:t,upcoming_matches:n}=await u.home.summary();if(t.length===0){J(e);return}const s=(()=>{const r=localStorage.getItem("activeLeagueId");return r?parseInt(r):null})(),i=[...t].sort((r,o)=>r.league_id===s?-1:o.league_id===s?1:0);e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:r}=await u.auth.register(s);_.setUser(r,i),p("¡Cuenta creada! Bienvenido a PickGoal");const o=sessionStorage.getItem("pendingInviteCode");if(o){sessionStorage.removeItem("pendingInviteCode");try{const{league:c}=await u.leagues.joinByCode(o);p(`¡Te has unido a "${c.name}"!`),h.navigate(`/ligas/${c.id}`)}catch{h.navigate("/ligas")}}else h.navigate("/campeon")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function W(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Z(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(_.isLoggedIn()){const{leagues:d}=await u.leagues.my();if(d.length===0){e.innerHTML=q();return}}const a=W(),[{groups:t},n]=await Promise.all([u.matches.grouped(),_.isLoggedIn()?u.predictions.mine(a):Promise.resolve({predictions:[]})]),s={};for(const d of n.predictions)s[d.match_id]=d;const i=t.flatMap(d=>d.matches),r=new Map;for(const d of i){const g=T(d.match_datetime);r.has(g)||r.set(g,[]),r.get(g).push(d)}const o=[...r.keys()].sort(),c=T(new Date().toISOString()),l=o.find(d=>d>=c)??o[0];e.innerHTML=`
    ...
    `,ee(o,l,r,s,a),_.isLoggedIn()&&ne(i,s,a)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${a.message}</p></div>`}}function ee(e,a,t,n,s){var r;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(o=>`
    <button class="date-nav__btn ${o===a?"date-nav__btn--active":""}" data-day="${o}">
      ${ae(o)}
    </button>
  `).join(""),(r=i.querySelector(".date-nav__btn--active"))==null||r.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),i.querySelectorAll(".date-nav__btn").forEach(o=>{o.addEventListener("click",()=>{i.querySelectorAll(".date-nav__btn").forEach(c=>c.classList.remove("date-nav__btn--active")),o.classList.add("date-nav__btn--active"),k(t.get(o.dataset.day)??[],n,s)})}),k(t.get(a)??[],n,s))}function k(e,a,t){const n=document.getElementById("matchesContent");if(n){if(e.length===0){n.innerHTML='<p class="empty">Sin partidos este día.</p>';return}n.innerHTML=`<div class="matches-grid">${e.map(s=>te(s,a[s.id])).join("")}</div>`,_.isLoggedIn()&&n.querySelectorAll(".prediction-form").forEach(s=>{ie(s,a,t)})}}function T(e){const a=new Date(e);return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`}function ae(e){const[a,t,n]=e.split("-").map(Number);return new Date(a,t-1,n).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function te(e,a){const t=e.is_locked,n=a?`<span class="pts-badge">${a.total_points} pts</span>`:"",s={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
  ...
  `}async function ne(e,a,t){const n=e.filter(s=>!s.is_locked&&!a[s.id]);for(const s of n)try{const{prediction:i}=await u.predictions.save({match_id:s.id,predicted_result:"X",predicted_home:0,predicted_away:0,league_id:t??null});a[s.id]=i}catch{}}function ie(e,a,t){e.addEventListener("submit",async n=>{var l;n.preventDefault();const s=parseInt(e.dataset.matchId),i=parseInt(e.querySelector("[name=predicted_home]").value),r=parseInt(e.querySelector("[name=predicted_away]").value),o=(l=e.querySelector("[name=predicted_result]:checked"))==null?void 0:l.value;if(isNaN(i)||isNaN(r)||!o)return;const c=e.querySelector("button");c.disabled=!0,c.textContent="…";try{const{prediction:d}=await u.predictions.save({match_id:s,predicted_result:o,predicted_home:i,predicted_away:r,league_id:t??null});a[s]=d,p("Predicción guardada"),c.textContent="✓ Guardado",setTimeout(()=>{c.disabled=!1,c.textContent="Guardar"},2e3)}catch(d){p(d.message||"Error al guardar","error"),c.disabled=!1,c.textContent="Guardar"}})}function re(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function oe(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(_.isLoggedIn()){const{leagues:o}=await u.leagues.my();if(o.length===0){e.innerHTML=q();return}}const a=re(),[{ranking:t},n]=await Promise.all([u.auth.ranking(a),_.isLoggedIn()?u.leagues.my():Promise.resolve({leagues:[]})]),s=_.getUser(),i=n.leagues.find(o=>o.id===a),r=i?i.name:"Clasificación General";e.innerHTML=`
      <div class="container">
    ...
    `}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}async function ce(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let a=1,t=1;async function n(){const{messages:o,pages:c}=await u.board.messages(a);return t=c,o}try{const o=await n();s(o)}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p></div>`}function s(o){var l,d,g;const c=_.getUser();e.innerHTML=`
      <div class="container">
    ...
    `,(l=document.getElementById("boardForm"))==null||l.addEventListener("submit",async v=>{v.preventDefault();const b=document.getElementById("boardMsg"),w=b.value.trim();if(w)try{await u.board.post(w),b.value="";const L=await n();document.getElementById("boardMessages").innerHTML=i(L,c),r(c),p("Mensaje publicado")}catch(L){p(L.message,"error")}}),(d=document.getElementById("prevPage"))==null||d.addEventListener("click",async()=>{a--;const v=await n();document.getElementById("boardMessages").innerHTML=i(v,c),r(c)}),(g=document.getElementById("nextPage"))==null||g.addEventListener("click",async()=>{a++;const v=await n();document.getElementById("boardMessages").innerHTML=i(v,c),r(c)}),r(c)}function i(o,c){return o.length?o.map(l=>`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),p("Enlace copiado")}catch{p("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function me(e,{params:a}){var n,s,i,r;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:o,ranking:c,is_member:l}=await u.leagues.get(t),d=_.getUser(),g=o.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
    ...
    `,(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(o.invite_link),p("Enlace copiado")}catch{p("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${o.name} en PickGoal`,url:o.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await u.leagues.leave(t),p("Has abandonado la liga"),h.navigate("/ligas")}catch(v){p(v.message,"error")}}),(r=document.getElementById("btnJoin"))==null||r.addEventListener("click",async()=>{try{await u.leagues.join({league_id:t}),p("¡Te has unido a la liga!"),h.navigate(`/ligas/${t}`)}catch(v){p(v.message,"error")}})}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p><a href="#/ligas">Volver</a></div>`}}async function pe(e){var t,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=_.getUser();try{const s=(()=>{const l=localStorage.getItem("activeLeagueId");return l?parseInt(l):null})(),[i,r,o]=await Promise.all([u.predictions.mine(s),u.predictions.getChampion(s),u.leagues.my()]),c=i.predictions.reduce((l,d)=>l+d.total_points,0)+(((t=r.champion_prediction)==null?void 0:t.points_earned)||0);e.innerHTML=`
      <div class="container">
    ...
    `,(n=e.querySelector("#btnLogoutPerfil"))==null||n.addEventListener("click",()=>{_.logout(),window.location.hash="/"})}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error: ${s.message}</p></div>`}}function ge(e){return`
  ...
  `).join(""),(i=a.querySelector(".phase-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),a.querySelectorAll(".phase-nav__btn").forEach((r,o)=>{r.addEventListener("click",()=>{a.querySelectorAll(".phase-nav__btn").forEach(l=>l.classList.remove("phase-nav__btn--active")),r.classList.add("phase-nav__btn--active");const c=s.find(l=>l.key===r.dataset.key);c&&j(c.data,c.isGroup)})}),j(s[0].data,s[0].isGroup))}function j(e,a){const t=document.getElementById("phaseContent");if(!t)return;const n=Ie(e.matches);if(a){const s=we(e.matches);t.innerHTML=`
    ...
    `}else t.innerHTML=`<div class="resultados-matches">${n}</div>`}function Ie(e){return!e||e.length===0?'<p class="empty">Sin partidos en esta fase.</p>':e.map(a=>{const t={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[a.status]||a.status,n=a.status!=="scheduled"?`<span class="res-score">${a.home_score_90??"?"} - ${a.away_score_90??"?"}</span>`:'<span class="res-score res-score--pending">vs</span>';return`
    ...
    `}).join("")}function we(e){const a={};for(const t of e)if(a[t.home_team]||(a[t.home_team]={name:t.home_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),a[t.away_team]||(a[t.away_team]={name:t.away_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),t.status==="finished"&&t.home_score_90!==null&&t.away_score_90!==null){const n=a[t.home_team],s=a[t.away_team];n.pj++,s.pj++,n.gf+=t.home_score_90,n.gc+=t.away_score_90,s.gf+=t.away_score_90,s.gc+=t.home_score_90,t.home_score_90>t.away_score_90?(n.g++,n.pts+=3,s.p++):t.home_score_90<t.away_score_90?(s.g++,s.pts+=3,n.p++):(n.e++,n.pts++,s.e++,s.pts++)}return Object.values(a).sort((t,n)=>{if(n.pts!==t.pts)return n.pts-t.pts;const s=n.gf-n.gc,i=t.gf-t.gc;return s!==i?s-i:n.gf-t.gf})}function Se(e){return e.length===0?'<p class="empty">Sin datos de clasificación.</p>':`
