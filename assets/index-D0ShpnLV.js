(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const de="https://pickgoal-backend.onrender.com/api";function ce(){return localStorage.getItem("token")}async function g(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=ce();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${de}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const u={get:e=>g(e),post:(e,a)=>g(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>g(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>g(e,{method:"DELETE"}),auth:{register:e=>g("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>g("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>g("/auth/me"),forgotPassword:e=>g("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>g("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>g(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>g("/auth/users"),toggleAdmin:e=>g(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>g("/matches/grouped"),list:(e="")=>g(`/matches/${e}`),get:e=>g(`/matches/${e}`),sync:()=>g("/matches/sync",{method:"POST"})},predictions:{mine:e=>g(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>g(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>g("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>g(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>g(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>g("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>g("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>g("/leagues/all"),public:()=>g("/leagues/public"),my:()=>g("/leagues/my"),create:e=>g("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>g("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>g(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>g("/leagues/admin"),get:e=>g(`/leagues/${e}`),update:(e,a)=>g(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(a)}),leave:e=>g(`/leagues/${e}/leave`,{method:"DELETE"}),members:e=>g(`/leagues/${e}/members`),matchPredictions:(e,a)=>g(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>g("/home/summary")},board:{messages:(e=1,a=null)=>g(`/board/?page=${e}${a?`&league_id=${a}`:""}`),unread:(e,a)=>g(`/board/unread?league_id=${e}&since=${encodeURIComponent(a)}`),post:(e,a=null)=>g("/board/",{method:"POST",body:JSON.stringify({message:e,league_id:a})}),pin:e=>g(`/board/${e}/pin`,{method:"POST"}),reply:(e,a)=>g(`/board/${e}/reply`,{method:"POST",body:JSON.stringify({message:a})}),delete:e=>g(`/board/${e}`,{method:"DELETE"})},notifications:{vapidPublicKey:()=>g("/notifications/vapid-public-key"),subscribe:e=>g("/notifications/subscribe",{method:"POST",body:JSON.stringify(e)}),send:e=>g("/notifications/send",{method:"POST",body:JSON.stringify(e)})}};let B=null;const y={async init(){if(localStorage.getItem("token"))try{const{user:a}=await u.auth.me();B=a}catch{localStorage.removeItem("token")}},setUser(e,a){B=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){B=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return B},isLoggedIn(){return!!B},isAdmin(){return(B==null?void 0:B.is_admin)===!0}};let R=null;function v(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,R&&clearTimeout(R),R=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function ae(){return`
  ...
  `}function T(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}async function ue(e){if(!y.getUser()){pe(e);return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{leagues_summary:t,upcoming_matches:n}=await u.home.summary();if(t.length===0){me(e);return}const s=(()=>{const o=localStorage.getItem("activeLeagueId");return o?parseInt(o):null})(),i=[...t].sort((o,l)=>o.league_id===s?-1:l.league_id===s?1:0);e.innerHTML=`
  ...
  `}function ge(e){return`${e}º`}function _e(e){const a=e.next_to_predict?`<div class="league-card__next">
    ...
    <div class="league-card" data-league-id="${e.league_id}">
      <div class="league-card__header">
        ...
        <span class="league-card__rank">${ge(e.rank)} de ${e.member_count}</span>
      </div>
  ...
  `}function he(e){const a=e.querySelector("#pointsModal"),t=e.querySelector("#btnPointsInfo"),n=e.querySelector("#pointsClose"),s=e.querySelector("#pointsOverlay");function i(){a.classList.add("points-modal--open"),document.body.style.overflow="hidden"}function o(){a.classList.remove("points-modal--open"),document.body.style.overflow=""}t==null||t.addEventListener("click",i),n==null||n.addEventListener("click",o),s==null||s.addEventListener("click",o),document.addEventListener("keydown",l=>{l.key==="Escape"&&o()},{once:!1})}function be(e){return e.length?`
  ...
  `:""}const z="pickgoal_welcome_shown";function te(e="/ligas"){if(localStorage.getItem(z))return;localStorage.setItem(z,"1");const a=document.createElement("div");a.innerHTML=`
  ...
  `;const t=a.firstElementChild;document.body.appendChild(t),document.body.style.overflow="hidden",requestAnimationFrame(()=>t.classList.add("welcome-modal--open"));function n(s){t.classList.remove("welcome-modal--open"),document.body.style.overflow="",t.addEventListener("transitionend",()=>t.remove(),{once:!0}),s&&(window.location.hash=s)}document.getElementById("welcomeOverlay").addEventListener("click",()=>n()),document.getElementById("welcomeCta").addEventListener("click",()=>n(e)),document.addEventListener("keydown",function s(i){i.key==="Escape"&&(n(),document.removeEventListener("keydown",s))})}function fe(e){e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await u.auth.register(s);y.setUser(o,i),v("¡Cuenta creada! Bienvenido a PickGoal");const l=sessionStorage.getItem("pendingInviteCode");if(l){sessionStorage.removeItem("pendingInviteCode");try{const{league:c}=await u.leagues.joinByCode(l);v(`¡Te has unido a "${c.name}"!`),E.navigate(`/ligas/${c.id}`)}catch{E.navigate("/ligas")}}else E.navigate("/campeon"),te("/ligas")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function J(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function $e(e){var a;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{let t=null;if(y.isLoggedIn()){const{leagues:_}=await u.leagues.my();if(_.length===0){e.innerHTML=ae();return}const b=J(),f=_.find(w=>w.id===b);t=f?f.name:((a=_[0])==null?void 0:a.name)??null}const n=J(),[{groups:s},i]=await Promise.all([u.matches.grouped(),y.isLoggedIn()?u.predictions.mine(n):Promise.resolve({predictions:[]})]),o={};for(const _ of i.predictions)o[_.match_id]=_;const l=s.flatMap(_=>_.matches),c=new Map;for(const _ of l){const b=X(_.match_datetime);c.has(b)||c.set(b,[]),c.get(b).push(_)}const m=[...c.keys()].sort(),h=X(new Date().toISOString()),p=m.find(_=>_>=h)??m[0];e.innerHTML=`
      ${t?`<span class="page-league-name">${t}</span>`:""}
      <div class="container">
    ...
    `,Ee(m,p,c,o,n)}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${t.message}</p></div>`}}function Ee(e,a,t,n,s){var o;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(l=>`
    <button class="date-nav__btn ${l===a?"date-nav__btn--active":""}" data-day="${l}">
      ${Le(l)}
    </button>
  `).join(""),(o=i.querySelector(".date-nav__btn--active"))==null||o.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),i.querySelectorAll(".date-nav__btn").forEach(l=>{l.addEventListener("click",()=>{i.querySelectorAll(".date-nav__btn").forEach(c=>c.classList.remove("date-nav__btn--active")),l.classList.add("date-nav__btn--active"),V(t.get(l.dataset.day)??[],n,s)})}),V(t.get(a)??[],n,s))}function V(e,a,t){const n=document.getElementById("matchesContent");if(n){if(e.length===0){n.innerHTML='<p class="empty">Sin partidos este día.</p>';return}n.innerHTML=`<div class="matches-grid">${e.map(s=>we(s,a[s.id])).join("")}</div>`,y.isLoggedIn()&&n.querySelectorAll(".prediction-form").forEach(s=>{Se(s,a,t)})}}function X(e){const a=new Date(e);return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`}function Le(e){const[a,t,n]=e.split("-").map(Number);return new Date(a,t-1,n).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function we(e,a){const t=e.is_locked,n=a?`<span class="pts-badge">${a.total_points} pts</span>`:"",s={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
  ...
  `}function Se(e,a,t){const n=parseInt(e.dataset.matchId),s=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function l(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function c(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado",s.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(m=>{m.addEventListener("change",l),m.addEventListener("input",l)}),e.addEventListener("submit",async m=>{var b;m.preventDefault();const h=parseInt(e.querySelector("[name=predicted_home]").value),p=parseInt(e.querySelector("[name=predicted_away]").value),_=(b=e.querySelector("[name=predicted_result]:checked"))==null?void 0:b.value;if(!(isNaN(h)||isNaN(p)||!_)){s.disabled=!0,s.textContent="…";try{const{prediction:f}=await u.predictions.save({match_id:n,predicted_result:_,predicted_home:h,predicted_away:p,league_id:t??null});a[n]=f,v("Predicción guardada"),c()}catch(f){v(f.message||"Error al guardar","error"),s.disabled=!1,o?(s.className="btn btn--saved btn--sm pred-save-btn",s.textContent="✓ Guardado"):(s.className="btn btn--primary btn--sm pred-save-btn",s.textContent="Guardar")}}})}function xe(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ke(e){var a;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(y.isLoggedIn()){const{leagues:h}=await u.leagues.my();if(h.length===0){e.innerHTML=ae();return}}const t=xe(),[{ranking:n},s]=await Promise.all([u.auth.ranking(t),y.isLoggedIn()?u.leagues.my():Promise.resolve({leagues:[]})]),i=y.getUser(),o=s.leagues.find(h=>h.id===t),l=document.getElementById("tablonBadge"),c=l&&!l.classList.contains("hidden"),m=c?l.textContent:"";e.innerHTML=`
      ...
      <div class="container">
        ...
        <div class="ranking-table-wrapper">
          <table class="ranking-table">
            ...
            <tbody>
              ${n.map(h=>{var p,_;return`
                <tr class="${i&&h.id===i.id?"ranking-table__row--me":""}">
                  <td class="ranking-table__pos" data-pos="${h.position}">${h.position}</td>
                  <td>
                    <a class="ranking-table__link" href="#/jugador/${h.id}">
                      <span class="status-emoji" title="${((p=h.status)==null?void 0:p.name)||""}">${((_=h.status)==null?void 0:_.emoji)||""}</span>${h.username}
                    </a>
                  </td>
                  <td>${h.country||"—"}</td>
                  <td class="ranking-table__pts">${h.total_points}</td>
                  <td class="ranking-table__stat">${h.correct_results}</td>
                  <td class="ranking-table__stat">${h.exact_scores}</td>
                </tr>
              `}).join("")}
            </tbody>
          </table>
    ...
    `,(a=e.querySelector(".ranking-tablon-btn"))==null||a.addEventListener("click",()=>{E.navigate(`/tablon?liga=${t}`)})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}async function se(e,{query:a={}}={}){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const t=y.getUser();let n=a.liga?parseInt(a.liga):null;if(n){localStorage.setItem(`tablon_last_read_${n}`,new Date().toISOString());const r=document.getElementById("tablonBadge");r&&(r.classList.add("hidden"),r.textContent="")}let s=null,i=[],o=1,l=1;try{if(!n&&t){const{leagues:r}=await u.leagues.my();r&&r.length&&(n=r[0].id,s=r[0].name)}else if(n)try{const{league:r}=await u.leagues.get(n);s=r.name}catch{}if(n&&t)try{const{members:r}=await u.leagues.members(n);i=r||[]}catch{}}catch{}async function c(){const r=await u.board.messages(o,n);return l=r.pages||1,r}try{const r=await c();m(r)}catch(r){e.innerHTML=`<div class="container"><p class="form__error">Error: ${r.message}</p></div>`}function m(r){const{pinned:d=[],messages:$=[]}=r;e.innerHTML=`
      <div class="container">
    ...
    `,f(),w(),x()}function h(r){return r.length?r.map(d=>`
      <div class="board-message board-message--pinned" data-id="${d.id}">
        <div class="board-message__header">
          ...
          <strong class="board-message__author">${H(d.username)}</strong>
          <span class="board-message__date">${T(d.created_at)}</span>
    ...
    `}function _(r){return r.length?r.map(d=>`
    ...
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function b(r){const d=H(r);if(!i.length)return d;const $=i.map(L=>Ce(L.username)),S=new RegExp(`@(${$.join("|")})`,"gi");return d.replace(S,'<span class="mention">@$1</span>')}function f(){const r=document.getElementById("boardForm");if(!r)return;const d=document.getElementById("boardMsg"),$=document.getElementById("charCounter"),S=document.getElementById("mentionDropdown");d.addEventListener("input",()=>{$.textContent=`${d.value.length} / 500`,k(d,S)}),r.addEventListener("submit",async L=>{L.preventDefault();const M=d.value.trim();if(M)try{await u.board.post(M,n),d.value="",$.textContent="0 / 500",S.classList.add("hidden");const C=await c();I(C),v("Mensaje publicado")}catch(C){v(C.message,"error")}})}function w(){e.querySelectorAll(".reply-form").forEach(r=>{const d=parseInt(r.dataset.parent),$=r.querySelector(".reply-input"),S=`mentionDropdown-${d}`,L=document.getElementById(S);$==null||$.addEventListener("input",()=>{k($,L)}),r.addEventListener("submit",async M=>{M.preventDefault();const C=$.value.trim();if(C)try{await u.board.reply(d,C),$.value="",L==null||L.classList.add("hidden");const j=await c();I(j),v("Respuesta enviada")}catch(j){v(j.message,"error")}})}),e.querySelectorAll(".pin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await u.board.pin(r.dataset.id);const d=await c();I(d),v("Mensaje fijado")}catch(d){v(d.message,"error")}})}),e.querySelectorAll(".unpin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await u.board.pin(r.dataset.id);const d=await c();I(d),v("Mensaje desfijado")}catch(d){v(d.message,"error")}})})}function x(){e.querySelectorAll(".delete-msg").forEach(r=>{r.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await u.board.delete(r.dataset.id);const d=await c();I(d),v("Mensaje eliminado")}catch(d){v(d.message,"error")}})})}function I(r){const{pinned:d=[],messages:$=[]}=r,S=document.getElementById("boardPinned");if(S)S.innerHTML=h(d);else if(d.length){m(r);return}const L=document.getElementById("boardMessages");L&&(L.innerHTML=_($)),w(),x()}e.addEventListener("click",async r=>{if(r.target.id==="prevPage"&&o>1){o--;const d=await c();I(d)}else if(r.target.id==="nextPage"&&o<l){o++;const d=await c();I(d)}});function k(r,d){if(!d||!i.length)return;const $=r.value,S=r.selectionStart,L=$.slice(0,S),M=L.match(/@(\w*)$/);if(!M){d.classList.add("hidden");return}const C=M[1].toLowerCase(),j=i.filter(P=>P.username.toLowerCase().startsWith(C)&&P.id!==(t==null?void 0:t.id));if(!j.length){d.classList.add("hidden");return}d.innerHTML=j.slice(0,6).map(P=>`<div class="mention-item" data-username="${H(P.username)}">${H(P.username)}</div>`).join(""),d.classList.remove("hidden"),d.querySelectorAll(".mention-item").forEach(P=>{P.addEventListener("mousedown",re=>{re.preventDefault();const le=P.dataset.username,O=L.replace(/@(\w*)$/,`@${le} `);if(r.value=O+$.slice(S),r.setSelectionRange(O.length,O.length),d.classList.add("hidden"),r.tagName==="TEXTAREA"){const F=document.getElementById("charCounter");F&&(F.textContent=`${r.value.length} / 500`)}})})}document.addEventListener("click",r=>{!r.target.closest(".board-form__input-wrap")&&!r.target.closest(".reply-form__input-wrap")&&document.querySelectorAll(".mention-dropdown").forEach(d=>d.classList.add("hidden"))},{capture:!0})}function H(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Ce(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}async function Pe(e){var a,t,n,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const i=y.getUser(),o=i==null?void 0:i.is_admin,[l,c]=await Promise.all([o?u.leagues.adminAll():u.leagues.all(),y.isLoggedIn()&&!o?u.leagues.my():Promise.resolve({leagues:[]})]),m=new Set(c.leagues.map(p=>p.id)),h=o?l.leagues:l.leagues.filter(p=>!m.has(p.id));e.innerHTML=`
      <div class="container">
        ...
        ${i&&!o&&c.leagues.length>0?`
          <section class="section">
            <h2>Mis ligas</h2>
            <div class="leagues-grid">${c.leagues.map(p=>K(p,!0)).join("")}</div>
          </section>
        `:""}
        ...
        <section class="section">
          ...
          ${h.length?`<div class="leagues-grid">${h.map(p=>K(p,!1,m,o)).join("")}</div>`:o?'<p class="empty">No hay ligas creadas aún.</p>':c.leagues.length>0?'<p class="empty">Ya participas en todas las ligas disponibles.</p>':'<p class="empty">No hay ligas aún. ¡Crea la primera!</p>'}
        </section>
      </div>
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(p=>{p.addEventListener("click",()=>E.navigate(`/ligas/${p.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(p=>{p.addEventListener("click",async _=>{_.stopPropagation();const b=parseInt(p.dataset.id);p.disabled=!0,p.textContent="…";try{const{league:f}=await u.leagues.join({league_id:b});v(`¡Te has unido a "${f.name}"!`),E.navigate(`/ligas/${f.id}`)}catch(f){v(f.message,"error"),p.disabled=!1,p.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(p=>{p.addEventListener("click",_=>{_.stopPropagation(),v("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(a=document.getElementById("btnShowCreate"))==null||a.addEventListener("click",()=>{var p,_;(p=document.getElementById("createLeaguePanel"))==null||p.classList.remove("hidden"),(_=document.getElementById("btnShowCreate"))==null||_.classList.add("hidden")}),(t=document.getElementById("btnCancelCreate"))==null||t.addEventListener("click",()=>{var p,_;(p=document.getElementById("createLeaguePanel"))==null||p.classList.add("hidden"),(_=document.getElementById("btnShowCreate"))==null||_.classList.remove("hidden")}),(n=document.getElementById("joinCodeForm"))==null||n.addEventListener("submit",async p=>{p.preventDefault();const _=document.getElementById("inviteCode").value.trim().toUpperCase();if(_)try{const{league:b}=await u.leagues.join({invite_code:_});v(`Te has unido a "${b.name}"`),E.navigate(`/ligas/${b.id}`)}catch(b){v(b.message,"error")}}),(s=document.getElementById("createLeagueForm"))==null||s.addEventListener("submit",async p=>{var k;p.preventDefault();const _=document.getElementById("createBtn");_.disabled=!0,_.textContent="Creando…";const b=document.getElementById("leagueName").value.trim(),f=document.getElementById("leagueDesc").value.trim(),w=document.getElementById("leaguePrize").value.trim(),x=document.getElementById("isPublic").checked,I=((k=document.getElementById("isOfficial"))==null?void 0:k.checked)??!1;try{const{league:r}=await u.leagues.create({name:b,description:f,prize:w,is_public:x,is_official:I});Be(r)}catch(r){v(r.message,"error"),_.disabled=!1,_.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function K(e,a=!1,t=new Set,n=!1){const s=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=n?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:a?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),v("Enlace copiado")}catch{v("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function Te(e,{params:a}){var n,s,i,o,l;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const c=await u.leagues.get(t),{league:m,ranking:h,is_member:p,is_admin_view:_}=c,b=y.getUser(),f=m.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
        ...
        ${(p||b!=null&&b.is_admin)&&m.invite_link?`
        ...
        <div class="league-actions">
          ...
          ${b!=null&&b.is_admin||p&&b&&m.created_by===b.id?'<button class="btn btn--outline btn--sm" id="btnEditLeague">Editar liga</button>':""}
        </div>
    ...
    `,(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(m.invite_link),v("Enlace copiado")}catch{v("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${m.name} en PickGoal`,url:m.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await u.leagues.leave(t),v("Has abandonado la liga"),E.navigate("/ligas")}catch(r){v(r.message,"error")}}),(o=document.getElementById("btnJoin"))==null||o.addEventListener("click",async()=>{try{await u.leagues.join({league_id:t}),v("¡Te has unido a la liga!"),E.navigate(`/ligas/${t}`)}catch(r){v(r.message,"error")}}),(l=document.getElementById("btnEditLeague"))==null||l.addEventListener("click",()=>{Me(m,t,b)});const w=document.getElementById("tabRanking"),x=document.getElementById("tabTablon"),I=document.getElementById("sectionRanking"),k=document.getElementById("sectionTablon");w&&x&&(w.addEventListener("click",()=>{w.classList.add("league-tab--active"),x.classList.remove("league-tab--active"),I.classList.remove("hidden"),k.classList.add("hidden")}),x.addEventListener("click",()=>{x.classList.add("league-tab--active"),w.classList.remove("league-tab--active"),I.classList.add("hidden"),k.classList.remove("hidden");const r=document.getElementById("tablonEmbed");r&&!r.dataset.loaded&&(r.dataset.loaded="1",se(r,{query:{liga:String(t)}}))}))}catch(c){e.innerHTML=`<div class="container"><p class="form__error">Error: ${c.message}</p><a href="#/ligas">Volver</a></div>`}}function Me(e,a,t){const n=document.getElementById("editLeagueModal");n&&n.remove();const s=document.createElement("div");s.id="editLeagueModal",s.className="edit-league-modal",s.innerHTML=`
  ...
  `,document.body.appendChild(s),requestAnimationFrame(()=>s.classList.add("edit-league-modal--open"));const i=()=>{s.classList.remove("edit-league-modal--open"),s.addEventListener("transitionend",()=>s.remove(),{once:!0})};s.querySelector(".edit-league-modal__overlay").addEventListener("click",i),document.getElementById("btnCancelEdit").addEventListener("click",i),document.getElementById("editLeagueForm").addEventListener("submit",async o=>{o.preventDefault();const l=document.getElementById("btnSaveEdit");l.disabled=!0,l.textContent="Guardando…";const c={name:document.getElementById("editName").value.trim(),description:document.getElementById("editDesc").value.trim(),prize:document.getElementById("editPrize").value.trim(),is_public:document.getElementById("editPublic").checked};t!=null&&t.is_admin&&(c.is_official=document.getElementById("editOfficial").checked);try{await u.leagues.update(a,c),v("Liga actualizada"),i(),E.navigate(`/ligas/${a}`)}catch(m){v(m.message,"error"),l.disabled=!1,l.textContent="Guardar cambios"}})}async function je(e){var t,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=y.getUser();try{const s=(()=>{const f=localStorage.getItem("activeLeagueId");return f?parseInt(f):null})(),[i,o,l,c,m]=await Promise.all([u.predictions.mine(s),u.predictions.getChampion(s),u.leagues.my(),u.auth.me(),a!=null&&a.is_admin?u.leagues.adminAll():Promise.resolve({leagues:[]})]),h=i.predictions.reduce((f,w)=>f+w.total_points,0)+(((t=o.champion_prediction)==null?void 0:t.points_earned)||0),p=c.user,_=p.status,b=p.total_points_all_time;e.innerHTML=`
    ...
    `,(n=e.querySelector("#btnLogoutPerfil"))==null||n.addEventListener("click",()=>{y.logout(),window.location.hash="/"})}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error: ${s.message}</p></div>`}}function He(e,a){if(e.next_threshold===null)return`
  ...
  `}const W=new Date("2026-06-11T21:00:00Z"),Ae=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];function Ne(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function ne(e){var a;if(!y.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const t=Ne(),{champion_prediction:n}=await u.predictions.getChampion(t),s=new Date>=W;let i;n&&s?i=`
  ...
  `}async function De(e){if(!y.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{users:a}=await u.auth.users();e.innerHTML=`
      <div class="container">
    ...
    `,document.getElementById("btnSync").addEventListener("click",async()=>{const s=document.getElementById("syncResult");s.textContent="Sincronizando…";try{await u.matches.sync(),s.textContent="✓ Sincronización completada",v("Sincronización completada")}catch(i){s.textContent=`Error: ${i.message}`,v(i.message,"error")}}),document.getElementById("awardForm").addEventListener("submit",async s=>{s.preventDefault();const i=document.getElementById("winnerTeam").value.trim();if(i)try{const{message:o}=await u.predictions.awardChampion(i);v(o)}catch(o){v(o.message,"error")}});const t=document.getElementById("pushTarget"),n=document.getElementById("pushTargetIdGroup");t.addEventListener("change",()=>{n.classList.toggle("hidden",t.value==="all")}),document.getElementById("pushForm").addEventListener("submit",async s=>{s.preventDefault();const i=document.getElementById("pushTitle").value.trim()||"PickGoal",o=document.getElementById("pushBody").value.trim(),l=t.value,c=parseInt(document.getElementById("pushTargetId").value)||null,m=document.getElementById("pushResult"),h={title:i,body:o};l==="league"&&c&&(h.league_id=c),l==="user"&&c&&(h.user_id=c),m.textContent="Enviando…";try{const{sent:p}=await u.notifications.send(h);m.textContent=`✓ Enviada a ${p} suscripción(es)`,v(`Notificación enviada a ${p} suscripción(es)`)}catch(p){m.textContent=`Error: ${p.message}`,v(p.message,"error")}}),document.getElementById("usersTableBody").addEventListener("click",async s=>{const i=s.target.closest(".toggle-admin");if(!i)return;const o=parseInt(i.dataset.id);try{const{user:l}=await u.auth.toggleAdmin(o);i.closest("tr").querySelector(".admin-badge").textContent=l.is_admin?"Sí":"No",v(`${l.username} ${l.is_admin?"ahora es admin":"ya no es admin"}`)}catch(l){v(l.message,"error")}})}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}function Oe(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
  ...
  `,document.getElementById("resetForm").addEventListener("submit",async n=>{n.preventDefault();const s=document.getElementById("resetBtn"),i=document.getElementById("resetError"),o=document.getElementById("password").value;s.disabled=!0,s.textContent="Guardando…",i.classList.add("hidden");try{await u.auth.resetPassword(t,o),v("Contraseña actualizada. Ya puedes iniciar sesión."),E.navigate("/login")}catch(l){i.textContent=l.message||"Error al restablecer la contraseña",i.classList.remove("hidden")}finally{s.disabled=!1,s.textContent="Guardar contraseña"}})}const Ge={r32:"Dieciseisavos",r16:"Octavos",quarters:"Cuartos",semis:"Semis",third:"3º y 4º",final:"Final"};async function Fe(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{groups:a}=await u.matches.grouped();e.innerHTML=`
      <div class="container">
        <h1 class="page-title">Resultados — Mundial 2026</h1>
        <nav class="phase-nav" id="phaseNav"></nav>
        <div id="phaseContent"></div>
      </div>
    `,ze(a)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${a.message}</p></div>`}}function ze(e){var i;const a=document.getElementById("phaseNav");if(!a)return;const t=e.filter(o=>o.phase==="group"),n=e.filter(o=>o.phase!=="group"),s=[...t.map(o=>({key:`group_${o.group_name}`,label:`Grupo ${o.group_name}`,data:o,isGroup:!0})),...n.map(o=>({key:o.phase,label:Ge[o.phase]||o.label,data:o,isGroup:!1}))];s.length!==0&&(a.innerHTML=s.map((o,l)=>`
    <button class="phase-nav__btn ${l===0?"phase-nav__btn--active":""}" data-key="${o.key}">
      ${o.label}
    </button>
  `).join(""),(i=a.querySelector(".phase-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),a.querySelectorAll(".phase-nav__btn").forEach((o,l)=>{o.addEventListener("click",()=>{a.querySelectorAll(".phase-nav__btn").forEach(m=>m.classList.remove("phase-nav__btn--active")),o.classList.add("phase-nav__btn--active");const c=s.find(m=>m.key===o.dataset.key);c&&Y(c.data,c.isGroup)})}),Y(s[0].data,s[0].isGroup))}function Y(e,a){const t=document.getElementById("phaseContent");if(!t)return;const n=Je(e.matches);if(a){const s=Ve(e.matches);t.innerHTML=`
    ...
    `}else t.innerHTML=`<div class="resultados-matches">${n}</div>`}function Je(e){return!e||e.length===0?'<p class="empty">Sin partidos en esta fase.</p>':e.map(a=>{const t={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[a.status]||a.status,n=a.status!=="scheduled"?`<span class="res-score">${a.home_score_90??"?"} - ${a.away_score_90??"?"}</span>`:'<span class="res-score res-score--pending">vs</span>';return`
    ...
    `}).join("")}function Ve(e){const a={};for(const t of e)if(a[t.home_team]||(a[t.home_team]={name:t.home_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),a[t.away_team]||(a[t.away_team]={name:t.away_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),t.status==="finished"&&t.home_score_90!==null&&t.away_score_90!==null){const n=a[t.home_team],s=a[t.away_team];n.pj++,s.pj++,n.gf+=t.home_score_90,n.gc+=t.away_score_90,s.gf+=t.away_score_90,s.gc+=t.home_score_90,t.home_score_90>t.away_score_90?(n.g++,n.pts+=3,s.p++):t.home_score_90<t.away_score_90?(s.g++,s.pts+=3,n.p++):(n.e++,n.pts++,s.e++,s.pts++)}return Object.values(a).sort((t,n)=>{if(n.pts!==t.pts)return n.pts-t.pts;const s=n.gf-n.gc,i=t.gf-t.gc;return s!==i?s-i:n.gf-t.gf})}function Xe(e){return e.length===0?'<p class="empty">Sin datos de clasificación.</p>':`
  ...
  `}async function Ke(e,{query:a}){const t=(a.codigo||"").trim().toUpperCase();if(!t){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!y.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",t),E.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:n}=await u.leagues.joinByCode(t);v(`¡Te has unido a "${n.name}"!`),E.navigate(`/ligas/${n.id}`)}catch(n){if(n.status===409){v("Ya eres miembro de esta liga");try{const{leagues:s}=await u.leagues.my(),i=s.find(o=>o.invite_code===t);if(i){E.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
    ...
    `}}function We(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Ye(e,{params:a}){const t=parseInt(a.id);if(!t){e.innerHTML='<div class="container"><p class="form__error">Usuario no válido.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const n=We(),{user:s,predictions:i}=await u.predictions.forUser(t,n);e.innerHTML=`
      <div class="container">
  ...
  `}const Ze={"/":ue,"/login":fe,"/register":ye,"/quiniela":$e,"/resultados":Fe,"/ranking":ke,"/tablon":se,"/ligas":Pe,"/ligas/:id":Te,"/perfil":je,"/campeon":ne,"/admin":De,"/forgot-password":Re,"/reset-password":Ue,"/unirse":Ke,"/jugador/:id":Ye};function ea(e){for(const[a,t]of Object.entries(Ze)){const n=[],s=new RegExp("^"+a.replace(/:([^/]+)/g,(o,l)=>(n.push(l),"([^/]+)"))+"$"),i=e.match(s);if(i){const o={};return n.forEach((l,c)=>{o[l]=i[c+1]}),{handler:t,params:o}}}return null}const Q=()=>document.getElementById("mainContent"),E={init(){window.addEventListener("hashchange",()=>this.resolve()),this.resolve()},navigate(e){window.location.hash=e},resolve(){const e=window.location.hash.slice(1)||"/",[a,t]=e.split("?"),n=Object.fromEntries(new URLSearchParams(t||"")),s=ea(a);if(!s){Q().innerHTML='<div class="error-page"><h2>Página no encontrada</h2><a href="#/">Volver al inicio</a></div>';return}const{handler:i,params:o}=s;if(["/perfil","/campeon","/admin"].includes(a)&&!y.isLoggedIn()){this.navigate("/login");return}if(a==="/admin"&&!y.isAdmin()){this.navigate("/");return}const c=Q();c.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>',i(c,{params:o,query:n})}};let D=[],N=null,q=null;async function aa(){await y.init(),E.init(),na(),ta(),ra()}function ie(){return localStorage.getItem("pwa_installed")==="true"||window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===!0}function ta(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),!ie()&&(N=e,sa())}),window.addEventListener("appinstalled",()=>{var e;localStorage.setItem("pwa_installed","true"),N=null,(e=document.getElementById("installBanner"))==null||e.remove()})}function sa(){if(ie()||sessionStorage.getItem("installBannerDismissed"))return;const e=document.createElement("div");e.id="installBanner",e.className="install-banner",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById("installBtn").addEventListener("click",async()=>{N&&(N.prompt(),await N.userChoice,N=null,e.remove())}),document.getElementById("installDismissBtn").addEventListener("click",()=>{sessionStorage.setItem("installBannerDismissed","1"),e.remove()})}function A(){var e,a;(e=document.getElementById("userDropdown"))==null||e.classList.add("hidden"),(a=document.getElementById("userBtn"))==null||a.classList.remove("navbar__dropdown-btn--open")}async function G(){const e=document.getElementById("tablonBadge");if(!e)return;if(!y.getUser()){e.classList.add("hidden");return}const t=localStorage.getItem("activeLeagueId");if(!t){e.classList.add("hidden");return}const n=localStorage.getItem(`tablon_last_read_${t}`)||new Date(0).toISOString();try{const{count:s}=await u.board.unread(parseInt(t),n);s>0?(e.textContent=s>99?"99+":String(s),e.classList.remove("hidden")):e.classList.add("hidden")}catch{e.classList.add("hidden")}}function na(){var e,a,t;document.addEventListener("auth:change",Z),window.addEventListener("hashchange",()=>{A(),oe(),setTimeout(G,200)}),document.addEventListener("click",A),(e=document.getElementById("userBtn"))==null||e.addEventListener("click",n=>{var o;n.stopPropagation();const s=document.getElementById("userDropdown"),i=s==null?void 0:s.classList.contains("hidden");A(),i&&(s==null||s.classList.remove("hidden"),(o=document.getElementById("userBtn"))==null||o.classList.add("navbar__dropdown-btn--open"))}),(a=document.getElementById("userDropdown"))==null||a.addEventListener("click",n=>{n.stopPropagation(),n.target.closest("#navProfileLink")&&A()}),(t=document.getElementById("navLogoutBtn"))==null||t.addEventListener("click",()=>{D=[],localStorage.removeItem("activeLeagueId"),A(),y.logout(),E.navigate("/")}),Z()}async function Z(){const e=document.getElementById("navAuthLinks"),a=document.getElementById("userBtn"),t=document.getElementById("navUsername"),n=document.getElementById("bottomNav"),s=y.getUser();if(A(),s){e==null||e.classList.add("hidden"),t&&(t.textContent=s.username),a.style.visibility="visible",n==null||n.classList.remove("hidden"),document.body.classList.add("has-bottom-nav");try{const{leagues:i}=s.is_admin?await u.leagues.adminAll():await u.leagues.my();D=i}catch{D=[]}ia(D),G(),q&&clearInterval(q),q=setInterval(G,5*60*1e3)}else e==null||e.classList.remove("hidden"),a.style.visibility="hidden",n==null||n.classList.add("hidden"),document.body.classList.remove("has-bottom-nav"),D=[],localStorage.removeItem("activeLeagueId"),q&&(clearInterval(q),q=null);oe()}function ia(e){const a=localStorage.getItem("activeLeagueId");a&&e.some(n=>String(n.id)===String(a))||(e.length>0?localStorage.setItem("activeLeagueId",String(e[0].id)):localStorage.removeItem("activeLeagueId"))}function oe(){const e=window.location.hash.slice(1).split("?")[0]||"/";document.querySelectorAll(".bottom-nav__item").forEach(a=>{const t=a.dataset.route,n=t==="/"?e==="/":e===t||e.startsWith(t+"/");a.classList.toggle("bottom-nav__item--active",n)})}function oa(e){const a="=".repeat((4-e.length%4)%4),t=(e+a).replace(/-/g,"+").replace(/_/g,"/"),n=atob(t);return Uint8Array.from([...n].map(s=>s.charCodeAt(0)))}async function ra(){if(!(!("serviceWorker"in navigator)||!("PushManager"in window)))try{const e=await navigator.serviceWorker.register("/sw.js");document.addEventListener("auth:change",async a=>{a.detail&&await ee(e)}),y.getUser()&&await ee(e)}catch{}}async function ee(e){try{if(await Notification.requestPermission()!=="granted")return;const t=await e.pushManager.getSubscription();if(t){await u.notifications.subscribe(t.toJSON());return}const{public_key:n}=await u.notifications.vapidPublicKey();if(!n)return;const s=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:oa(n)});await u.notifications.subscribe(s.toJSON())}catch{}}aa();
