(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const Pe="https://pickgoal-backend.onrender.com/api";function Te(){return localStorage.getItem("token")}async function v(e,a={}){const t={"Content-Type":"application/json",...a.headers},s=Te();s&&(t.Authorization=`Bearer ${s}`);const n=await fetch(`${Pe}${e}`,{...a,headers:t}),i=await n.json().catch(()=>({}));if(!n.ok)throw{status:n.status,message:i.error||"Error desconocido"};return i}const u={get:e=>v(e),post:(e,a)=>v(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>v(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>v(e,{method:"DELETE"}),auth:{register:e=>v("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>v("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>v("/auth/me"),forgotPassword:e=>v("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>v("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:e=>v(`/auth/ranking${e?`?league_id=${e}`:""}`),users:()=>v("/auth/users"),toggleAdmin:e=>v(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>v("/matches/grouped"),list:(e="")=>v(`/matches/${e}`),get:e=>v(`/matches/${e}`),today:()=>v("/matches/today"),setResult:(e,a,t,s=null)=>v(`/matches/${e}/result`,{method:"PATCH",body:JSON.stringify({home_score:a,away_score:t,...s?{result_90:s}:{}})}),sync:()=>v("/matches/sync",{method:"POST"}),recalculate:()=>v("/matches/recalculate",{method:"POST"})},predictions:{mine:e=>v(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>v(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>v("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>v(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>v(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>v("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>v("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>v("/leagues/all"),public:()=>v("/leagues/public"),my:()=>v("/leagues/my"),create:e=>v("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>v("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>v(`/leagues/join/${encodeURIComponent(e)}`),adminAll:()=>v("/leagues/admin"),get:e=>v(`/leagues/${e}`),update:(e,a)=>v(`/leagues/${e}`,{method:"PUT",body:JSON.stringify(a)}),leave:e=>v(`/leagues/${e}/leave`,{method:"DELETE"}),members:e=>v(`/leagues/${e}/members`),matchPredictions:(e,a)=>v(`/leagues/${e}/predictions/${a}`)},home:{summary:()=>v("/home/summary")},board:{messages:(e=1,a=null)=>v(`/board/?page=${e}${a?`&league_id=${a}`:""}`),unread:(e,a)=>v(`/board/unread?league_id=${e}&since=${encodeURIComponent(a)}`),post:(e,a=null)=>v("/board/",{method:"POST",body:JSON.stringify({message:e,league_id:a})}),pin:e=>v(`/board/${e}/pin`,{method:"POST"}),reply:(e,a)=>v(`/board/${e}/reply`,{method:"POST",body:JSON.stringify({message:a})}),delete:e=>v(`/board/${e}`,{method:"DELETE"})},notifications:{vapidPublicKey:()=>v("/notifications/vapid-public-key"),subscribe:e=>v("/notifications/subscribe",{method:"POST",body:JSON.stringify(e)}),send:e=>v("/notifications/send",{method:"POST",body:JSON.stringify(e)})},adminV2:{partidos:e=>v(`/v2/admin/partidos-disponibles?semana=${encodeURIComponent(e)}`),jornadas:()=>v("/v2/admin/jornadas"),createJornada:e=>v("/v2/admin/jornada",{method:"POST",body:JSON.stringify(e)}),updateJornada:(e,a)=>v(`/v2/admin/jornada/${e}`,{method:"PUT",body:JSON.stringify(a)}),deleteJornada:e=>v(`/v2/admin/jornada/${e}`,{method:"DELETE"})},jornada:{current:()=>v("/v2/jornada/current"),predict:e=>v("/v2/jornada/predict",{method:"POST",body:JSON.stringify({predictions:e})}),history:()=>v("/v2/jornada/history")},duelo:{current:()=>v("/v2/duelo/current")},clasificacion:{division:e=>v(`/v2/clasificacion/division${e?`?league_id=${e}`:""}`),general:()=>v("/v2/clasificacion/general")}};let D=null;const $={async init(){if(localStorage.getItem("token"))try{const{user:a}=await u.auth.me();D=a}catch{localStorage.removeItem("token")}},setUser(e,a){D=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){D=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return D},isLoggedIn(){return!!D},isAdmin(){return(D==null?void 0:D.is_admin)===!0}};let W=null;function p(e,a="success"){let t=document.getElementById("toast");t||(t=document.createElement("div"),t.id="toast",document.body.appendChild(t)),t.textContent=e,t.className=`toast toast--${a} toast--visible`,W&&clearTimeout(W),W=setTimeout(()=>{t.classList.remove("toast--visible")},3e3)}function ve(){return`
  ...
  `}function be(e){const a=e.querySelector("#pointsModal"),t=e.querySelector("#btnPointsInfo"),s=e.querySelector("#pointsClose"),n=e.querySelector("#pointsOverlay");function i(){a.classList.add("points-modal--open"),document.body.style.overflow="hidden"}function o(){a.classList.remove("points-modal--open"),document.body.style.overflow=""}return t==null||t.addEventListener("click",i),s==null||s.addEventListener("click",o),n==null||n.addEventListener("click",o),document.addEventListener("keydown",d=>{d.key==="Escape"&&o()},{once:!1}),i}function P(e){return e?new Date(e).toLocaleString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}async function Me(e){if(!$.getUser()){De(e);return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{leagues_summary:t,upcoming_matches:s}=await u.home.summary();if(t.length===0){He(e);return}const n=(()=>{const o=localStorage.getItem("activeLeagueId");return o?parseInt(o):null})(),i=[...t].sort((o,d)=>o.league_id===n?-1:d.league_id===n?1:0);e.innerHTML=`
  ...
  `,ye(e)}function qe(e){const a=new Date,t=new Date(e),s=Math.ceil((t-a)/(1e3*60*60*24));return Math.max(0,s)}function fe(){const e=qe("2026-08-15");return`
  ...
  `}function ye(e){var a;(a=e.querySelector("#btnWaitlist"))==null||a.addEventListener("click",()=>{p("¡Ya estás dentro! Te avisaremos cuando empiece la temporada 🎉")})}function Ae(e){const a=e.next_to_predict?`<div class="league-card__next">
  ...
  `:""}const ne="pickgoal_welcome_shown";function $e(e="/ligas"){if(localStorage.getItem(ne))return;localStorage.setItem(ne,"1");const a=document.createElement("div");a.innerHTML=`
  ...
  `;const t=a.firstElementChild;document.body.appendChild(t),document.body.style.overflow="hidden",requestAnimationFrame(()=>t.classList.add("welcome-modal--open"));function s(n){t.classList.remove("welcome-modal--open"),document.body.style.overflow="",t.addEventListener("transitionend",()=>t.remove(),{once:!0}),n&&(window.location.hash=n)}document.getElementById("welcomeOverlay").addEventListener("click",()=>s()),document.getElementById("welcomeCta").addEventListener("click",()=>s(e)),document.addEventListener("keydown",function n(i){i.key==="Escape"&&(s(),document.removeEventListener("keydown",n))})}function Oe(e){e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),s=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",s.classList.add("hidden");const n={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:o}=await u.auth.register(n);$.setUser(o,i),p("¡Cuenta creada! Bienvenido a PickGoal");const d=sessionStorage.getItem("pendingInviteCode");if(d){sessionStorage.removeItem("pendingInviteCode");try{const{league:g}=await u.leagues.joinByCode(d);p(`¡Te has unido a "${g.name}"!`),S.navigate(`/ligas/${g.id}`)}catch{S.navigate("/ligas")}}else S.navigate("/campeon"),$e("/ligas")}catch(i){s.textContent=i.message||"Error al registrarse",s.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}const Ge=new Set(["r32","r16","quarters","semis","third","final"]);let Q=null;function ie(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Ue(e){var a,t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{let s=null;if($.isLoggedIn()){const{leagues:y}=await u.leagues.my();if(y.length===0){e.innerHTML=ve();return}const L=ie(),w=y.find(r=>r.id===L);s=w?w.name:((a=y[0])==null?void 0:a.name)??null}const n=ie(),[{groups:i},o,d]=await Promise.all([u.matches.grouped(),$.isLoggedIn()?u.predictions.mine(n):Promise.resolve({predictions:[]}),$.isLoggedIn()?u.predictions.getChampion(n):Promise.resolve({champion_prediction:null})]),g={};for(const y of o.predictions)g[y.match_id]=y;const m=i.flatMap(y=>y.matches),h=new Map;for(const y of m){const L=re(y.match_datetime);h.has(L)||h.set(L,[]),h.get(L).push(y)}const l=[...h.keys()].sort(),_=re(new Date().toISOString()),b=l.find(y=>y>=_)??l[0],f=((t=d.champion_prediction)==null?void 0:t.team_name)??null,E=$.isLoggedIn()?f?`<p class="champion-banner champion-banner--set">🏆 Tu campeón: <a href="#/campeon" style="color:inherit;font-weight:bold;">${f}</a></p>`:'<p class="champion-banner champion-banner--missing">⚠️ <a href="#/campeon">¡Elige tu campeón antes del inicio del torneo!</a></p>':"";e.innerHTML=`
      ...
      ${he()}
    `,Q=be(e),Fe(l,b,h,g,n)}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${s.message}</p></div>`}}function Fe(e,a,t,s,n){var o;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(d=>`
    <button class="date-nav__btn ${d===a?"date-nav__btn--active":""}" data-day="${d}">
      ${Je(d)}
    </button>
  `).join(""),(o=i.querySelector(".date-nav__btn--active"))==null||o.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),i.querySelectorAll(".date-nav__btn").forEach(d=>{d.addEventListener("click",()=>{i.querySelectorAll(".date-nav__btn").forEach(g=>g.classList.remove("date-nav__btn--active")),d.classList.add("date-nav__btn--active"),oe(t.get(d.dataset.day)??[],s,n)})}),oe(t.get(a)??[],s,n))}function oe(e,a,t){const s=document.getElementById("matchesContent");if(s){if(e.length===0){s.innerHTML='<p class="empty">Sin partidos este día.</p>';return}s.innerHTML=`<div class="matches-grid">${e.map(n=>ze(n,a[n.id])).join("")}</div>`,Q&&s.querySelectorAll(".knockout-info-btn").forEach(n=>{n.addEventListener("click",i=>{i.stopPropagation(),Q()})}),$.isLoggedIn()&&s.querySelectorAll(".prediction-form").forEach(n=>{We(n,a,t)})}}function re(e){const a=new Date(e);return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`}function Je(e){const[a,t,s]=e.split("-").map(Number);return new Date(a,t-1,s).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function ze(e,a){const t=e.is_locked,s=a?`<span class="pts-badge">${a.total_points} pts</span>`:"",n={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
    <div class="match-card ${t?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!t&&$.isLoggedIn()?Ve(e,a):t&&a?`<div class="prediction-result">
               Tu predicción: <strong>${a.predicted_home}-${a.predicted_away}</strong>
               (${a.predicted_result}) · ${a.total_points} pts
             </div>`:""}
    </div>
  `}function Ve(e,a){const t=!!a,s=(a==null?void 0:a.predicted_home)??0,n=(a==null?void 0:a.predicted_away)??0,i=(a==null?void 0:a.predicted_result)??"X",o=t?"prediction-form--saved":"prediction-form--unsaved",d=t?'<span class="pred-status pred-status--saved">✓ Guardado</span>':'<span class="pred-status pred-status--unsaved">Sin predicción</span>',g=t?"btn btn--saved btn--sm pred-save-btn":"btn btn--ghost btn--sm pred-save-btn",m=t?"✓ Guardado":"Guardar",h=Ge.has(e.phase);return`
    <form class="prediction-form ${o}" data-match-id="${e.id}" data-saved="${t}" data-is-knockout="${h?"1":"0"}">
      ...
      <button type="submit" class="${g}">${m}</button>
    </form>
  `}function Xe(e,a,t,s){return e==="1"?a>t:e==="2"?t>a:e==="X"?s?a!==t:a===t:!0}function We(e,a,t){const s=parseInt(e.dataset.matchId),n=e.querySelector(".pred-save-btn"),i=e.querySelector(".pred-status");let o=e.dataset.saved==="true";function d(){e.classList.contains("prediction-form--dirty")||(e.classList.remove("prediction-form--saved","prediction-form--unsaved"),e.classList.add("prediction-form--dirty"),n.className="btn btn--primary btn--sm pred-save-btn",n.textContent="Guardar",i&&(i.className="pred-status pred-status--unsaved",i.textContent="Sin guardar"))}function g(){o=!0,e.classList.remove("prediction-form--unsaved","prediction-form--dirty"),e.classList.add("prediction-form--saved"),n.className="btn btn--saved btn--sm pred-save-btn",n.textContent="✓ Guardado",n.disabled=!1,i&&(i.className="pred-status pred-status--saved",i.textContent="✓ Guardado")}e.querySelectorAll("input").forEach(m=>{m.addEventListener("change",d),m.addEventListener("input",d)}),e.addEventListener("submit",async m=>{var f;m.preventDefault();const h=parseInt(e.querySelector("[name=predicted_home]").value),l=parseInt(e.querySelector("[name=predicted_away]").value),_=(f=e.querySelector("[name=predicted_result]:checked"))==null?void 0:f.value;if(isNaN(h)||isNaN(l)||!_)return;const b=e.dataset.isKnockout==="1";if(!Xe(_,h,l,b)){p("El marcador no coincide con el resultado 1X2 seleccionado","error");return}n.disabled=!0,n.textContent="…";try{const{prediction:E}=await u.predictions.save({match_id:s,predicted_result:_,predicted_home:h,predicted_away:l,league_id:t??null});a[s]=E,p("Predicción guardada"),g()}catch(E){p(E.message||"Error al guardar","error"),n.disabled=!1,o?(n.className="btn btn--saved btn--sm pred-save-btn",n.textContent="✓ Guardado"):(n.className="btn btn--primary btn--sm pred-save-btn",n.textContent="Guardar")}})}function Ke(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Ye(e){var a,t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if($.isLoggedIn()){const{leagues:_}=await u.leagues.my();if(_.length===0){e.innerHTML=ve();return}}const s=Ke(),[{ranking:n},i]=await Promise.all([u.auth.ranking(s),$.isLoggedIn()?u.leagues.my():Promise.resolve({leagues:[]})]),o=$.getUser(),d=i.leagues.find(_=>_.id===s),g=document.getElementById("tablonBadge"),m=g&&!g.classList.contains("hidden"),h=m?g.textContent:"",l=((a=n[0])==null?void 0:a.matches_played)??0;e.innerHTML=`
      ...
      <div class="container">
        ...
        <div class="ranking-table-wrapper">
          <table class="ranking-table">
            ...
            <tbody>
              ${n.map(_=>{var w,r,c;const b=_.predictions_made??0,f=`${b}/${l}`,E=`${_.correct_results??0}/${b}`,y=`${_.exact_scores??0}/${b}`;return`
                  <tr class="${o&&_.id===o.id?"ranking-table__row--me":""}">
                    ...
                    <td>
                      <a class="ranking-table__link" href="#/jugador/${_.id}">
                        <span class="status-emoji" title="${((w=_.status)==null?void 0:w.name)||""}">${((r=_.status)==null?void 0:r.emoji)||""}</span>${_.username}
                      </a>
                    </td>
    ...
    `,(t=e.querySelector(".ranking-tablon-btn"))==null||t.addEventListener("click",()=>{S.navigate(`/tablon?liga=${s}`)})}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error: ${s.message}</p></div>`}}async function Ee(e,{query:a={}}={}){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const t=$.getUser();let s=a.liga?parseInt(a.liga):null;if(s){localStorage.setItem(`tablon_last_read_${s}`,new Date().toISOString());const r=document.getElementById("tablonBadge");r&&(r.classList.add("hidden"),r.textContent="")}let n=null,i=[],o=1,d=1;try{if(!s&&t){const{leagues:r}=await u.leagues.my();r&&r.length&&(s=r[0].id,n=r[0].name)}else if(s)try{const{league:r}=await u.leagues.get(s);n=r.name}catch{}if(s&&t)try{const{members:r}=await u.leagues.members(s);i=r||[]}catch{}}catch{}async function g(){const r=await u.board.messages(o,s);return d=r.pages||1,r}try{const r=await g();m(r)}catch(r){e.innerHTML=`<div class="container"><p class="form__error">Error: ${r.message}</p></div>`}function m(r){const{pinned:c=[],messages:I=[]}=r;e.innerHTML=`
      <div class="container">
    ...
    `,f(),E(),y()}function h(r){return r.length?r.map(c=>`
      <div class="board-message board-message--pinned" data-id="${c.id}">
        <div class="board-message__header">
          ...
          <strong class="board-message__author">${M(c.username)}</strong>
          <span class="board-message__date">${P(c.created_at)}</span>
    ...
    `).join(""):""}function l(r){return`
      <div class="board-reply ${r.is_deleted?"board-reply--deleted":""}" data-id="${r.id}">
        <div class="board-reply__header">
          ...
          <span class="board-reply__date">${P(r.created_at)}</span>
    ...
    `).join(""):'<p class="empty">Sin mensajes aún. ¡Sé el primero!</p>'}function b(r){const c=M(r);if(!i.length)return c;const I=i.map(j=>Qe(j.username)),x=new RegExp(`@(${I.join("|")})`,"gi");return c.replace(x,'<span class="mention">@$1</span>')}function f(){const r=document.getElementById("boardForm");if(!r)return;const c=document.getElementById("boardMsg"),I=document.getElementById("charCounter"),x=document.getElementById("mentionDropdown");c.addEventListener("input",()=>{I.textContent=`${c.value.length} / 500`,w(c,x)}),r.addEventListener("submit",async j=>{j.preventDefault();const H=c.value.trim();if(H)try{await u.board.post(H,s),c.value="",I.textContent="0 / 500",x.classList.add("hidden");const T=await g();L(T),p("Mensaje publicado")}catch(T){p(T.message,"error")}})}function E(){e.querySelectorAll(".reply-form").forEach(r=>{const c=parseInt(r.dataset.parent),I=r.querySelector(".reply-input"),x=`mentionDropdown-${c}`,j=document.getElementById(x);I==null||I.addEventListener("input",()=>{w(I,j)}),r.addEventListener("submit",async H=>{H.preventDefault();const T=I.value.trim();if(T)try{await u.board.reply(c,T),I.value="",j==null||j.classList.add("hidden");const U=await g();L(U),p("Respuesta enviada")}catch(U){p(U.message,"error")}})}),e.querySelectorAll(".pin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await u.board.pin(r.dataset.id);const c=await g();L(c),p("Mensaje fijado")}catch(c){p(c.message,"error")}})}),e.querySelectorAll(".unpin-msg").forEach(r=>{r.addEventListener("click",async()=>{try{await u.board.pin(r.dataset.id);const c=await g();L(c),p("Mensaje desfijado")}catch(c){p(c.message,"error")}})})}function y(){e.querySelectorAll(".delete-msg").forEach(r=>{r.addEventListener("click",async()=>{if(confirm("¿Eliminar este mensaje?"))try{await u.board.delete(r.dataset.id);const c=await g();L(c),p("Mensaje eliminado")}catch(c){p(c.message,"error")}})})}function L(r){const{pinned:c=[],messages:I=[]}=r,x=document.getElementById("boardPinned");if(x)x.innerHTML=h(c);else if(c.length){m(r);return}const j=document.getElementById("boardMessages");j&&(j.innerHTML=_(I)),E(),y()}e.addEventListener("click",async r=>{if(r.target.id==="prevPage"&&o>1){o--;const c=await g();L(c)}else if(r.target.id==="nextPage"&&o<d){o++;const c=await g();L(c)}});function w(r,c){if(!c||!i.length)return;const I=r.value,x=r.selectionStart,j=I.slice(0,x),H=j.match(/@(\w*)$/);if(!H){c.classList.add("hidden");return}const T=H[1].toLowerCase(),U=i.filter(B=>B.username.toLowerCase().startsWith(T)&&B.id!==(t==null?void 0:t.id)),te=[...$.isAdmin()&&"todos".startsWith(T)?[{username:"todos",description:"Notificar a todos los miembros"}]:[],...U.slice(0,6)];if(!te.length){c.classList.add("hidden");return}c.innerHTML=te.map(B=>B.description?`<div class="mention-item mention-item--broadcast" data-username="${M(B.username)}">
             <span class="mention-item__name">@${M(B.username)}</span>
             <span class="mention-item__desc">${M(B.description)}</span>
           </div>`:`<div class="mention-item" data-username="${M(B.username)}">${M(B.username)}</div>`).join(""),c.classList.remove("hidden"),c.querySelectorAll(".mention-item").forEach(B=>{B.addEventListener("mousedown",Be=>{Be.preventDefault();const Ce=B.dataset.username,X=j.replace(/@(\w*)$/,`@${Ce} `);if(r.value=X+I.slice(x),r.setSelectionRange(X.length,X.length),c.classList.add("hidden"),r.tagName==="TEXTAREA"){const se=document.getElementById("charCounter");se&&(se.textContent=`${r.value.length} / 500`)}})})}document.addEventListener("click",r=>{!r.target.closest(".board-form__input-wrap")&&!r.target.closest(".reply-form__input-wrap")&&document.querySelectorAll(".mention-dropdown").forEach(c=>c.classList.add("hidden"))},{capture:!0})}function M(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Qe(e){return e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}async function Ze(e){var a,t,s,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const i=$.getUser(),o=i==null?void 0:i.is_admin,[d,g]=await Promise.all([o?u.leagues.adminAll():u.leagues.all(),$.isLoggedIn()&&!o?u.leagues.my():Promise.resolve({leagues:[]})]),m=new Set(g.leagues.map(l=>l.id)),h=o?d.leagues:d.leagues.filter(l=>!m.has(l.id));e.innerHTML=`
      <div class="container">
        ...
        ${i&&!o&&g.leagues.length>0?`
          <section class="section">
            <h2>Mis ligas</h2>
            <div class="leagues-grid">${g.leagues.map(l=>de(l,!0)).join("")}</div>
          </section>
        `:""}
        ...
        <section class="section">
          ...
          ${h.length?`<div class="leagues-grid">${h.map(l=>de(l,!1,m,o)).join("")}</div>`:o?'<p class="empty">No hay ligas creadas aún.</p>':g.leagues.length>0?'<p class="empty">Ya participas en todas las ligas disponibles.</p>':'<p class="empty">No hay ligas aún. ¡Crea la primera!</p>'}
        </section>
      </div>
    `,e.querySelectorAll('.league-card[data-navigate="true"]').forEach(l=>{l.addEventListener("click",()=>S.navigate(`/ligas/${l.dataset.id}`))}),e.querySelectorAll(".btn-join-league").forEach(l=>{l.addEventListener("click",async _=>{_.stopPropagation();const b=parseInt(l.dataset.id);l.disabled=!0,l.textContent="…";try{const{league:f}=await u.leagues.join({league_id:b});p(`¡Te has unido a "${f.name}"!`),S.navigate(`/ligas/${f.id}`)}catch(f){p(f.message,"error"),l.disabled=!1,l.textContent="Unirse"}})}),e.querySelectorAll(".btn-private-info").forEach(l=>{l.addEventListener("click",_=>{_.stopPropagation(),p("Esta liga es privada. Pide el enlace de invitación a cualquier miembro para unirte.","info")})}),(a=document.getElementById("btnShowCreate"))==null||a.addEventListener("click",()=>{var l,_;(l=document.getElementById("createLeaguePanel"))==null||l.classList.remove("hidden"),(_=document.getElementById("btnShowCreate"))==null||_.classList.add("hidden")}),(t=document.getElementById("btnCancelCreate"))==null||t.addEventListener("click",()=>{var l,_;(l=document.getElementById("createLeaguePanel"))==null||l.classList.add("hidden"),(_=document.getElementById("btnShowCreate"))==null||_.classList.remove("hidden")}),(s=document.getElementById("joinCodeForm"))==null||s.addEventListener("submit",async l=>{l.preventDefault();const _=document.getElementById("inviteCode").value.trim().toUpperCase();if(_)try{const{league:b}=await u.leagues.join({invite_code:_});p(`Te has unido a "${b.name}"`),S.navigate(`/ligas/${b.id}`)}catch(b){p(b.message,"error")}}),(n=document.getElementById("createLeagueForm"))==null||n.addEventListener("submit",async l=>{var w;l.preventDefault();const _=document.getElementById("createBtn");_.disabled=!0,_.textContent="Creando…";const b=document.getElementById("leagueName").value.trim(),f=document.getElementById("leagueDesc").value.trim(),E=document.getElementById("leaguePrize").value.trim(),y=document.getElementById("isPublic").checked,L=((w=document.getElementById("isOfficial"))==null?void 0:w.checked)??!1;try{const{league:r}=await u.leagues.create({name:b,description:f,prize:E,is_public:y,is_official:L});ea(r)}catch(r){p(r.message,"error"),_.disabled=!1,_.textContent="Crear liga"}})}catch(i){e.innerHTML=`<div class="container"><p class="form__error">Error: ${i.message}</p></div>`}}function de(e,a=!1,t=new Set,s=!1){const n=e.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"",i=e.is_public?"🌍":"🔒",o=s?`<button class="btn btn--sm btn--outline btn-admin-view" data-id="${e.id}">Ver (admin)</button>`:a?`<button class="btn btn--sm btn--outline" onclick="event.stopPropagation(); window.location.hash='/ligas/${e.id}'">Ver liga</button>`:e.is_public?`<button class="btn btn--sm btn--primary btn-join-league" data-id="${e.id}">Unirse</button>`:'<button class="btn btn--sm btn--ghost btn-private-info">🔒 Solicitar enlace</button>';return`
  ...
  `}function ea(e){var s,n;const a=e.invite_link||"",t=document.getElementById("createLeaguePanel");t&&(t.innerHTML=`
  ...
  `,(s=document.getElementById("btnCopyLink"))==null||s.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),p("Enlace copiado")}catch{p("No se pudo copiar","error")}}),(n=document.getElementById("btnShare"))==null||n.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function aa(e,{params:a}){var s,n,i,o,d;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const g=await u.leagues.get(t),{league:m,ranking:h,is_member:l,is_admin_view:_}=g,b=$.getUser(),f=m.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
        ...
        ${(l||b!=null&&b.is_admin)&&m.invite_link?`
        ...
        <div class="league-actions">
          ...
          ${b!=null&&b.is_admin||l&&b&&m.created_by===b.id?'<button class="btn btn--outline btn--sm" id="btnEditLeague">Editar liga</button>':""}
        </div>
    ...
    `,(s=document.getElementById("btnCopyInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(m.invite_link),p("Enlace copiado")}catch{p("No se pudo copiar","error")}}),(n=document.getElementById("btnShareInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${m.name} en PickGoal`,url:m.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await u.leagues.leave(t),p("Has abandonado la liga"),S.navigate("/ligas")}catch(r){p(r.message,"error")}}),(o=document.getElementById("btnJoin"))==null||o.addEventListener("click",async()=>{try{await u.leagues.join({league_id:t}),p("¡Te has unido a la liga!"),S.navigate(`/ligas/${t}`)}catch(r){p(r.message,"error")}}),(d=document.getElementById("btnEditLeague"))==null||d.addEventListener("click",()=>{ta(m,t,b)});const E=document.getElementById("tabRanking"),y=document.getElementById("tabTablon"),L=document.getElementById("sectionRanking"),w=document.getElementById("sectionTablon");E&&y&&(E.addEventListener("click",()=>{E.classList.add("league-tab--active"),y.classList.remove("league-tab--active"),L.classList.remove("hidden"),w.classList.add("hidden")}),y.addEventListener("click",()=>{y.classList.add("league-tab--active"),E.classList.remove("league-tab--active"),L.classList.add("hidden"),w.classList.remove("hidden");const r=document.getElementById("tablonEmbed");r&&!r.dataset.loaded&&(r.dataset.loaded="1",Ee(r,{query:{liga:String(t)}}))}))}catch(g){e.innerHTML=`<div class="container"><p class="form__error">Error: ${g.message}</p><a href="#/ligas">Volver</a></div>`}}function ta(e,a,t){const s=document.getElementById("editLeagueModal");s&&s.remove();const n=document.createElement("div");n.id="editLeagueModal",n.className="edit-league-modal",n.innerHTML=`
  ...
  `,document.body.appendChild(n),requestAnimationFrame(()=>n.classList.add("edit-league-modal--open"));const i=()=>{n.classList.remove("edit-league-modal--open"),n.addEventListener("transitionend",()=>n.remove(),{once:!0})};n.querySelector(".edit-league-modal__overlay").addEventListener("click",i),document.getElementById("btnCancelEdit").addEventListener("click",i),document.getElementById("editLeagueForm").addEventListener("submit",async o=>{o.preventDefault();const d=document.getElementById("btnSaveEdit");d.disabled=!0,d.textContent="Guardando…";const g={name:document.getElementById("editName").value.trim(),description:document.getElementById("editDesc").value.trim(),prize:document.getElementById("editPrize").value.trim(),is_public:document.getElementById("editPublic").checked};t!=null&&t.is_admin&&(g.is_official=document.getElementById("editOfficial").checked);try{await u.leagues.update(a,g),p("Liga actualizada"),i(),S.navigate(`/ligas/${a}`)}catch(m){p(m.message,"error"),d.disabled=!1,d.textContent="Guardar cambios"}})}async function sa(e){var t,s;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=$.getUser();try{const n=(()=>{const f=localStorage.getItem("activeLeagueId");return f?parseInt(f):null})(),[i,o,d,g,m]=await Promise.all([u.predictions.mine(n),u.predictions.getChampion(n),u.leagues.my(),u.auth.me(),a!=null&&a.is_admin?u.leagues.adminAll():Promise.resolve({leagues:[]})]),h=i.predictions.reduce((f,E)=>f+E.total_points,0)+(((t=o.champion_prediction)==null?void 0:t.points_earned)||0),l=g.user,_=l.status,b=l.total_points_all_time;e.innerHTML=`
      <div class="container">
        ...
        <section class="section">
          ...
          ${i.predictions.length?`<div class="predictions-list">${i.predictions.map(ia).join("")}</div>`:'<p class="empty">Sin predicciones aún. <a href="#/quiniela">Ir a la quiniela</a></p>'}
        </section>
    ...
    `,(s=e.querySelector("#btnLogoutPerfil"))==null||s.addEventListener("click",()=>{$.logout(),window.location.hash="/"})}catch(n){e.innerHTML=`<div class="container"><p class="form__error">Error: ${n.message}</p></div>`}}function na(e,a){if(e.next_threshold===null)return`
  ...
  `}const le=new Date("2026-06-11T21:00:00Z"),oa=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];function ra(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function Le(e){var a;if(!$.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const t=ra(),{champion_prediction:s}=await u.predictions.getChampion(t),n=new Date>=le;let i;s&&n?i=`
      ...
      <div class="container">
        ...
        <p class="champion-desc">
          ...
          (${le.toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric",hour:"2-digit",minute:"2-digit"})}).
        </p>
        ${i}
      </div>
    `,(a=document.getElementById("championForm"))==null||a.addEventListener("submit",async o=>{o.preventDefault();const d=document.getElementById("champBtn"),g=document.getElementById("champError"),m=document.getElementById("teamSearch").value.trim();if(m){d.disabled=!0,d.textContent="Guardando…",g.classList.add("hidden");try{await u.predictions.saveChampion(m,t),p(`¡${m} guardado como campeón!`),Le(e)}catch(h){g.textContent=h.message,g.classList.remove("hidden"),d.disabled=!1,d.textContent=d.dataset.label||"Confirmar predicción"}}})}catch(t){e.innerHTML=`<div class="container"><p class="form__error">Error: ${t.message}</p></div>`}}function K(e){const a=e?"Actualizar predicción":"Confirmar predicción";return`
  ...
  `}const da=[{key:"group",label:"Grupos"},{key:"r32",label:"Dieciseisavos"},{key:"r16",label:"Octavos"},{key:"quarters",label:"Cuartos"},{key:"semis",label:"Semis"},{key:"third",label:"3er y 4to"},{key:"final",label:"Final"}];async function la(e){if(!$.isAdmin()){e.innerHTML='<div class="container"><p class="form__error">Acceso denegado.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const[{users:a},{groups:t}]=await Promise.all([u.auth.users(),u.matches.grouped()]),s=ca(t);e.innerHTML=`
      <div class="container">
    ...
    `,ma(e),ee(e)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}function ca(e){const a={};for(const t of e){const s=t.phase;a[s]||(a[s]=[]),a[s].push(...t.matches)}return a}function ua(e){const a=da.filter(n=>{var i;return(i=e[n.key])==null?void 0:i.length});if(!a.length)return'<p class="admin-section__desc">No hay partidos cargados.</p>';const t=a.map((n,i)=>`
  ...
  `}function pa(e){const a=e.status==="finished",t=a&&e.home_score_90!=null?e.home_score_90:"",s=a&&e.away_score_90!=null?e.away_score_90:"",n=a&&e.result_90?e.result_90:"",i=a?'<span class="admin-match-badge admin-match-badge--done">Terminado</span>':'<span class="admin-match-badge admin-match-badge--pending">Pendiente</span>',o=we(e.match_datetime);return`
  ...
  `}function we(e){return e?new Date(e).toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit"}):"—"}function ma(e){var s,n,i,o,d,g;(s=document.getElementById("btnSync"))==null||s.addEventListener("click",async()=>{const m=document.getElementById("syncResult");m.textContent="Sincronizando…";try{await u.matches.sync(),m.textContent="✓ Sincronización completada",p("Sincronización completada")}catch(h){m.textContent=`Error: ${h.message}`,p(h.message,"error")}}),(n=document.getElementById("awardForm"))==null||n.addEventListener("submit",async m=>{m.preventDefault();const h=document.getElementById("winnerTeam").value.trim();if(h)try{const{message:l}=await u.predictions.awardChampion(h);p(l)}catch(l){p(l.message,"error")}}),e.querySelectorAll(".admin-result-tab").forEach(m=>{m.addEventListener("click",()=>{var h;e.querySelectorAll(".admin-result-tab").forEach(l=>l.classList.remove("admin-result-tab--active")),e.querySelectorAll(".admin-result-panel").forEach(l=>l.classList.add("admin-result-panel--hidden")),m.classList.add("admin-result-tab--active"),(h=e.querySelector(`.admin-result-panel[data-phase="${m.dataset.phase}"]`))==null||h.classList.remove("admin-result-panel--hidden")})}),(i=document.getElementById("resultPanels"))==null||i.addEventListener("click",async m=>{var L;const h=m.target.closest(".admin-match-row__save");if(!h)return;const l=h.closest(".admin-match-row"),_=parseInt(l.dataset.id),b=l.querySelectorAll(".admin-match-row__input"),f=b[0].value,E=b[1].value,y=((L=l.querySelector(".admin-match-row__result90"))==null?void 0:L.value)||null;if(f===""||E===""){p("Introduce ambos marcadores","error");return}h.disabled=!0;try{await u.matches.setResult(_,parseInt(f),parseInt(E),y);const w=l.querySelector(".admin-match-badge");w&&(w.className="admin-match-badge admin-match-badge--done",w.textContent="Terminado"),p(`Resultado ${f}-${E} guardado`)}catch(w){p(w.message,"error")}finally{h.disabled=!1}}),(o=document.getElementById("btnRecalcAll"))==null||o.addEventListener("click",async()=>{const m=document.getElementById("btnRecalcAll"),h=document.getElementById("recalcResult");m.disabled=!0,h.textContent="Recalculando…";try{const{message:l}=await u.matches.recalculate();h.textContent=`✓ ${l}`,p(l)}catch(l){h.textContent=`Error: ${l.message}`,p(l.message,"error")}finally{m.disabled=!1}});const a=document.getElementById("pushTarget"),t=document.getElementById("pushTargetIdGroup");a==null||a.addEventListener("change",()=>{t.classList.toggle("hidden",a.value==="all")}),(d=document.getElementById("pushForm"))==null||d.addEventListener("submit",async m=>{m.preventDefault();const h=document.getElementById("pushTitle").value.trim()||"Aviso",l=document.getElementById("pushBody").value.trim(),_=a.value,b=parseInt(document.getElementById("pushTargetId").value)||null,f=document.getElementById("pushResult"),E={title:`📣 PickGoal — ${h}`,body:l};_==="league"&&b&&(E.league_id=b),_==="user"&&b&&(E.user_id=b),f.textContent="Enviando…";try{const{sent:y}=await u.notifications.send(E);f.textContent=`✓ Enviada a ${y} suscripción(es)`,p(`Notificación enviada a ${y} suscripción(es)`)}catch(y){f.textContent=`Error: ${y.message}`,p(y.message,"error")}}),(g=document.getElementById("usersTableBody"))==null||g.addEventListener("click",async m=>{const h=m.target.closest(".toggle-admin");if(!h)return;const l=parseInt(h.dataset.id);try{const{user:_}=await u.auth.toggleAdmin(l);h.closest("tr").querySelector(".admin-badge").textContent=_.is_admin?"Sí":"No",p(`${_.username} ${_.is_admin?"ahora es admin":"ya no es admin"}`)}catch(_){p(_.message,"error")}})}function ga(e){return`
    <tr>
      <td>${e.id}</td>
      <td>${e.username}</td>
      <td>${e.email}</td>
  ...
  `}const _a={PD:"🇪🇸 LaLiga",PL:"🏴󠁧󠁢󠁥󠁮󠁧󠁿 Premier League",CL:"⭐ Champions League"};let k=[],z=null;async function ee(e){const a=document.getElementById("jornadasV2Content");if(a)try{const{jornadas:t}=await u.adminV2.jornadas();a.innerHTML=va(t),ba(a)}catch(t){a.innerHTML=`<p class="form__error">Error: ${t.message}</p>`}}function va(e){const a=new Date;a.getFullYear(),String(Ie(a)).padStart(2,"0");const t=La(a);return`
    <div class="jv2-panel">
      ...
      <div class="jv2-list">
        ${e.length===0?'<p class="admin-section__desc">No hay jornadas creadas.</p>':e.map(ha).join("")}
      </div>
  ...
  `}function ba(e){var a,t,s,n;(a=e.querySelector("#btnNuevaJornada"))==null||a.addEventListener("click",()=>{z=null,k=[],document.getElementById("jv2FormTitle").textContent="Nueva jornada",document.getElementById("jv2EditId").value="",document.getElementById("jv2Number").value="",document.getElementById("jv2DateStart").value="",document.getElementById("jv2DateEnd").value="",document.getElementById("jv2MatchPicker").style.display="none",document.getElementById("jv2Form").style.display="block",ae()}),(t=e.querySelector("#btnCancelarJornada"))==null||t.addEventListener("click",()=>{document.getElementById("jv2Form").style.display="none",k=[],z=null}),(s=e.querySelector("#btnBuscarPartidos"))==null||s.addEventListener("click",ya),(n=e.querySelector("#btnGuardarJornada"))==null||n.addEventListener("click",Ea),e.querySelectorAll(".jv2-edit-btn").forEach(i=>{i.addEventListener("click",()=>fa(i.dataset.id))}),e.querySelectorAll(".jv2-del-btn").forEach(i=>{i.addEventListener("click",async()=>{if(confirm(`¿Eliminar jornada ${i.dataset.num}?`))try{await u.adminV2.deleteJornada(i.dataset.id),p("Jornada eliminada"),ee(document.querySelector("#jornadasV2Content").parentElement.parentElement)}catch(o){p(o.message,"error")}})})}async function fa(e){const{jornadas:a}=await u.adminV2.jornadas(),t=a.find(s=>String(s.id)===String(e));t&&(z=t.id,k=[],document.getElementById("jv2FormTitle").textContent=`Editar jornada ${t.number}`,document.getElementById("jv2EditId").value=t.id,document.getElementById("jv2Number").value=t.number,t.date_start&&(document.getElementById("jv2DateStart").value=t.date_start.slice(0,16)),t.date_end&&(document.getElementById("jv2DateEnd").value=t.date_end.slice(0,16)),document.getElementById("jv2MatchPicker").style.display="none",document.getElementById("jv2Form").style.display="block",ae())}async function ya(){const e=document.getElementById("btnBuscarPartidos"),a=document.getElementById("jv2Week").value;if(!a){p("Selecciona una semana","error");return}e.disabled=!0,e.textContent="Buscando…";try{const{matches:t}=await u.adminV2.partidos(a);$a(t),document.getElementById("jv2MatchPicker").style.display="block"}catch(t){p(`Error: ${t.message}`,"error")}finally{e.disabled=!1,e.textContent="Buscar partidos"}}function $a(e){const a=document.getElementById("jv2MatchList");if(Object.values(e).flat().length===0){a.innerHTML='<p class="admin-section__desc">No hay partidos disponibles para esta semana.</p>';return}a.innerHTML=Object.entries(e).map(([s,n])=>n.length?`
      <div class="jv2-comp-group">
        <div class="jv2-comp-group__title">${_a[s]||s}</div>
        ${n.map(i=>`
          <label class="jv2-match-item">
            ...
            <span class="jv2-match-item__date">${we(i.match_datetime)}</span>
          </label>
        `).join("")}
      </div>
    `:"").join(""),a.querySelectorAll(".jv2-match-check").forEach(s=>{s.addEventListener("change",()=>{const n=JSON.parse(s.dataset.match);if(s.checked){if(k.length>=10){s.checked=!1,p("Máximo 10 partidos","error");return}k.push(n)}else k=k.filter(i=>i.api_id!==n.api_id);ae()})})}function ae(){const e=document.getElementById("jv2Count"),a=document.getElementById("jv2CountWarn");e&&(e.textContent=k.length),a&&(a.style.display=k.length>0&&k.length!==10?"inline":"none")}async function Ea(){const e=parseInt(document.getElementById("jv2Number").value),a=document.getElementById("jv2DateStart").value,t=document.getElementById("jv2DateEnd").value,s=document.getElementById("jv2EditId").value;if(!e||!a||!t){p("Completa número y fechas","error");return}if(k.length!==10){p("Selecciona exactamente 10 partidos","error");return}const n={number:e,date_start:new Date(a).toISOString(),date_end:new Date(t).toISOString(),matches:k},i=document.getElementById("btnGuardarJornada");i.disabled=!0;try{s?(await u.adminV2.updateJornada(s,n),p(`Jornada ${e} actualizada`)):(await u.adminV2.createJornada(n),p(`Jornada ${e} guardada como borrador`)),document.getElementById("jv2Form").style.display="none",k=[],z=null,await ee(document.getElementById("jornadasV2Section"))}catch(o){p(o.message,"error")}finally{i.disabled=!1}}function Ie(e){const a=new Date(Date.UTC(e.getFullYear(),e.getMonth(),e.getDate())),t=a.getUTCDay()||7;a.setUTCDate(a.getUTCDate()+4-t);const s=new Date(Date.UTC(a.getUTCFullYear(),0,1));return Math.ceil(((a-s)/864e5+1)/7)}function La(e){const a=new Date(e);a.setDate(a.getDate()+7);const t=a.getFullYear(),s=String(Ie(a)).padStart(2,"0");return`${t}-W${s}`}function wa(e){e.innerHTML=`
    <div class="auth-container container">
      <div class="auth-card">
  ...
  `,document.getElementById("resetForm").addEventListener("submit",async s=>{s.preventDefault();const n=document.getElementById("resetBtn"),i=document.getElementById("resetError"),o=document.getElementById("password").value;n.disabled=!0,n.textContent="Guardando…",i.classList.add("hidden");try{await u.auth.resetPassword(t,o),p("Contraseña actualizada. Ya puedes iniciar sesión."),S.navigate("/login")}catch(d){i.textContent=d.message||"Error al restablecer la contraseña",i.classList.remove("hidden")}finally{n.disabled=!1,n.textContent="Guardar contraseña"}})}const Sa={r32:"Dieciseisavos",r16:"Octavos",quarters:"Cuartos",semis:"Semis",third:"3º y 4º",final:"Final"};let O=!1,C=null;async function ja(e){O=!1,C=null,e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{groups:a}=await u.matches.grouped(),t=$.isAdmin();e.innerHTML=`
    ...
    `,t&&document.getElementById("btnEditResults").addEventListener("click",()=>{O=!O;const s=document.getElementById("btnEditResults");O?(s.textContent="✅ Editando — salir",s.classList.add("btn--warning")):(s.textContent="✏️ Editar resultados",s.classList.remove("btn--warning")),C&&V(C.data,C.isGroup)}),document.getElementById("phaseContent").addEventListener("click",async s=>{var h;const n=s.target.closest(".res-match__save");if(!n)return;const i=parseInt(n.dataset.id),o=n.closest(".res-match"),d=o.querySelector(".res-match__input-home").value,g=o.querySelector(".res-match__input-away").value,m=((h=o.querySelector(".res-match__result90"))==null?void 0:h.value)||null;if(d===""||g===""){p("Introduce ambos marcadores","error");return}n.disabled=!0;try{if(await u.matches.setResult(i,parseInt(d),parseInt(g),m),p(`${d} - ${g} guardado`),C){const l=C.data.matches.find(_=>_.id===i);l&&(l.home_score_90=parseInt(d),l.away_score_90=parseInt(g),l.status="finished"),V(C.data,C.isGroup)}}catch(l){p(l.message,"error"),n.disabled=!1}}),ka(a)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${a.message}</p></div>`}}function ka(e){var i;const a=document.getElementById("phaseNav");if(!a)return;const t=e.filter(o=>o.phase==="group"),s=e.filter(o=>o.phase!=="group"),n=[...t.map(o=>({key:`group_${o.group_name}`,label:`Grupo ${o.group_name}`,data:o,isGroup:!0})),...s.map(o=>({key:o.phase,label:Sa[o.phase]||o.label,data:o,isGroup:!1}))];n.length!==0&&(a.innerHTML=n.map((o,d)=>`
    <button class="phase-nav__btn ${d===0?"phase-nav__btn--active":""}" data-key="${o.key}">
      ${o.label}
    </button>
  `).join(""),(i=a.querySelector(".phase-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),a.querySelectorAll(".phase-nav__btn").forEach(o=>{o.addEventListener("click",()=>{a.querySelectorAll(".phase-nav__btn").forEach(g=>g.classList.remove("phase-nav__btn--active")),o.classList.add("phase-nav__btn--active");const d=n.find(g=>g.key===o.dataset.key);d&&(C=d,V(d.data,d.isGroup))})}),C=n[0],V(n[0].data,n[0].isGroup))}function V(e,a){const t=document.getElementById("phaseContent");if(!t)return;const s=xa(e.matches);if(a){const n=Ba(e.matches);t.innerHTML=`
    ...
    `}else t.innerHTML=`<div class="resultados-matches">${s}</div>`}function xa(e){return!e||e.length===0?'<p class="empty">Sin partidos en esta fase.</p>':e.map(a=>{const t={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[a.status]||a.status;let s;if(O){const n=a.home_score_90??"",i=a.away_score_90??"",o=a.result_90??"";s=`
      ...
      <div class="res-match ${a.status==="finished"?"res-match--finished":""} ${a.status==="live"?"res-match--live":""} ${O?"res-match--editing":""}">
        <div class="res-match__meta">
          ...
          <span class="res-match__date">${P(a.match_datetime)}</span>
        </div>
  ...
  `}async function Pa(e,{query:a}){const t=(a.codigo||"").trim().toUpperCase();if(!t){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!$.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",t),S.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:s}=await u.leagues.joinByCode(t);p(`¡Te has unido a "${s.name}"!`),S.navigate(`/ligas/${s.id}`)}catch(s){if(s.status===409){p("Ya eres miembro de esta liga");try{const{leagues:n}=await u.leagues.my(),i=n.find(o=>o.invite_code===t);if(i){S.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
      <div class="container">
        <div class="league-gate">
  ...
  `}const J=20,Se=5;let q={},G=0;async function Ha(e){var a,t;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const s=await u.jornada.current();if(!s.jornada){e.innerHTML=qa(s.next_jornada);return}const{jornada:n,matches:i,units_used:o}=s;q={};for(const d of i)q[d.jornada_match_id]={predicted_result:((a=d.prediction)==null?void 0:a.predicted_result)??null,units:((t=d.prediction)==null?void 0:t.units_wagered)??0};G=o,e.innerHTML=`
      <div class="container">
        <h1 class="page-title">Jornada ${n.number} — del ${ce(n.date_start)} al ${ce(n.date_end)}</h1>
        ${n.locked?'<p class="notice">⚠️ El plazo de predicción ha cerrado (ya empezó el primer partido).</p>':""}
        <div class="units-counter" id="unitsCounter"></div>
        <div class="jornada-matches">
          ${i.map(Aa).join("")}
        </div>
        ${n.locked?"":'<button class="btn btn--primary btn--full jornada-save-btn" id="jornadaSaveBtn">Guardar predicciones</button>'}
      </div>
    `,je(),Na(e,n.locked)}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error cargando la jornada: ${s.message}</p></div>`}}function qa(e){return`
    <div class="container">
      <div class="jornada-empty">
        ...
        ${e?`<p class="jornada-empty__text">Próxima jornada: <strong>Jornada ${e.number}</strong> — ${P(e.date_start)}</p>`:'<p class="jornada-empty__text">Todavía no hay una próxima jornada programada.</p>'}
      </div>
    </div>
  `}function ce(e){return new Date(e).toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit"})}function Y(e){return e!=null?e.toFixed(2):"—"}function Aa(e){const a=e.status!=="scheduled"||new Date(e.match_datetime)<=new Date,t=q[e.jornada_match_id]??{predicted_result:null,units:0};return`
    <div class="match-card jornada-match ${a?"match-card--locked":""}" data-jm-id="${e.jornada_match_id}">
      <div class="match-card__header">
        <span class="match-card__date">${P(e.match_datetime)}</span>
        ${a?'<span class="tag tag--locked">Bloqueado</span>':""}
      </div>
      <div class="match-card__teams">
        <span class="team team--home">${e.home_team}</span>
        <div class="match-card__score">
      ...
      <div class="jornada-odds">
        <span class="jornada-odds__item"><b>1</b> (${Y(e.odds_1)})</span>
        <span class="jornada-odds__item"><b>X</b> (${Y(e.odds_x)})</span>
        <span class="jornada-odds__item"><b>2</b> (${Y(e.odds_2)})</span>
      </div>
      <div class="jornada-match__controls ${a?"jornada-match__controls--disabled":""}">
        ...
        <div class="jornada-units">
          ...
          <input type="number" id="units-${e.jornada_match_id}" class="jornada-units__input" min="0" max="${Se}" value="${t.units}" ${a?"disabled":""} />
        </div>
      </div>
    </div>
  `}function je(){const e=document.getElementById("unitsCounter");if(!e)return;const a=G>J;e.innerHTML=`
    <div class="units-counter__bar">
      <div class="units-counter__fill ${a?"units-counter__fill--over":""}" style="width:${Math.min(100,G/J*100)}%"></div>
    </div>
    <span class="units-counter__label ${a?"units-counter__label--over":""}">${G}/${J} unidades usadas</span>
  `}function ue(){G=Object.values(q).reduce((e,a)=>e+(a.predicted_result?a.units:0),0),je()}function Na(e,a){var t;a||(e.querySelectorAll(".jornada-match").forEach(s=>{const n=parseInt(s.dataset.jmId);s.querySelectorAll('input[type="radio"]').forEach(o=>{o.addEventListener("change",()=>{q[n].predicted_result=o.value,ue()})});const i=s.querySelector(".jornada-units__input");i==null||i.addEventListener("input",()=>{let o=parseInt(i.value);isNaN(o)&&(o=0),o=Math.max(0,Math.min(Se,o)),q[n].units=o,ue()})}),(t=document.getElementById("jornadaSaveBtn"))==null||t.addEventListener("click",Oa))}async function Oa(){const e=document.getElementById("jornadaSaveBtn"),a=Object.entries(q).filter(([,t])=>t.predicted_result).map(([t,s])=>({jornada_match_id:parseInt(t),predicted_result:s.predicted_result,units:s.units}));if(a.length===0){p("Selecciona al menos un resultado 1X2","error");return}if(G>J){p(`Superas el máximo de ${J} unidades`,"error");return}e.disabled=!0,e.textContent="…";try{await u.jornada.predict(a),p("Predicciones guardadas"),e.textContent="✓ Guardadas"}catch(t){p(t.message||"Error al guardar","error")}finally{e.disabled=!1,setTimeout(()=>{e&&(e.textContent="Guardar predicciones")},2e3)}}const pe={en_curso:{label:"En curso",cls:"duelo-status--curso"},ganado:{label:"Ganaste",cls:"duelo-status--ganado"},perdido:{label:"Perdiste",cls:"duelo-status--perdido"},empate:{label:"Empate",cls:"duelo-status--empate"}};async function Ra(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{duelo:a}=await u.duelo.current(),t=$.getUser();if(!a){e.innerHTML=`
        <div class="container">
      ...
      `;return}const s=pe[a.status]??pe.en_curso,n=a.rival?a.rival.username:t.username,i=!a.rival||a.rival.id===t.id;e.innerHTML=`
      <div class="container">
        <h1 class="page-title">Tu duelo esta jornada</h1>
    ...
    `,Ga(a.division_league_id,t.id)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando el duelo: ${a.message}</p></div>`}}async function Ga(e,a){const t=document.getElementById("divisionStandings");if(t)try{const{standings:s}=await u.clasificacion.division(e);if(s.length===0){t.innerHTML='<p class="empty">Sin clasificación disponible.</p>';return}t.innerHTML=`
      <div class="ranking-table-wrapper">
        <table class="ranking-table">
          <thead>
            <tr>
    ...
    `}catch(s){t.innerHTML=`<p class="form__error">Error cargando la clasificación: ${s.message}</p>`}}async function Ua(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{standings:a}=await u.clasificacion.general(),t=$.getUser();e.innerHTML=`
      <div class="container">
        <h1 class="page-title">Clasificación</h1>
    ...
    `,Fa(t)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando la clasificación: ${a.message}</p></div>`}}function Fa(e){const a=document.getElementById("tabGeneral"),t=document.getElementById("tabMiDivision"),s=document.getElementById("panelGeneral"),n=document.getElementById("panelMiDivision");!a||!t||(a.addEventListener("click",()=>{a.classList.add("league-tab--active"),t.classList.remove("league-tab--active"),s.classList.remove("hidden"),n.classList.add("hidden")}),t.addEventListener("click",()=>{t.classList.add("league-tab--active"),a.classList.remove("league-tab--active"),n.classList.remove("hidden"),s.classList.add("hidden"),n.dataset.loaded||(n.dataset.loaded="1",Ja(e))}))}async function Ja(e){const a=document.getElementById("panelMiDivision");if(a)try{const{standings:t}=await u.clasificacion.division();if(t.length===0){a.innerHTML='<p class="empty">Todavía no perteneces a ninguna división.</p>';return}a.innerHTML=`
      <div class="ranking-table-wrapper">
        <table class="ranking-table">
          <thead>
            <tr>
          ...
          <tbody>
            ${t.map(s=>`
              <tr class="${e&&s.user_id===e.id?"ranking-table__row--me":""}">
                <td class="ranking-table__pos" data-pos="${s.pos}">${s.pos}</td>
                <td>${s.username}${s.is_bot?" 🤖":""}</td>
                <td class="ranking-table__stat">${s.pj}</td>
                <td class="ranking-table__stat">${s.g}</td>
                <td class="ranking-table__stat">${s.e}</td>
                <td class="ranking-table__stat">${s.p}</td>
                <td class="ranking-table__pts">${s.pts_division}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
      </div>
    `}catch(t){a.innerHTML=`<p class="form__error">Error cargando la clasificación: ${t.message}</p>`}}const za={"/":Me,"/login":Oe,"/register":Re,"/quiniela":Ue,"/resultados":ja,"/ranking":Ye,"/tablon":Ee,"/ligas":Ze,"/ligas/:id":aa,"/perfil":sa,"/campeon":Le,"/admin":la,"/forgot-password":wa,"/reset-password":Ia,"/unirse":Pa,"/jugador/:id":Ma,"/jornada":Ha,"/duelo":Ra,"/tabla-v2":Ua};function Va(e){for(const[a,t]of Object.entries(za)){const s=[],n=new RegExp("^"+a.replace(/:([^/]+)/g,(o,d)=>(s.push(d),"([^/]+)"))+"$"),i=e.match(n);if(i){const o={};return s.forEach((d,g)=>{o[d]=i[g+1]}),{handler:t,params:o}}}return null}const me=()=>document.getElementById("mainContent"),S={init(){window.addEventListener("hashchange",()=>this.resolve()),this.resolve()},navigate(e){window.location.hash=e},resolve(){const e=window.location.hash.slice(1)||"/",[a,t]=e.split("?"),s=Object.fromEntries(new URLSearchParams(t||"")),n=Va(a);if(!n){me().innerHTML='<div class="error-page"><h2>Página no encontrada</h2><a href="#/">Volver al inicio</a></div>';return}const{handler:i,params:o}=n;if(["/perfil","/campeon","/admin","/jornada","/duelo","/tabla-v2"].includes(a)&&!$.isLoggedIn()){this.navigate("/login");return}if(a==="/admin"&&!$.isAdmin()){this.navigate("/");return}const g=me();g.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>',i(g,{params:o,query:s})}};let F=[],R=null,A=null;async function Xa(){await $.init(),S.init(),Ya(),Wa(),et()}function ke(){return localStorage.getItem("pwa_installed")==="true"||window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===!0}function Wa(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),!ke()&&(R=e,Ka())}),window.addEventListener("appinstalled",()=>{var e;localStorage.setItem("pwa_installed","true"),R=null,(e=document.getElementById("installBanner"))==null||e.remove()})}function Ka(){if(ke()||sessionStorage.getItem("installBannerDismissed"))return;const e=document.createElement("div");e.id="installBanner",e.className="install-banner",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById("installBtn").addEventListener("click",async()=>{R&&(R.prompt(),await R.userChoice,R=null,e.remove())}),document.getElementById("installDismissBtn").addEventListener("click",()=>{sessionStorage.setItem("installBannerDismissed","1"),e.remove()})}function N(){var e,a;(e=document.getElementById("userDropdown"))==null||e.classList.add("hidden"),(a=document.getElementById("userBtn"))==null||a.classList.remove("navbar__dropdown-btn--open")}async function Z(){const e=document.getElementById("tablonBadge");if(!e)return;if(!$.getUser()){e.classList.add("hidden");return}const t=localStorage.getItem("activeLeagueId");if(!t){e.classList.add("hidden");return}const s=localStorage.getItem(`tablon_last_read_${t}`)||new Date(0).toISOString();try{const{count:n}=await u.board.unread(parseInt(t),s);n>0?(e.textContent=n>99?"99+":String(n),e.classList.remove("hidden")):e.classList.add("hidden")}catch{e.classList.add("hidden")}}function Ya(){var e,a,t;document.addEventListener("auth:change",ge),window.addEventListener("hashchange",()=>{N(),xe(),setTimeout(Z,200)}),document.addEventListener("click",N),(e=document.getElementById("userBtn"))==null||e.addEventListener("click",s=>{var o;s.stopPropagation();const n=document.getElementById("userDropdown"),i=n==null?void 0:n.classList.contains("hidden");N(),i&&(n==null||n.classList.remove("hidden"),(o=document.getElementById("userBtn"))==null||o.classList.add("navbar__dropdown-btn--open"))}),(a=document.getElementById("userDropdown"))==null||a.addEventListener("click",s=>{s.stopPropagation(),s.target.closest("#navProfileLink")&&N()}),(t=document.getElementById("navLogoutBtn"))==null||t.addEventListener("click",()=>{F=[],localStorage.removeItem("activeLeagueId"),N(),$.logout(),S.navigate("/")}),ge()}async function ge(){const e=document.getElementById("navAuthLinks"),a=document.getElementById("userBtn"),t=document.getElementById("navUsername"),s=document.getElementById("bottomNav"),n=$.getUser();if(N(),n){e==null||e.classList.add("hidden"),t&&(t.textContent=n.username),a.style.visibility="visible",s==null||s.classList.remove("hidden"),document.body.classList.add("has-bottom-nav");try{const{leagues:i}=n.is_admin?await u.leagues.adminAll():await u.leagues.my();F=i}catch{F=[]}Qa(F),Z(),A&&clearInterval(A),A=setInterval(Z,5*60*1e3)}else e==null||e.classList.remove("hidden"),a.style.visibility="hidden",s==null||s.classList.add("hidden"),document.body.classList.remove("has-bottom-nav"),F=[],localStorage.removeItem("activeLeagueId"),A&&(clearInterval(A),A=null);xe()}function Qa(e){const a=localStorage.getItem("activeLeagueId");a&&e.some(s=>String(s.id)===String(a))||(e.length>0?localStorage.setItem("activeLeagueId",String(e[0].id)):localStorage.removeItem("activeLeagueId"))}function xe(){const e=window.location.hash.slice(1).split("?")[0]||"/";document.querySelectorAll(".bottom-nav__item").forEach(a=>{const t=a.dataset.route,s=t==="/"?e==="/":e===t||e.startsWith(t+"/");a.classList.toggle("bottom-nav__item--active",s)})}function Za(e){const a="=".repeat((4-e.length%4)%4),t=(e+a).replace(/-/g,"+").replace(/_/g,"/"),s=atob(t);return Uint8Array.from([...s].map(n=>n.charCodeAt(0)))}async function et(){if(!(!("serviceWorker"in navigator)||!("PushManager"in window)))try{const e=await navigator.serviceWorker.register("/sw.js");document.addEventListener("auth:change",async a=>{a.detail&&await _e(e)}),$.getUser()&&await _e(e)}catch{}}async function _e(e){try{if(await Notification.requestPermission()!=="granted")return;const t=await e.pushManager.getSubscription();if(t){await u.notifications.subscribe(t.toJSON());return}const{public_key:s}=await u.notifications.vapidPublicKey();if(!s)return;const n=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:Za(s)});await u.notifications.subscribe(n.toJSON())}catch{}}Xa();
