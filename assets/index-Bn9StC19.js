(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const U="https://pickgoal-backend.onrender.com/api";function R(){return localStorage.getItem("token")}async function u(e,a={}){const t={"Content-Type":"application/json",...a.headers},n=R();n&&(t.Authorization=`Bearer ${n}`);const s=await fetch(`${U}${e}`,{...a,headers:t}),i=await s.json().catch(()=>({}));if(!s.ok)throw{status:s.status,message:i.error||"Error desconocido"};return i}const l={get:e=>u(e),post:(e,a)=>u(e,{method:"POST",body:JSON.stringify(a)}),patch:(e,a)=>u(e,{method:"PATCH",body:JSON.stringify(a)}),delete:e=>u(e,{method:"DELETE"}),auth:{register:e=>u("/auth/register",{method:"POST",body:JSON.stringify(e)}),login:e=>u("/auth/login",{method:"POST",body:JSON.stringify(e)}),me:()=>u("/auth/me"),forgotPassword:e=>u("/auth/forgot-password",{method:"POST",body:JSON.stringify({email:e})}),resetPassword:(e,a)=>u("/auth/reset-password",{method:"POST",body:JSON.stringify({token:e,password:a})}),ranking:()=>u("/auth/ranking"),users:()=>u("/auth/users"),toggleAdmin:e=>u(`/auth/users/${e}/toggle-admin`,{method:"PATCH"})},matches:{grouped:()=>u("/matches/grouped"),list:(e="")=>u(`/matches/${e}`),get:e=>u(`/matches/${e}`),sync:()=>u("/matches/sync",{method:"POST"})},predictions:{mine:e=>u(`/predictions/${e?`?league_id=${e}`:""}`),forMatch:(e,a)=>u(`/predictions/match/${e}${a?`?league_id=${a}`:""}`),save:e=>u("/predictions/",{method:"POST",body:JSON.stringify(e)}),forUser:(e,a)=>u(`/predictions/user/${e}${a?`?league_id=${a}`:""}`),getChampion:e=>u(`/predictions/champion${e?`?league_id=${e}`:""}`),saveChampion:(e,a)=>u("/predictions/champion",{method:"POST",body:JSON.stringify({team_name:e,league_id:a??null})}),awardChampion:e=>u("/predictions/champion/award",{method:"POST",body:JSON.stringify({team_name:e})})},leagues:{all:()=>u("/leagues/all"),public:()=>u("/leagues/public"),my:()=>u("/leagues/my"),create:e=>u("/leagues/",{method:"POST",body:JSON.stringify(e)}),join:e=>u("/leagues/join",{method:"POST",body:JSON.stringify(e)}),joinByCode:e=>u(`/leagues/join/${encodeURIComponent(e)}`),get:e=>u(`/leagues/${e}`),leave:e=>u(`/leagues/${e}/leave`,{method:"DELETE"}),matchPredictions:(e,a)=>u(`/leagues/${e}/predictions/${a}`)},board:{messages:(e=1)=>u(`/board/?page=${e}`),post:e=>u("/board/",{method:"POST",body:JSON.stringify({message:e})}),delete:e=>u(`/board/${e}`,{method:"DELETE"})}};let y=null;const h={async init(){if(localStorage.getItem("token"))try{const{user:a}=await l.auth.me();y=a}catch{localStorage.removeItem("token")}},setUser(e,a){y=e,localStorage.setItem("token",a),document.dispatchEvent(new CustomEvent("auth:change",{detail:e}))},logout(){y=null,localStorage.removeItem("token"),document.dispatchEvent(new CustomEvent("auth:change",{detail:null}))},getUser(){return y},isLoggedIn(){return!!y},isAdmin(){return(y==null?void 0:y.is_admin)===!0}};function G(e){const a=h.getUser();e.innerHTML=`
  ...
  `,document.getElementById("registerForm").addEventListener("submit",async a=>{a.preventDefault();const t=document.getElementById("registerBtn"),n=document.getElementById("registerError");t.disabled=!0,t.textContent="Creando cuenta…",n.classList.add("hidden");const s={username:document.getElementById("username").value.trim(),email:document.getElementById("email").value.trim(),country:document.getElementById("country").value.trim(),password:document.getElementById("password").value};try{const{token:i,user:r}=await l.auth.register(s);h.setUser(r,i),m("¡Cuenta creada! Bienvenido a PickGoal");const o=sessionStorage.getItem("pendingInviteCode");if(o){sessionStorage.removeItem("pendingInviteCode");try{const{league:c}=await l.leagues.joinByCode(o);m(`¡Te has unido a "${c.name}"!`),_.navigate(`/ligas/${c.id}`)}catch{_.navigate("/ligas")}}else _.navigate("/campeon")}catch(i){n.textContent=i.message||"Error al registrarse",n.classList.remove("hidden")}finally{t.disabled=!1,t.textContent="Crear cuenta"}})}function J(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function V(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:d}=await l.leagues.my();if(d.length===0){e.innerHTML=N();return}}const a=J(),[{groups:t},n]=await Promise.all([l.matches.grouped(),h.isLoggedIn()?l.predictions.mine(a):Promise.resolve({predictions:[]})]),s={};for(const d of n.predictions)s[d.match_id]=d;const i=t.flatMap(d=>d.matches),r=new Map;for(const d of i){const g=k(d.match_datetime);r.has(g)||r.set(g,[]),r.get(g).push(d)}const o=[...r.keys()].sort(),c=k(new Date().toISOString()),p=o.find(d=>d>=c)??o[0];e.innerHTML=`
    ...
    `,X(o,p,r,s,a),h.isLoggedIn()&&W(i,s,a)}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error cargando los partidos: ${a.message}</p></div>`}}function X(e,a,t,n,s){var r;const i=document.getElementById("dateNav");i&&(i.innerHTML=e.map(o=>`
    <button class="date-nav__btn ${o===a?"date-nav__btn--active":""}" data-day="${o}">
      ${Y(o)}
    </button>
  `).join(""),(r=i.querySelector(".date-nav__btn--active"))==null||r.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),i.querySelectorAll(".date-nav__btn").forEach(o=>{o.addEventListener("click",()=>{i.querySelectorAll(".date-nav__btn").forEach(c=>c.classList.remove("date-nav__btn--active")),o.classList.add("date-nav__btn--active"),P(t.get(o.dataset.day)??[],n,s)})}),P(t.get(a)??[],n,s))}function P(e,a,t){const n=document.getElementById("matchesContent");if(n){if(e.length===0){n.innerHTML='<p class="empty">Sin partidos este día.</p>';return}n.innerHTML=`<div class="matches-grid">${e.map(s=>Q(s,a[s.id])).join("")}</div>`,h.isLoggedIn()&&n.querySelectorAll(".prediction-form").forEach(s=>{Z(s,a,t)})}}function k(e){const a=new Date(e);return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`}function Y(e){const[a,t,n]=e.split("-").map(Number);return new Date(a,t-1,n).toLocaleDateString("es-ES",{day:"numeric",month:"short"})}function Q(e,a){const t=e.is_locked,n=a?`<span class="pts-badge">${a.total_points} pts</span>`:"",s={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[e.status];return`
    <div class="match-card ${t?"match-card--locked":""}" data-match-id="${e.id}">
      ...
      ${!t&&h.isLoggedIn()?K(e,a):t&&a?`<div class="prediction-result">
  ...
  `}async function W(e,a,t){const n=e.filter(s=>!s.is_locked&&!a[s.id]);for(const s of n)try{const{prediction:i}=await l.predictions.save({match_id:s.id,predicted_result:"X",predicted_home:0,predicted_away:0,league_id:t??null});a[s.id]=i}catch{}}function Z(e,a,t){e.addEventListener("submit",async n=>{var p;n.preventDefault();const s=parseInt(e.dataset.matchId),i=parseInt(e.querySelector("[name=predicted_home]").value),r=parseInt(e.querySelector("[name=predicted_away]").value),o=(p=e.querySelector("[name=predicted_result]:checked"))==null?void 0:p.value;if(isNaN(i)||isNaN(r)||!o)return;const c=e.querySelector("button");c.disabled=!0,c.textContent="…";try{const{prediction:d}=await l.predictions.save({match_id:s,predicted_result:o,predicted_home:i,predicted_away:r,league_id:t??null});a[s]=d,m("Predicción guardada"),c.textContent="✓ Guardado",setTimeout(()=>{c.disabled=!1,c.textContent="Guardar"},2e3)}catch(d){m(d.message||"Error al guardar","error"),c.disabled=!1,c.textContent="Guardar"}})}async function ee(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{if(h.isLoggedIn()){const{leagues:n}=await l.leagues.my();if(n.length===0){e.innerHTML=N();return}}const{ranking:a}=await l.auth.ranking(),t=h.getUser();e.innerHTML=`
      <div class="container">
        ...
        <div class="ranking-table-wrapper">
          <table class="ranking-table">
            ...
            <tbody>
              ${a.map(n=>`
                <tr class="${t&&n.id===t.id?"ranking-table__row--me":""}">
                  <td class="ranking-table__pos" data-pos="${n.position}">${n.position}</td>
                  <td><a class="ranking-table__link" href="#/jugador/${n.id}">${n.username}</a></td>
                  <td>${n.country||"—"}</td>
    ...
    `}catch(a){e.innerHTML=`<div class="container"><p class="form__error">Error: ${a.message}</p></div>`}}async function ae(e){e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';let a=1,t=1;async function n(){const{messages:o,pages:c}=await l.board.messages(a);return t=c,o}try{const o=await n();s(o)}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p></div>`}function s(o){var p,d,g;const c=h.getUser();e.innerHTML=`
      <div class="container">
  ...
  `}function ne(e){var n,s;const a=e.invite_link||"",t=document.getElementById("createLeaguePanel");t&&(t.innerHTML=`
  ...
  `,(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a),m("Enlace copiado")}catch{m("No se pudo copiar","error")}}),(s=document.getElementById("btnShare"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${e.name} en PickGoal`,url:a})}catch{}}))}async function ie(e,{params:a}){var n,s,i,r;const t=parseInt(a.id);e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:o,ranking:c,is_member:p}=await l.leagues.get(t),d=h.getUser(),g=o.is_official?'<span class="league-badge league-badge--official">⭐ Oficial</span>':"";e.innerHTML=`
      <div class="container">
    ...
    `,(n=document.getElementById("btnCopyInvite"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(o.invite_link),m("Enlace copiado")}catch{m("No se pudo copiar","error")}}),(s=document.getElementById("btnShareInvite"))==null||s.addEventListener("click",async()=>{try{await navigator.share({title:`Únete a ${o.name} en PickGoal`,url:o.invite_link})}catch{}}),(i=document.getElementById("btnLeave"))==null||i.addEventListener("click",async()=>{if(confirm("¿Seguro que quieres abandonar esta liga?"))try{await l.leagues.leave(t),m("Has abandonado la liga"),_.navigate("/ligas")}catch(v){m(v.message,"error")}}),(r=document.getElementById("btnJoin"))==null||r.addEventListener("click",async()=>{try{await l.leagues.join({league_id:t}),m("¡Te has unido a la liga!"),_.navigate(`/ligas/${t}`)}catch(v){m(v.message,"error")}})}catch(o){e.innerHTML=`<div class="container"><p class="form__error">Error: ${o.message}</p><a href="#/ligas">Volver</a></div>`}}async function re(e){var t,n;e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';const a=h.getUser();try{const[s,i,r]=await Promise.all([l.predictions.mine(),l.predictions.getChampion(),l.leagues.my()]),o=s.predictions.reduce((c,p)=>c+p.total_points,0)+(((t=i.champion_prediction)==null?void 0:t.points_earned)||0);e.innerHTML=`
      <div class="container">
        ...
        <section class="section">
          ...
          ${s.predictions.length?`<div class="predictions-list">${s.predictions.map(oe).join("")}</div>`:'<p class="empty">Sin predicciones aún. <a href="#/quiniela">Ir a la quiniela</a></p>'}
        </section>
    ...
    `,(n=e.querySelector("#btnLogoutPerfil"))==null||n.addEventListener("click",()=>{h.logout(),window.location.hash="/"})}catch(s){e.innerHTML=`<div class="container"><p class="form__error">Error: ${s.message}</p></div>`}}function oe(e){return`
  ...
  `}const M=new Date("2026-06-11T00:00:00Z"),ce=["Argentina","Brasil","Francia","España","Inglaterra","Alemania","Portugal","Países Bajos","Italia","Bélgica","Uruguay","Colombia","México","Estados Unidos","Canadá","Marruecos","Senegal","Nigeria","Japón","Corea del Sur","Australia","Arabia Saudí","Irán","Qatar","Ecuador","Chile","Perú","Venezuela","Bolivia","Paraguay","Costa Rica","Honduras","Panamá","Jamaica","Trinidad y Tobago","Guatemala","Turquía","Polonia","Croacia","Serbia","República Checa","Eslovaquia","Austria","Suiza","Dinamarca","Suecia","Noruega","Escocia","Ucrania","Rumanía","Hungría","Grecia","Egipto","Camerún","Ghana","Costa de Marfil","Túnez","Argelia","China","India","Irak","Uzbekistán","Nueva Zelanda","Fiji"];async function q(e){var a;if(!h.isLoggedIn()){e.innerHTML='<div class="container"><p class="notice"><a href="#/login">Inicia sesión</a> para predecir el campeón.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{champion_prediction:t}=await l.predictions.getChampion(),n=new Date>=M;e.innerHTML=`
      <div class="container">
  ...
  `).join(""),(i=a.querySelector(".phase-nav__btn--active"))==null||i.scrollIntoView({inline:"center",behavior:"instant",block:"nearest"}),a.querySelectorAll(".phase-nav__btn").forEach((r,o)=>{r.addEventListener("click",()=>{a.querySelectorAll(".phase-nav__btn").forEach(p=>p.classList.remove("phase-nav__btn--active")),r.classList.add("phase-nav__btn--active");const c=s.find(p=>p.key===r.dataset.key);c&&x(c.data,c.isGroup)})}),x(s[0].data,s[0].isGroup))}function x(e,a){const t=document.getElementById("phaseContent");if(!t)return;const n=he(e.matches);if(a){const s=_e(e.matches);t.innerHTML=`
    ...
    `}else t.innerHTML=`<div class="resultados-matches">${n}</div>`}function he(e){return!e||e.length===0?'<p class="empty">Sin partidos en esta fase.</p>':e.map(a=>{const t={scheduled:"Programado",live:"🔴 En juego",finished:"Finalizado"}[a.status]||a.status,n=a.status!=="scheduled"?`<span class="res-score">${a.home_score_90??"?"} - ${a.away_score_90??"?"}</span>`:'<span class="res-score res-score--pending">vs</span>';return`
    ...
    `}).join("")}function _e(e){const a={};for(const t of e)if(a[t.home_team]||(a[t.home_team]={name:t.home_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),a[t.away_team]||(a[t.away_team]={name:t.away_team,pj:0,g:0,e:0,p:0,gf:0,gc:0,pts:0}),t.status==="finished"&&t.home_score_90!==null&&t.away_score_90!==null){const n=a[t.home_team],s=a[t.away_team];n.pj++,s.pj++,n.gf+=t.home_score_90,n.gc+=t.away_score_90,s.gf+=t.away_score_90,s.gc+=t.home_score_90,t.home_score_90>t.away_score_90?(n.g++,n.pts+=3,s.p++):t.home_score_90<t.away_score_90?(s.g++,s.pts+=3,n.p++):(n.e++,n.pts++,s.e++,s.pts++)}return Object.values(a).sort((t,n)=>{if(n.pts!==t.pts)return n.pts-t.pts;const s=n.gf-n.gc,i=t.gf-t.gc;return s!==i?s-i:n.gf-t.gf})}function be(e){return e.length===0?'<p class="empty">Sin datos de clasificación.</p>':`
  ...
  `}async function fe(e,{query:a}){const t=(a.codigo||"").trim().toUpperCase();if(!t){e.innerHTML='<div class="container"><p class="form__error">Enlace de invitación inválido.</p><a href="#/ligas">Ver ligas</a></div>';return}if(!h.isLoggedIn()){sessionStorage.setItem("pendingInviteCode",t),_.navigate("/register");return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const{league:n}=await l.leagues.joinByCode(t);m(`¡Te has unido a "${n.name}"!`),_.navigate(`/ligas/${n.id}`)}catch(n){if(n.status===409){m("Ya eres miembro de esta liga");try{const{leagues:s}=await l.leagues.my(),i=s.find(r=>r.invite_code===t);if(i){_.navigate(`/ligas/${i.id}`);return}}catch{}}e.innerHTML=`
    ...
    `}}function ye(){const e=localStorage.getItem("activeLeagueId");return e?parseInt(e):null}async function $e(e,{params:a}){const t=parseInt(a.id);if(!t){e.innerHTML='<div class="container"><p class="form__error">Usuario no válido.</p></div>';return}e.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>';try{const n=ye(),{user:s,predictions:i}=await l.predictions.forUser(t,n);e.innerHTML=`
      <div class="container">
  ...
  `}const Le={"/":G,"/login":F,"/register":z,"/quiniela":V,"/resultados":ge,"/ranking":ee,"/tablon":ae,"/ligas":se,"/ligas/:id":ie,"/perfil":re,"/campeon":q,"/admin":de,"/forgot-password":ue,"/reset-password":me,"/unirse":fe,"/jugador/:id":$e};function we(e){for(const[a,t]of Object.entries(Le)){const n=[],s=new RegExp("^"+a.replace(/:([^/]+)/g,(r,o)=>(n.push(o),"([^/]+)"))+"$"),i=e.match(s);if(i){const r={};return n.forEach((o,c)=>{r[o]=i[c+1]}),{handler:t,params:r}}}return null}const j=()=>document.getElementById("mainContent"),_={init(){window.addEventListener("hashchange",()=>this.resolve()),this.resolve()},navigate(e){window.location.hash=e},resolve(){const e=window.location.hash.slice(1)||"/",[a,t]=e.split("?"),n=Object.fromEntries(new URLSearchParams(t||"")),s=we(a);if(!s){j().innerHTML='<div class="error-page"><h2>Página no encontrada</h2><a href="#/">Volver al inicio</a></div>';return}const{handler:i,params:r}=s;if(["/perfil","/campeon","/admin"].includes(a)&&!h.isLoggedIn()){this.navigate("/login");return}if(a==="/admin"&&!h.isAdmin()){this.navigate("/");return}const c=j();c.innerHTML='<div class="loading"><div class="loading__spinner"></div></div>',i(c,{params:r,query:n})}};let $=[],L=null;async function Ie(){await h.init(),_.init(),Ce(),Be()}function Be(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),L=e,Se()})}function Se(){if(sessionStorage.getItem("installBannerDismissed"))return;const e=document.createElement("div");e.id="installBanner",e.className="install-banner",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById("installBtn").addEventListener("click",async()=>{L&&(L.prompt(),await L.userChoice,L=null,e.remove())}),document.getElementById("installDismissBtn").addEventListener("click",()=>{sessionStorage.setItem("installBannerDismissed","1"),e.remove()})}function f(){var e,a,t,n;(e=document.getElementById("leagueDropdown"))==null||e.classList.add("hidden"),(a=document.getElementById("leagueBtn"))==null||a.classList.remove("navbar__dropdown-btn--open"),(t=document.getElementById("userDropdown"))==null||t.classList.add("hidden"),(n=document.getElementById("userBtn"))==null||n.classList.remove("navbar__dropdown-btn--open")}function Ce(){var e,a,t,n,s;document.addEventListener("auth:change",H),window.addEventListener("hashchange",()=>{f(),A()}),document.addEventListener("click",f),(e=document.getElementById("leagueBtn"))==null||e.addEventListener("click",i=>{var c;i.stopPropagation();const r=document.getElementById("leagueDropdown"),o=r==null?void 0:r.classList.contains("hidden");f(),o&&(r==null||r.classList.remove("hidden"),(c=document.getElementById("leagueBtn"))==null||c.classList.add("navbar__dropdown-btn--open"))}),(a=document.getElementById("leagueDropdown"))==null||a.addEventListener("click",i=>{i.stopPropagation();const r=i.target.closest("[data-league-id]");if(r){localStorage.setItem("activeLeagueId",r.dataset.leagueId),f(),D($);return}i.target.closest("a")&&f()}),(t=document.getElementById("userBtn"))==null||t.addEventListener("click",i=>{var c;i.stopPropagation();const r=document.getElementById("userDropdown"),o=r==null?void 0:r.classList.contains("hidden");f(),o&&(r==null||r.classList.remove("hidden"),(c=document.getElementById("userBtn"))==null||c.classList.add("navbar__dropdown-btn--open"))}),(n=document.getElementById("userDropdown"))==null||n.addEventListener("click",i=>{i.stopPropagation(),i.target.closest("#navProfileLink")&&f()}),(s=document.getElementById("navLogoutBtn"))==null||s.addEventListener("click",()=>{$=[],localStorage.removeItem("activeLeagueId"),f(),h.logout(),_.navigate("/")}),H()}async function H(){const e=document.getElementById("navAuthLinks"),a=document.getElementById("userBtn"),t=document.getElementById("navUsername"),n=document.getElementById("navLeague"),s=document.getElementById("bottomNav"),i=h.getUser();if(f(),i){e==null||e.classList.add("hidden"),t&&(t.textContent=i.username),n.style.visibility="visible",a.style.visibility="visible",s==null||s.classList.remove("hidden"),document.body.classList.add("has-bottom-nav");try{const{leagues:r}=await l.leagues.my();$=r}catch{$=[]}D($)}else e==null||e.classList.remove("hidden"),n.style.visibility="hidden",a.style.visibility="hidden",s==null||s.classList.add("hidden"),document.body.classList.remove("has-bottom-nav"),$=[],localStorage.removeItem("activeLeagueId");A()}function D(e){const a=document.getElementById("leagueDropdown"),t=document.getElementById("navLeagueName");if(!a||!t)return;let n=localStorage.getItem("activeLeagueId"),s=e.find(r=>String(r.id)===String(n));!s&&e.length>0&&(s=e[0],localStorage.setItem("activeLeagueId",String(s.id))),s||localStorage.removeItem("activeLeagueId"),t.textContent=s?s.name:"Inicia Liga";const i=e.map(r=>`
  ...
  `}function A(){const e=window.location.hash.slice(1).split("?")[0]||"/";document.querySelectorAll(".bottom-nav__item").forEach(a=>{const t=a.dataset.route,n=t==="/"?e==="/":e===t||e.startsWith(t+"/");a.classList.toggle("bottom-nav__item--active",n)})}Ie();
